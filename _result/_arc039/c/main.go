...
func solve2(n int, s string) (int, int) {
...
func solve(n int, s string) (int, int) {
	...
	for i := 0; i < n; i++ {
		...
		mp[[2]int{nx, ny}] = []int{nf[0], nf[1], nf[2], nf[3]}
		switch d {
		case 0:
			xy = [2]int{nx, nf[1]}
			mp[xy][0] = ny
		...
		if d != 0 {
			if v, ok := mp[[2]int{nx, ny - 1}]; ok {
				mp[[2]int{nx, nf[0]}][1] = v[1]
				mp[[2]int{nx, ny}][1] = v[1]
				mp[[2]int{nx, v[1]}][0] = mp[[2]int{nx, nf[0]}][0]
			}
		}
		if d != 1 {
			if v, ok := mp[[2]int{nx, ny + 1}]; ok {
				mp[[2]int{nx, nf[1]}][0] = v[0]
				mp[[2]int{nx, ny}][0] = v[0]
				mp[[2]int{nx, v[0]}][1] = mp[[2]int{nx, nf[1]}][1]
			}
		}
		if d != 2 {
			if v, ok := mp[[2]int{nx + 1, ny}]; ok {
				mp[[2]int{nf[2], ny}][3] = v[3]
				mp[[2]int{nx, ny}][3] = v[3]
				mp[[2]int{v[3], ny}][2] = mp[[2]int{nf[2], ny}][2]
			}
		}
		if d != 3 {
			if v, ok := mp[[2]int{nx - 1, ny}]; ok {
				mp[[2]int{nf[3], ny}][2] = v[2]
				mp[[2]int{nx, ny}][2] = v[2]
				mp[[2]int{v[2], ny}][3] = mp[[2]int{nf[3], ny}][3]
			}
		}
		if d != 0 {
			if v, ok := mp[[2]int{nx, ny - 1}]; ok {
