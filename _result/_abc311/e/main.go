...
func main() {
	...
	for i := 0; i < n; i++ {
		hs[as[i]] = append(hs[as[i]], bs[i])
		ws[bs[i]] = append(ws[bs[i]], as[i])
		mp[[2]int{as[i], bs[i]}] = true
	}
	for i := 1; i <= h; i++ {
		hs[i] = append([]int{0}, hs[i]...)
		sorti(hs[i])
	}
	for i := 1; i <= w; i++ {
		ws[i] = append([]int{0}, ws[i]...)
		sorti(ws[i])
	}
	dp := i2s(h+1, w+1, 0)
	for i := 1; i <= h; i++ {
		for j := 1; j <= w; j++ {
			...
			th := i - ws[j][lowerBound(i, ws[j])]
			tw := j - hs[i][lowerBound(j, hs[i])]
			dp[i][j] = min(min(dp[i-1][j-1]+1, th), tw)
			o += dp[i][j]
		}
	}
