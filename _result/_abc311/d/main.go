	...
	for i := 0; i < n; i++ {
		...
		for j := 0; j < m; j++ {
			if mp[i][j] == 0 {
				...
				if t != -1 {
					for k := t; k < j; k++ {
						edges[i*m+k] = append(edges[i*m+k], edge{to: i*m + t, cost: 1})
						edges[i*m+k] = append(edges[i*m+k], edge{to: i*m + j - 1, cost: 1})
					}
				}
				t = -1
			}
		...
		for i := 0; i < n; i++ {
			if mp[i][j] == 0 {
				...
				if t != -1 {
					//out(t*m+j, (i-1)*m+j)
					for k := t; k < i; k++ {
						edges[k*m+j] = append(edges[k*m+j], edge{to: t*m + j, cost: 1})
						edges[k*m+j] = append(edges[k*m+j], edge{to: (i-1)*m + j, cost: 1})
					}
				}
				t = -1
			}
