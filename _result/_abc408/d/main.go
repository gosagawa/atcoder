...
func solve() {
	...
	for i := 0; i < n; i++ {
		if as[i] == 0 {
			...
			dp[i+1][1] = min(dp[i+1][1], safeadd(dp[i][0], 1))
			dp[i+1][1] = min(dp[i+1][1], safeadd(dp[i][1], 1))
			dp[i+1][2] = min(dp[i+1][2], dp[i][1])
			dp[i+1][2] = min(dp[i+1][2], dp[i][2])
		} else {
			dp[i+1][0] = min(dp[i+1][0], safeadd(dp[i][0], 1))
			dp[i+1][1] = min(dp[i+1][1], dp[i][0])
			dp[i+1][1] = min(dp[i+1][1], dp[i][1])
			dp[i+1][2] = min(dp[i+1][2], safeadd(dp[i][1], 1))
			dp[i+1][2] = min(dp[i+1][2], safeadd(dp[i][2], 1))
		}
		dbg(i, as[i])
		dbg(dp[i+1])
	}
	o = min(min(dp[n][0], dp[n][1]), dp[n][2])
	out(o)
}
