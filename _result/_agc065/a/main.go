...
func main() {
	...
	for i := 0; i < 100; i++ {
		...
		v2 := solve2(n, k, ns2, -1)
		if v1 != v2 {
			out(ns2, solve(n, k, ns), solve2(n, k, ns2, v2))
			return
		}
...
func gv2(k, f, t int) int {
	return (t - f + k) % k
}

func solve(n, k int, sl []int) int {
	ns := make([]int, n)
	copy(ns, sl)
	mp := make(map[int]int)
	i := 0
	...
	for _, v := range ns {
		if _, ok := mp[v]; !ok {
			mp[v] = i
			i++
			ts = append(ts, [2]int{v, 1})
		} else {
			ts[mp[v]][1]++
		}
	}
	sort2ar(ts, opt2ar(1, desc), opt2ar(0, desc))
	...
	check := func(c int) int {
		...
		nc := 0
		for j := 0; j < n-1; j++ {
			nc, has = s.LowerBound(c)
			if !has {
				nc, has = s.LowerBound(k)
			}
			t += (nc - c + k) % k
			c = nc
			s.Decrement(c)
...
func solve2(n, k int, ns []int, check int) int {
	sorti(ns)
	o := 0
	for {
		to := 0
