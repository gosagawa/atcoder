...
func main() {
	...
	cusum2dj := newCusum2d(m, n)
	cusum2do := newCusum2d(m, n)
	cusum2di := newCusum2d(m, n)
	for i := 0; i < m; i++ {
		s := ns()
		for j, v := range s {
			switch string(v) {
	...
	for i := 0; i < k; i++ {
		...
		a--
		b--
		sj := cusum2dj.get(a, b, c, d)
		so := cusum2do.get(a, b, c, d)
		si := cusum2di.get(a, b, c, d)
		out(fmt.Sprintf("%d %d %d", sj, so, si))
	}
}
