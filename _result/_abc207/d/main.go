...
func main() {
	...
	ss := make([]point, n)
	ts := make([]point, n)
	var sxsum, sysum, txsum, tysum int
	for i := 0; i < n; i++ {
		x, y := ni2()
		ss[i] = point{x: x * n, y: y * n}
		sxsum += x
		sysum += y
	}
	for i := 0; i < n; i++ {
		x, y := ni2()
		ts[i] = point{x: x * n, y: y * n}
		txsum += x
		tysum += y
	}
	sorigin := -1
	...
	for i := 0; i < n; i++ {
		...
		ts[i].x -= txsum
		ts[i].y -= tysum
		if ss[i].x == 0 && ss[i].y == 0 {
			sorigin = i
		} else {
			ss[i].size = ss[i].getSize()
			ss[i].rad = ss[i].getRad()
		}
		if ts[i].x == 0 && ts[i].y == 0 {
			torigin = i
		} else {
			ts[i].size = ts[i].getSize()
			ts[i].rad = ts[i].getRad()
		}
	}
	if sorigin != -1 {
		ss = append(ss[:sorigin], ss[sorigin+1:]...)
	}
	if torigin != -1 {
		ts = append(ts[:torigin], ts[torigin+1:]...)
		n--
	}
	if len(ts) != len(ss) {
		out("No")
	...
	sort.Slice(ss, func(i, j int) bool {
		if ss[i].rad < ss[j].rad {
			return true
		} else if ss[i].rad == ss[j].rad {
	...
	sort.Slice(ts, func(i, j int) bool {
		if ts[i].rad < ts[j].rad {
			return true
		} else if ts[i].rad == ts[j].rad {
			if ts[i].size < ts[j].size {
				return true
			}
		}
		return false
	...
	for i := 0; i < n; i++ {
		nexti := (i + 1) % n
		ss[i].dotProduct = ss[i].x*ss[nexti].x + ss[i].y*ss[nexti].y
		ss[i].crossProduct = ss[i].x*ss[nexti].y - ss[i].y*ss[nexti].x
		ts[i].dotProduct = ts[i].x*ts[nexti].x + ts[i].y*ts[nexti].y
		ts[i].crossProduct = ts[i].x*ts[nexti].y - ts[i].y*ts[nexti].x
	}
	...
	for i := 0; i < n; i++ {
		...
		for j := 0; j < n; j++ {
			idxf := (i + j) % n
			idxt := j
			if ss[idxf].size != ts[idxt].size {
				isOk = false
				break
			}
			if ss[idxf].dotProduct != ts[idxt].dotProduct {
				isOk = false
				break
			}
			if ss[idxf].crossProduct != ts[idxt].crossProduct {
				isOk = false
				break
			}
		}
