...
func main() {
	...
	q := ni()
	ts := make([][4]int, 0)
	mps := make([]map[int]int, h*w)
	mpsw := is(h*w, 0)
	rs := is(q, 0)
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			ts = append(ts, [4]int{i, j, fs[i][j], -1})
			mps[i*w+j] = make(map[int]int)
			mpsw[i*w+j] = i*w + j
		}
	}
	for qi := 0; qi < q; qi++ {
		...
		c--
		d--
		ts = append(ts, [4]int{a, b, y, qi})
		ts = append(ts, [4]int{c, d, z, qi})
	}
	sort2ar(ts, opt2ar(2, desc), opt2ar(3, asc))

	uf := newUnionFind(h * w)
	set := func(ti, k, h, ch int) {
		i := uf.root(ti)
		if v2, ok := mps[mpsw[i]][k]; ok {
			rs[k] = v2 + h - ch*2
			delete(mps[mpsw[i]], k)
		} else {
			mps[mpsw[i]][k] = h
	...
	merge := func(ti, tj, ch int) {
		i, j := uf.root(ti), uf.root(tj)
		if len(mps[mpsw[i]]) > len(mps[mpsw[j]]) {
			i, j = j, i
		}
		for k, h := range mps[mpsw[i]] {
			set(j, k, h, ch)
		}
		mps[mpsw[i]] = make(map[int]int)
		t := mpsw[j]
		uf.unite(ti, tj)
		mpsw[uf.root(tj)] = t
	}
	...
	for _, v := range ts {
		...
		*/
		...
		if v[3] == -1 {
			for k := 0; k < 4; k++ {
				np := point{v[0] + dx[k], v[1] + dy[k]}
				if !np.isValid(h, w) || fs[np.x][np.y] < v[2] {
					continue
				}
				if uf.issame(np.x*w+np.y, idx) {
					continue
				}
				merge(np.x*w+np.y, idx, v[2])
			}
		} else {
			set(idx, v[3], v[2], v[2])
		}
