...
func main() {
	...
	sets := [][2]int{}
	for i := 0; i < n; i++ {
		hf := int(math.Sqrt(float64(qs[i])))
		for j := 1; j <= hf; j++ {

			if qs[i]%j == 0 {
				sets = append(sets, [2]int{pi[j-1], i})
				sub := qs[i] / j
				if j != sub {
					sets = append(sets, [2]int{pi[sub-1], i})
				}
			}
		}
	}
	sort2ar(sets, opt2ar(0, asc), opt2ar(1, desc))
	r := make([]int, n)
	for i := 0; i < n; i++ {
		r[i] = inf
	}
	for _, v := range sets {
		idx := bs(0, n-1, func(c int) bool {
			return v[1] > r[c]
		})
		r[idx+1] = v[1]
	}
