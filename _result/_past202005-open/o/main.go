...
func main() {
	...
	mpts := i2s(3, n, 0)
	for i := 0; i < n; i++ {
		pts[0][i] = as[i] * bs[i] % rs[0]
		pts[1][i] = as[i] * bs[i] * bs[i] % rs[1]
		pts[2][i] = as[i] * bs[i] * bs[i] * bs[i] % rs[2]
		mpts[0][i] = -as[i] * bs[i]
		mpts[1][i] = -as[i]*bs[i]*bs[i] - mpts[0][i]*2
		mpts[2][i] = -as[i]*bs[i]*bs[i]*bs[i] - mpts[0][i]*3 - mpts[1][i]*2
	}
	dbg(pts)
	dbg(m, mpts)
	mcg := NewMcfGraph(n*3 + 5)
	mcg.AddEdge(0, 1, m, 0)
	mcg.AddEdge(0, 2, m, 0)
	mcg.AddEdge(0, 3, m, 0)
	gl := n*3 + 4
	for i := 0; i < n; i++ {
		...
		p3 := i*3 + 6
		for j := 0; j < 3; j++ {
			mcg.AddEdge(j+1, p1, 1, inf-pts[j][i])
		}
		mcg.AddEdge(p1, p2, 3, -mpts[0][i])
		mcg.AddEdge(p2, gl, 1, 0)
		mcg.AddEdge(p2, p3, 2, -mpts[1][i])
		mcg.AddEdge(p3, gl, 1, -mpts[2][i])
		mcg.AddEdge(p3, gl, 1, 0)
