...
func main() {
	...
	pq12 := newpq[pqst](func(p, q pqst) bool {
		return p.v > q.v
	})

	pq1 := newpq[pqst](func(p, q pqst) bool {
		return p.v > q.v
	})
	...
	for i := 0; i < n; i++ {
		...
		usedm[i] = make([]bool, k)
		if k >= 1 {
			pq1.Push(pqst{v: ts[i][0], x: i, y: 0})
			pq12.Push(pqst{v: ts[i][0], x: i, y: 0})
		}
		if k >= 2 {
			pq12.Push(pqst{v: ts[i][1], x: i, y: 1})
		}
	}
	m := ni()
	as := nis(m)
	...
	for _, v := range as {
		...
		if usedc[cp.x] < len(ts[cp.x]) {
			i := cp.x
			tk := usedc[cp.x]
			pq1.Push(pqst{v: ts[i][tk], x: i, y: tk})
			pq12.Push(pqst{v: ts[i][tk], x: i, y: tk})
		}
		if usedc[cp.x]+1 < len(ts[cp.x]) {
			i := cp.x
			tk := usedc[cp.x] + 1
			pq12.Push(pqst{v: ts[i][tk], x: i, y: tk})
		}
