...
func main() {
	...
	for i := 0; i < m; i++ {
		...
		lst.set(bs[i], segstruct{0, 1})

		tv := t.v
		lv := tv % n
		lst.applyrange(0, n, segfstruct(tv/n))
		if lv > 0 {
			if st+lv <= n {
				lst.applyrange(st, st+lv, segfstruct(1))

			} else {
				lst.applyrange(st, n, segfstruct(1))
				lst.applyrange(0, st+lv-n, segfstruct(1))
			}
		}
