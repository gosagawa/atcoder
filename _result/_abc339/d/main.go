...
func main() {
	...
	n := ni()
	st := [60][60][60][60]int{}
	mp := convidxi2s(nsi2s(n), map[string]int{".": 0, "#": 1, "P": 2})
	ckP := false
	f := [4]int{0, 0, 0, 0}
	for i := 0; i < n; i++ {
		for j := 0; j < n; j++ {
	...
	for e != nil {
		...
		p2 := point{p[2], p[3]}
		for i := 0; i < 4; i++ {
			np1 := pointAdd(p1, point{dx[i], dy[i]})
			np2 := pointAdd(p2, point{dx[i], dy[i]})
			if !np1.isValid(n, n) || mp[np1.x][np1.y] == 1 {
				np1 = p1
			}
			if !np2.isValid(n, n) || mp[np2.x][np2.y] == 1 {
				np2 = p2
			}
			if np1 == np2 {
				out(st[p[0]][p[1]][p[2]][p[3]])
				return
			}

			np := [4]int{np1.x, np1.y, np2.x, np2.y}
			if st[np[0]][np[1]][np[2]][np[3]] == 0 {
				st[np[0]][np[1]][np[2]][np[3]] = st[p[0]][p[1]][p[2]][p[3]] + 1
				q.PushBack(np)
			}
		}
		e = e.Next()
