...
func main() {
	...
	mst := make([][]int, 2*size)
	for i := 0; i < size; i++ {
		mst[size-1+i] = []int{as[i]}
	}
	for i := size - 2; i >= 0; i-- {
		mst[i] = mergeSort(mst[2*i+1], mst[2*i+2])
	}
	dbg(mst)

	sum := make([]*cusum, len(mst))
	for i := range mst {
		sum[i] = newcusum(mst[i])
	}

	q := ni()

	var soll, solr, solx int
	var solve func(l, r, id int) int
	solve = func(l, r, id int) int {
		if l >= r || solr < l || (r-1) < soll {
			return 0
		}

		if soll <= l && (r-1) <= solr {
			idx := bs(0, len(mst[id])-1, func(c int) bool {
				return mst[id][c] <= solx
			})
			return sum[id].get(0, idx+1)
		}

		mid := (l + r) / 2
		leftSum := solve(l, mid, 2*id+1)
		rightSum := solve(mid, r, 2*id+2)
...
func mergeSort(left, right []int) []int {
	...
	for lp < len(left) || rp < len(right) {
		if lp == len(left) {
			res = append(res, right[rp])
			rp++
		} else if rp == len(right) {
			res = append(res, left[lp])
			lp++
