...
func main() {
	...
	for i := a; i <= c; i++ {
		for j := b; j <= d; j++ {
			...
			//			out(i, j, dp[i-1][j], dp[i][j-1], dp[i-1][j-1], (i-1)*(j-1))
			dp[i][j] = madd(dp[i][j], mmul(dp[i-1][j], j))
			dp[i][j] = madd(dp[i][j], mmul(dp[i][j-1], i))
			dp[i][j] = madd(dp[i][j], -mmul(dp[i-1][j-1], (i-1)*(j-1)))
		}
	}
	o = dp[c][d]
