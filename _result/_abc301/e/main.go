...
func main() {
	...
	for i := 0; i < h; i++ {
		...
		for j := 0; j < w; j++ {
			...
			case "G":
				gp = point{i, j}
			case "o":
				op = append(op, point{i, j})
			case "#":
				mp[i][j] = 0
			}
		}
	}
	op = append([]point{sp}, op...)
	op = append(op, gp)
	lop := len(op)
	cs := i2s(lop, lop, 0)
	dr := []point{{1, 0}, {-1, 0}, {0, 1}, {0, -1}}
	for i := 0; i < lop; i++ {
		dist := i2s(h, w, inf)
		dist[op[i].x][op[i].y] = 0
		q := list.New()
		q.PushBack(op[i])
		e := q.Front()
		for e != nil {
			...
			for _, d := range dr {
				np := pointAdd(p, d)
				if !np.isValid(h, w) || mp[np.x][np.y] == 0 || dist[np.x][np.y] != inf {
					continue
				}
	...
	dp := i2s(pow2(lop)-2, lop-2, inf)
	for i := 0; i < lop-2; i++ {
		dp[pow2(i)][i] = cs[0][i+1]
	}
	for i := 0; i < pow2(lop-2); i++ {
		for j := 0; j < lop-2; j++ {
			if dp[i][j] == inf {
				continue
			}
			for k := 0; k < lop-2; k++ {
				...
				mins(&dp[i|pow2(k)][k], dp[i][j]+cs[j+1][k+1])
			}
		}
	}
	...
	for i := 0; i < pow2(lop-2); i++ {
		for j := 0; j < lop-2; j++ {
			if dp[i][j] != inf && cs[j+1][lop-1] != inf && dp[i][j]+cs[j+1][lop-1] <= t {
				maxs(&o, popcount(i))
			}
		}
	}
