...
func main() {
	...
	for i := 0; i < n-1; i++ {
		...
		s--
		t--
		edges[s] = append(edges[s], edge{to: t, color: cl, cost: cs})
		edges[t] = append(edges[t], edge{to: s, color: cl, cost: cs})
	}
	...
	for i := 0; i < q; i++ {
		...
		v--
		lca := tree.lca(u, v)
		uvs[u] = append(uvs[u], i)
		uvs[v] = append(uvs[v], i)
		lcas[lca] = append(lcas[lca], i)
		xys[i] = [2]int{x, y}
	}
	st := make([][2]int, n)
	var dfs func(v, p, tc int, st [][2]int)
	dfs = func(v, p, tc int, st [][2]int) {
		for _, vi := range uvs[v] {
			r[vi] += tc
			r[vi] -= st[xys[vi][0]][0]
			r[vi] += st[xys[vi][0]][1] * xys[vi][1]
			//			out("uvs", vi, r[vi], st[xys[vi][0]], xys[vi][1])
		}
		for _, vi := range lcas[v] {
			r[vi] -= tc * 2
			r[vi] += st[xys[vi][0]][0] * 2
			r[vi] -= st[xys[vi][0]][1] * xys[vi][1] * 2
			//			out("lca", vi, r[vi])
		}
