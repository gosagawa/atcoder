...
func main() {
	...
	h, w, k := ni3()
	k--
	mp := convidxi2s(nsi2s(h), map[string]int{".": 0, "o": 1, "x": 2})
	...
	cusum2d1 := newcusum2d(sl)
	cusum2d2 := newcusum2d(sl2)
	for i := 0; i < h; i++ {
		for j := 0; j < w-k; j++ {
			t1 := cusum2d1.get(i, j, i, j+k)
			t2 := cusum2d2.get(i, j, i, j+k)
			debug(i, j, t1, t2)
			if t2 > 0 {
				continue
			}
	...
	for i := 0; i < h-k; i++ {
		for j := 0; j < w; j++ {
			t1 := cusum2d1.get(i, j, i+k, j)
			t2 := cusum2d2.get(i, j, i+k, j)
			debug(i, j, t1, t2)
			if t2 > 0 {
				continue
			}
