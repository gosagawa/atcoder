...
func main() {
	...
	if mp[0][0] > 0 {
		base := mpow(3, h*w-k, mod)
		dp[0][0][mp[0][0]-1] = base
	} else {
		base := mpow(3, h*w-k-1, mod)
		dp[0][0] = [3]int{base, base, base}
	}
	inv := minvfermat(3, mod) % mod
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			if i > 0 {
				c := dp[i-1][j][1] + dp[i-1][j][2]
				if mp[i][j] > 0 {
					dp[i][j][mp[i][j]-1] += c
					dp[i][j][mp[i][j]-1] %= mod
				} else {
	...
	o = madd(o, dp[h-1][w-1][0])
	o = madd(o, dp[h-1][w-1][1])
	o = madd(o, dp[h-1][w-1][2])
	out(o)
}
