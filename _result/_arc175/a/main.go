...
func main() {
	...
	n := ni()
	ps := nis(n)
	ss := convidxis(nsis(), map[string]int{"R": 0, "L": 1, "?": 2})
	for i := 0; i < n; i++ {
		ps[i]--
	}
	for i := 0; i < 2; i++ {
		if ss[ps[0]] != 2 && ss[ps[0]] != i {
			continue
		}
		st := is(n, 2)
		st[ps[0]] = i
		...
		t := 1
		for j := 1; j < n; j++ {
			key, hasKey := s.UpperBound(ps[j])
			if !hasKey {
				key, hasKey = s.UpperBound(-1)
			}
			...
			if st[key] == 1 {
				...
				key, hasKey := s.LowerBound(ps[j])
				if !hasKey {
					key, hasKey = s.LowerBound(n)
				}
				if ps[j] == (key+1)%n {
					if ss[ps[j]] == 2 {
						t *= 2
