...
func main() {
	...
	dbg(pts)
	ad := func(i, sw int) int {
		if sw > 0 {
			return i
		}
		return n - 1 - i
	}
	sw := func(x, y int, pt1 point, pt2 point) (int, int) {
		if abs(pt1.x) == abs(pt2.x) {
			return ad(x, pt1.x*pt2.x), ad(y, pt1.y*pt2.y)
		} else {
			return ad(y, pt1.y*pt2.x), ad(x, pt1.x*pt2.y)
		}
	}
	mp := i2s(n, n, 0)
	for _, v := range ts {
		x, y, i := v[0], v[1], v[2]
		nx, ny := sw(x, y, pts[i], pts[q-1])
		mp[nx][ny] ^= 1
	}
	for _, v := range mp {
		outis(v)
