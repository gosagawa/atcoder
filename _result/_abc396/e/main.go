...
func main() {
	...
	xys := make([][][2]int, n)
	for i := 0; i < m; i++ {
		xys[xs[i]] = append(xys[xs[i]], [2]int{ys[i], zs[i]})
		xys[ys[i]] = append(xys[ys[i]], [2]int{xs[i], zs[i]})
	}
	rs := is(n, inf)
	for i := 0; i < n; i++ {
		if rs[i] != inf {
		...
		invalid := false
		group := []int{i}
		groupcnt := is(maxlogn, 0)
		dfs = func(v, p int) {
			for _, nv := range xys[v] {
				if nv[0] == p {
					continue
				}
				if rs[nv[0]] == inf {
					group = append(group, nv[0])
					rs[nv[0]] = rs[v] ^ nv[1]
					for i := 0; i < maxlogn; i++ {
						if hasbit(rs[nv[0]], i) {
							groupcnt[i]++
		...
		tx := 0
		for i := 0; i < maxlogn; i++ {
			if groupl-groupcnt[i] < groupcnt[i] {
				tx += pow(2, i)
			}
		}
		dbg(tx)
