...
func main() {
	...
	o := 0
	n := ni()
	ps := getPrimes(sqrt(int(1e11)))
	for i := 0; i < len(ps); i++ {
		if ps[i]*ps[i]*ps[i]*ps[i]*ps[i] > n {
			break
		}
		for j := i + 1; j < len(ps); j++ {
			if ps[i]*ps[i]*ps[j]*ps[j]*ps[j] > n {
				break
			}
			t := bs(0, len(ps)-1, func(c int) bool {
				if ps[i]*ps[i]*ps[j] > n {
					return false
				}
				if ps[i]*ps[i]*ps[j]*ps[c] > n {
					return false
				}
				return ps[i]*ps[i]*ps[j]*ps[c]*ps[c] <= n
			})
			if t > j {
				o += t - j
			}
