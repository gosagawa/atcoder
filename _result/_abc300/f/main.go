...
func main() {
	...
	o := 0
	n, m, k := ni3()
	s := convidxis(nsis(), map[string]int{"o": 1, "x": 0})
	debug(n, m, k, s)
	ts1 := []int{0}
	ts2 := []int{0}
	st := 0
	...
	for i := 0; i < n; i++ {
		if st == 0 {
			...
			if s[i] == 1 {
				ts1 = append(ts1, co+ts1[len(ts1)-1])
				ts2 = append(ts2, cx+ts2[len(ts2)-1])
				co = 1
				cx = 0
				st = 0
			} else {
	...
	for i := 0; i < len(ts1); i++ {
		...
		ri := upperBound(tk2, ts2)
		to3 := to2 + ts1[ri] + tk2

		to4 := to3 - ts1[i] - ts2[i]

		debug(i, to, to2, to3, to4, tk, tk2)
		maxs(&o, to4)
