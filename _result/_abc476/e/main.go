...
func main() {
	...
	n, m := ni2()
	as := nis(n)
	base1 := make([]segstruct, n)
	base2 := make([]segstruct, n)
	ais := is(n, 0)
	for i := 0; i < n; i++ {
		as[i] -= 1
		base1[i] = segstruct(as[i])
		base2[i] = segstruct(as[i])
		ais[as[i]] = i
	}

	op1 := func(a segstruct, b segstruct) segstruct {
	...
	mapping1 := func(f segfstruct, x segstruct) segstruct {
		if f == id1() {
			return x
		}
		return segstruct(int(f))
	}
	compostion1 := func(f segfstruct, g segfstruct) segfstruct {
		if f == id1() {
			return g
		}
		return f
	...
	op2 := func(a segstruct, b segstruct) segstruct {
		return segstruct(max(int(a), int(b)))
	}
	e2 := func() segstruct {
		return segstruct(-inf)
	}
	...
	mapping2 := func(f segfstruct, x segstruct) segstruct {
		if f == id2() {
			return x
		}
		return segstruct(int(f))
	}
	compostion2 := func(f segfstruct, g segfstruct) segfstruct {
		if f == id2() {
			return g
		}
		return f
	...
	for i := 0; i < m; i++ {
		l, r := ni2()
		l--
		miv := int(lstmi.prod(l, r))
		mxv := int(lstmx.prod(l, r))
		dbg(miv, mxv)
		mivi := ais[miv]
		mxvi := ais[mxv]
		lstmi.set(mxvi, segstruct(miv))
		lstmi.set(mivi, segstruct(mxv))
		lstmx.set(mxvi, segstruct(miv))
		lstmx.set(mivi, segstruct(mxv))
		ais[miv] = mxvi
		ais[mxv] = mivi
	}
	rs := is(n, 0)
	for i := 0; i < n; i++ {
		rs[i] = int(lstmi.get(i)) + 1
	}
	outis(rs)
}
