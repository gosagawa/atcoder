...
func main() {
	...
	for i := 0; i < n; i++ {
		for j := i + 1; j < n; j++ {
			ts = append(ts, [3]int{i, j, ni()})
		}
	}
	edges := make([][]edge, n)
	sort2ar(ts, opt2ar(2, asc))
	uf := newUnionFind(n)
	for _, v := range ts {
		s, t, c := v[0], v[1], v[2]
		if uf.issame(s, t) {
...
func (t *tree) dfs(v, p, dep, depthdist int) {
	t.order[v] = t.orderidx
	t.orderidx++
	t.parent[0][v] = p
	t.depth[v] = dep
	t.depthdist[v] = depthdist
	for _, nv := range t.edges[v] {
		dbg(nv)
		if nv.to != p {
			t.dfs(nv.to, v, dep+1, depthdist+nv.cost)
		}
	}
}
...
func (t *tree) dist(u, v int) int {
	return t.depthdist[u] + t.depthdist[v] - t.depthdist[t.lca(u, v)]*2
}
