...
func main() {
	...
	e := func() segstruct {
		return segstruct(-inf * 2)
	}
	id := func() segfstruct {
		return segfstruct(-inf * 2)
	}
	mapping := func(f segfstruct, x segstruct) segstruct {
		if f == id() {
			return x
		}
		return segstruct(int(x) + int(f))
	}
	...
	sort2ar(ts, opt2ar(1, desc))
	dbg(ts)
	base := make([]segstruct, n+q)
	rst := make([]int, n+q)
	t := 0
	for i := 0; i < n+q; i++ {
		if ts[i][3] < n {
			t += ts[i][0]
			base[i] = segstruct(t + ts[i][1])
		} else {
			base[i] = segstruct(t - inf + ts[i][1])
		}
		rst[ts[i][3]] = i
	}
	dbg(base)
	...
	for i := 0; i < q; i++ {
		nt := ts[rst[n+i]]
		ct := ts[rst[st[nt[2]]]]
		ntv := lst.get(rst[nt[3]])
		ctv := lst.get(rst[ct[3]])
		lst.set(rst[nt[3]], segstruct(int(ntv)+inf))
		lst.set(rst[ct[3]], segstruct(int(ctv)-inf))

		st[nt[2]] = nt[3]

		if debug {
			dt := make([]segstruct, n+q)
			for i := 0; i < n+q; i++ {
				dt[i] = lst.get(i)
			}
			dbg(dt)
		...
		//		dbg(nt, ct, nt[0])
		lst.applyrange(rst[nt[3]], n+q, segfstruct(nt[0]))
		lst.applyrange(rst[ct[3]], n+q, segfstruct(-ct[0]))
...
func sort2ar(sl [][4]int, setters ...Sort2ArOption) {
	args := &Sort2ArOptions{}
