...
func main() {
	...
	base := make([]pqst, n*(n-1)/2)
	k := 0
	for i := 0; i < n; i++ {
		for j := i + 1; j < n; j++ {
			mt := abs(xs[i]-xs[j]) + abs(ys[i]-ys[j])
			base[k] = pqst{v: mt, x: i, y: j}
			k++
		}
	}
	dbg(base)
	...
	for qi := 0; qi < q; qi++ {
		...
		case 1:
			...
			ys = append(ys, b)
			for i := 0; i < n-1; i++ {
				mt := abs(xs[i]-xs[n-1]) + abs(ys[i]-ys[n-1])
				pq.Push(pqst{v: mt, x: i, y: n - 1})
			}
		case 2:
			k := inf
			for !pq.IsEmpty() {
