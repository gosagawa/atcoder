...
func main() {
	...
	for i := -7; i <= 7; i++ {
		for j := -7; j <= 7; j++ {
			for k := -7; k <= 7; k++ {
				sl = append(sl, [3]int{i, j, k})
			}
		}
	}
	s0 := [3]int{0, 0, 0}
	cv2 := func(a, b [3]int) int {
		t := 1
		for i := 0; i < 3; i++ {
			l := max(a[i], b[i])
			r := min(a[i]+7, b[i]+7)
	...
	cv3 := func(a, b, c [3]int) int {
		t := 1
		for i := 0; i < 3; i++ {
			l := max(max(a[i], b[i]), c[i])
			r := min(min(a[i]+7, b[i]+7), c[i]+7)
			t *= max(r-l, 0)
		}
		return t
	}
	...
	for _, s1 := range sl {
		for _, s2 := range sl {
			tv3 := cv3(s0, s1, s2)
			tv2 := cv2(s0, s1) + cv2(s1, s2) + cv2(s2, s0) - tv3*3
			tv1 := 3*7*7*7 - tv3*3 - tv2*2
			if v1 == tv1 && v2 == tv2 && v3 == tv3 {
				out("Yes")
				outf("%d %d %d %d %d %d %d %d %d", 0, 0, 0, s1[0], s1[1], s1[2], s2[0], s2[1], s2[2])
				return
			}
		}
	}
