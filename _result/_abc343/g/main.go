...
func main() {
	...
	for i := 0; i < n; i++ {
		for j := i + 1; j < n; j++ {
			if strings.Contains(ss[i], ss[j]) {
				contain[j] = true
			}
			if strings.Contains(ss[j], ss[i]) {
				contain[i] = true
			}
		}
	}
	...
	for i := 0; i < n; i++ {
		...
		rhsl.setSearchBase(len(bs), len(bs))
		for j := len(bs) - 1; j >= 0; j-- {
			rhsl.addLeft()
			tls[j] = rhsl.chash
		}
		ls = append(ls, len(trs))
		ssr = append(ssr, trs)
		ssl = append(ssl, tls)
	}
	n = len(ssr)
	...
	for i := 0; i < n; i++ {
		for j := 0; j < n; j++ {
			...
			for k := 0; k < min(ls[i], ls[j]); k++ {
				if ssr[i][k] == ssl[j][ls[j]-1-k] {
					cv[i][j] = k + 1
				}
			}
		}
	...
	for i := 1; i < 1<<n; i++ {
		...
		for j := 0; j < n; j++ {
			...
			for k := 0; k < n; k++ {
				...
				mins(&dp[i][j], dp[i^(1<<j)][k]+ls[j]-cv[k][j])
			}
		}
