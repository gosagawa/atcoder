...
func main() {
	...
	for i := -1; i <= 1; i += 2 {
		for j := -1; j <= 1; j += 2 {
			np := pointAdd(sp, point{i, 0})
			if np.isValid(h, w) && m[np.x][np.y] {
			} else {
				continue
			}
			np = pointAdd(sp, point{0, j})
			if np.isValid(h, w) && m[np.x][np.y] {
			} else {
				continue
			}
			np = pointAdd(sp, point{i, j})
			if np.isValid(h, w) && m[np.x][np.y] {
			} else {
				continue
			}
	...
	for i := 0; i < 4; i++ {
		for j := 0; j < 4; j++ {
			...
			for l := 0; l < 4; l++ {
				for k := 0; k < 4; k++ {
					np2 := pointAdd(sp, dr[l])
					if !np2.isValid(h, w) || !m[np2.x][np2.y] || tm[np2] {
						continue
					}
					np2 = pointAdd(np2, dr[k])
					if !np2.isValid(h, w) || !m[np2.x][np2.y] || tm[np2] {
						continue
					}
					tm2 = append(tm2, [2]point{np, np2})
				}
			}
		}
	}
	np := pointAdd(sp, point{1, 0})
	if np.isValid(h, w) {
		m[np.x][np.y] = false
	}
	np = pointAdd(sp, point{-1, 0})
