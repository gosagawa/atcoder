...
func main() {
	...
	n, q := ni2()
	dp := i2s(q, n, inf)
	movable := func(f, t, d, other int) (bool, int) {
		if d == 1 {
			if t < f {
				if other < t {
					other += n
	...
	var mv func(fl, fr, tl, tr int) int
	mv = func(fl, fr, tl, tr int) int {
		o := inf
		f := []int{fl, fr}
		t := []int{tl, tr}
		for i := 0; i < 2; i++ {
			for k := 0; k < 2; k++ {
				...
				tv := 0
				r1, r2 := movable(f[i], t[i], k, f[i^1])
				if !r1 {
					continue
				}
				tv += r2
				r1, r2 = movable(f[i^1], t[i^1], k, t[i])
				if !r1 {
					continue
				}
				tv += r2
	...
	for qi := 0; qi < q; qi++ {
		...
		if qi == 0 {
			...
			for _, tg := range tgs {
				...
				if h == 0 {
					dp[qi][tg] = mv(0, 1, t, tg)
				} else {
					dp[qi][tg] = mv(0, 1, tg, t)
				}
			}
			...
			for i := 0; i < n; i++ {
				...
				for _, tg := range tgs {
					...
					d := mv(l, r, ml, mr)
					dbg(l, r, ml, mr, d, dp[qi-1][i])
					if d != inf {
						mins(&dp[qi][tg], dp[qi-1][i]+mv(l, r, ml, mr))
					}
				}
			}
		}
