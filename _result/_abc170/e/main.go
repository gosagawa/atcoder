...
func main() {
	...
	bas := make([][]int, 200001)
	for i := 0; i < n; i++ {
		bas[bl[i]] = append(bas[bl[i]], al[i])
	}
	s := newstree(200001, stmin, stset)
	for i := 1; i < 200001; i++ {
		if len(bas[i]) == 0 {
			continue
		}
	...
	for i := 0; i < q; i++ {
		...
		c--
		pd := bl[c]
		idx := bs(0, len(bas[pd])-1, func(bsc int) bool {
			return bas[pd][bsc] >= al[c]
		})
		bas[pd] = delIdx(idx, bas[pd])
		if len(bas[pd]) == 0 {
			s.set(pd, inf)
		} else {
			s.set(pd, bas[pd][0])
		...
		if len(bas[d]) == 0 {
			bas[d] = append(bas[d], al[c])
		} else {
			idx = bs(0, len(bas[d])-1, func(bsc int) bool {
				return bas[d][bsc] >= al[c]
			}) + 1
			bas[d] = addIdx(idx, al[c], bas[d])
		}
		s.set(d, bas[d][0])
		bl[c] = d

		out(s.query(0, 200001))
	}
