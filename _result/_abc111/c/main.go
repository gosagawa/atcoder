...
func main() {
	...
	ns := nis(n)
	st := make([][][2]int, 2)
	st[0] = make([][2]int, int(1e5)+1)
	st[1] = make([][2]int, int(1e5)+1)
	for i := 0; i < int(1e5)+1; i++ {
		st[0][i] = [2]int{i, 0}
		st[1][i] = [2]int{i, 0}
	}
	...
	sort2ar(st[0], opt2ar(1, desc))
	sort2ar(st[1], opt2ar(1, desc))
	if st[0][0][0] != st[1][0][0] {
		o = n - st[0][0][1] - st[1][0][1]
	} else {
		o = n - st[0][0][1] - max(st[0][1][1], st[1][1][1])
	}
	out(o)
}
