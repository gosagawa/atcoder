...
func main() {
	...
	n := ni()
	xys := make([]point, n)
	sl := make([][]int, int(1e5)*2+10)
	for i := 0; i < n; i++ {
		xys[i].x, xys[i].y = ni2()
		sl[xys[i].x] = append(sl[xys[i].x], xys[i].y)
	}
	dx := []int{1, 1, 0, -1, -1, -1, 0, 1}
	dy := []int{0, 1, 1, 1, 0, -1, -1, -1}
	mp := make(map[point]int)
	mp2 := make(map[point]bool)
	idx := 0
	set := func(p point) {
		if _, ok := mp[p]; ok {
			return
	...
	for i := 0; i < n; i++ {
		...
		mp2[xys[i]] = true
		for j := 0; j < 8; j++ {
			p := point{xys[i].x + dx[j], xys[i].y + dy[j]}
			set(p)
		}
	}
	dbg(mp)
	dbg(mp2)
	uf := newUnionFind(idx + 1)
	for i := 0; i < n; i++ {
		for j := 0; j < 8; j++ {
			p1 := point{xys[i].x + dx[j], xys[i].y + dy[j]}
			p2 := point{xys[i].x + dx[(j+1)%8], xys[i].y + dy[(j+1)%8]}
			if !mp2[p1] && !mp2[p2] {
				uf.unite(mp[p1], mp[p2])
			}
		}
	}
	for i := 0; i < int(1e5)*2+10; i++ {
		...
		for j := 0; j < len(sl[i]); j++ {
			dpt := point{i, sl[i][j] - 1}
			upt := point{i, sl[i][j] + 1}
			if isOut && !mp2[dpt] {
				pt3 = dpt
				ti = j
			}
			...
			if uf.issame(mp[pt3], mp[upt]) {
				//	out(i, upt.y, pt3.y, ti, j, o)
				o += upt.y - pt3.y - 1 - (j - ti + 1)
				//	out(i, upt.y, pt3.y, ti, j, o)
				isOut = true
			} else {
				isOut = false
