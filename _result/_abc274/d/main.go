...
func main() {
	...
	n, x, y := ni3()
	ns := nis(n)
	dpx := make([]map[int]bool, n)
	dpy := make([]map[int]bool, n)
	dpx[0] = make(map[int]bool)
	dpx[0][0] = true
	dpy[0] = make(map[int]bool)
	dpy[0][0] = true
	for i := 0; i < n; i++ {
		dpx[i] = make(map[int]bool)
		dpy[i] = make(map[int]bool)
		if i == 0 {
			dpx[i][ns[i]] = true
		} else if i == 1 {
			dpy[i][ns[i]] = true
	...
	//	out(dpx[xmax], dpy[ymax])

	if dpx[xmax][x] && dpy[ymax][y] {
