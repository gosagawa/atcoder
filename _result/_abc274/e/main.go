...
func main() {
	...
	gd := func(i, j int, sp float64) float64 {
		x := float64(xs[i] - xs[j])
		y := float64(ys[i] - ys[j])
		return math.Sqrt(x*x+y*y) / sp
	}
	gdo := func(i int, sp float64) float64 {
		x := float64(xs[i])
		y := float64(ys[i])
		return math.Sqrt(x*x+y*y) / sp
	}
	dp := make([][17]float64, pow2(n+m))
	for i := 0; i < pow2(n+m); i++ {
		for j := 0; j < n+m; j++ {
			dp[i][j] = float64(inf)
		}
	}
	...
	for i := 0; i < pow2(n+m); i++ {
		...
		for j := 0; j < n+m; j++ {
			...
			for k := 0; k < n+m; k++ {
				...
				if dp[i^pow2(k)][j] == float64(inf) {
					continue
				}
				if k >= n {
					sp /= 2
				}
				//				out(i, j, k, dp[i^pow2(k)][j], gd(j, k, sp))
				minsf(&dp[i][k], dp[i^pow2(k)][j]+gd(j, k, sp))
				if k >= n {
					sp *= 2
				}
			}
