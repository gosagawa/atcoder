...
func main() {
	...
	for i, v := range uvs {
		...
		s, t := i[0], i[1]
		edges[s] = append(edges[s], edge{to: t, cost: v})
		edges[t] = append(edges[t], edge{to: s, cost: v})
	}
	type status struct {
		st int
		v  int
	...
	for i := 0; i < n; i++ {
		...
		for _, nv := range edges[i] {
			if nv.to < i {
				dp[pow2(nv.to)][nv.to] = nv.cost
			}
		}
		to := 0
		for j := 0; j < pow2(i); j++ {
			for k := 0; k < i; k++ {
				for _, nv := range edges[k] {
					if nv.to > i || hasbit(j, nv.to) {
						continue
					}
					if nv.to == i {
						if pcs[j] >= 2 {
							to += mmul(mmul(dp[j][k], nv.cost), inv2)
							to %= mod
						}
						continue
					}
					nst := j + pow2(nv.to)
					dp[nst][nv.to] = madd(dp[nst][nv.to], mmul(dp[j][k], nv.cost))
				}
			}
		}
		o += to
