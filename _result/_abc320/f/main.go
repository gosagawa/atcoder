	...
	for i := 0; i < n; i++ {
		...
		for j := 0; j < h+1; j++ {
			for k := 0; k < h+1; k++ {
				if dp[i][j][k] != inf {
					if j-d >= 0 && k+d <= h {
						mins(&dp[i+1][j-d][k+d], dp[i][j][k])
						mins(&dp[i+1][min(j-d+fs[i], h)][k+d], dp[i][j][k]+ps[i])
					}
				}
				if j+d <= h && min(k+fs[i], h)-d >= 0 && dp[i][j+d][min(k+fs[i], h)-d] != inf {
					mins(&dp[i+1][j][k], dp[i][j+d][min(k+fs[i], h)-d]+ps[i])
				}
