...
func main() {
	...
	s := ni()
	t := bs(1, int(1e9), func(c int) bool {
		return c*c <= s
	})
	mx := int(1e9)
	output := func(x1, y1, x2, y2, x3, y3 int) {
		if x1 > mx {
			panic("invalid")
		}
		if y1 > mx {
		...
		if abs(x2*y3-x3*y2) != s {
			panic("invalid v:" + itoa(abs(x2*y3-x3*y2)))
		}
		outf("%v %v %v %v %v %v", x1, y1, x2, y2, x3, y3)
	}
	if t*t == s {
		output(0, 0, t, 0, 0, t)
		return
	...
	if t*(t+1) >= s {
		d := t*(t+1) - s
		output(0, 0, t, 1, d, t+1)
		return
	}
	d := (t+1)*(t+1) - s
	output(0, 0, t+1, 1, d, t+1)
}
