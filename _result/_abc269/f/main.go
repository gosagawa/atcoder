...
func main() {
	...
	sum1 := func(x, y int) int {
		sum := 0
		sum += mmul(mdiv(mmul(x, (x+1)), 2), y) % mod
		sum += mmul(mdiv(mmul(mmul((y-1), m), y), 2), x) % mod
		return sum
	}
	sumr1 := func(x1, y1, x2, y2 int) int {
		sum := sum1(x2, y2)
		if x1 > 1 {
			sum -= sum1(x1-1, y2)
			sum += mod
	...
	sum2 := func(x, y int) int {
		x /= 2
		y = (y + 1) / 2
		sum := 0
		sum += mmul(mdiv(mmul(x, (x+1)), 2), y) * 2
		sum %= mod
		sum += mmul(mdiv(mmul(mmul(y-1, m*2), y), 2), x)
		sum %= mod
		return sum
	}
	sumr2 := func(x1, y1, x2, y2 int) int {
		sum := sum2(x2, y2)
		if x1 > 1 {
			sum -= sum2(x1-1, y2)
			sum += mod
	...
	sum3 := func(x, y int) int {
		x = (x + 1) / 2
		y /= 2
		sum := 0
		sum += mmul(mdiv(mmul(1+1+(x-1)*2, x), 2), y)
		sum %= mod
		sum += mmul(mdiv(mmul(m+m+mmul(y-1, m*2), y), 2), x)
		sum %= mod
		return sum
	}
	sumr3 := func(x1, y1, x2, y2 int) int {
		sum := sum3(x2, y2)
		if x1 > 1 {
			sum -= sum3(x1-1, y2)
			sum += mod
