...
func main() {
	...
	for i, v := range ts {
		base := cts.get(i)
		ts2[i] = (bs[base-2] - base + v) % v
	}
	tm := 0
	_, n, tm = chineseRem(ts2[0], ts[0], ts2[1], ts[1])
	for i := 2; i < len(ts); i++ {
		_, n, tm = chineseRem(n, tm, ts2[i], ts[i])
	}
	debug(ts2)
	out(n + 1)
	flush()
