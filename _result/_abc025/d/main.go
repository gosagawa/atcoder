...
func main() {
	...
	settable := func(x, y int) bool {
		...
		for i := 0; i < 4; i++ {
			...
			tp.x += dx[i]
			tp.y += dy[i]
			if !tp.isValid(5, 5) || !tmp[tp.x][tp.y] {
				continue
			}
			otp := p
			otp.x -= dx[i]
			otp.y -= dy[i]
			if otp.isValid(5, 5) && !tmp[otp.x][otp.y] {
				return false
			}
		}
		return true
	}
	subsettable := func(ci int) bool {
		r := true
		for i := ci + 1; i <= 25; i++ {
			if nmp[i][0] == -1 {
				break
			}
			if !settable(nmp[i][0], nmp[i][1]) {
				r = false
			}
			tmp[nmp[i][0]][nmp[i][1]] = true
		}
	...
	for i := 1; i <= 25; i++ {
		...
		for _, v := range sl[di] {
			...
			for j := 0; j < sc; j++ {
				...
				if !settable(smp[j][0], smp[j][1]) {
					continue
				}
				tmp[smp[j][0]][smp[j][1]] = true
				if !subsettable(i) {
					tmp[smp[j][0]][smp[j][1]] = false
					continue
				}
				dp[v|pow2(j)] += dp[v]
				dp[v|pow2(j)] %= mod
