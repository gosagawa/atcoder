...
func main() {
	...
	var mergeSort func(slice []int) []int
	mergeSort = func(slice []int) []int {
		if len(slice) <= 1 {
			return slice
		}
	...
	for t := 0; t < mx; t++ {
		...
		add(ts[0], ts[len(ts)-1])
		ts = ts[1 : len(ts)-1]
		if len(ts) == 0 {
			ts = append(ts, ci)
		} else {
			ti := bs(0, len(ts)-1, func(c int) bool {
				return ask(ts[c], ci)
			})
			ti++
			ts = addIdx(ti, ci, ts)
