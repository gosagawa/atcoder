...
func main() {
	...
	rhsl1 := newrhsl(ts1, mod)
	rhsl2 := newrhsl(ts2, mod)
	rhsl1.setSearchBase(1, n)
	rhsl2.setSearchBase(0, 0)
	t = [2]int{}
	...
	for i := 0; i < n; i++ {
		...
		v := mulrh(rhsl2.chash, mpowrh(mod, n-i, mod))
		v = addrh(v, mulrh(t3, mpowrh(mod, n-i-1, mod)))
		v = addrh(v, rhsl1.chash)
		debug(rhsl2.chash, t3, rhsl1.chash, v, mpowrh(mod, n-i-1, mod))
		if i != n-1 {
			t[s[i+1]]++
			rhsl1.removeLeft()
			rhsl2.addRight()
