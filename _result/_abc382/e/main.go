...
func main() {
	...
	for i := 0; i < n+1; i++ {
		dp1[i] = make([]float64, n+1)
	}
	dp1[0][0] = 1.0
	for i := 0; i < n; i++ {
		for j := 0; j < n; j++ {
			dp1[i+1][j] += dp1[i][j] * (1.0 - float64(ps[i])/100.0)
			dp1[i+1][j+1] += dp1[i][j] * float64(ps[i]) / 100.0
		}
	}
	dbg(dp1[n])
	dp2 := make([]float64, x+n+1)
	for i := x - 1; i >= 0; i-- {
		dp2[i] += 1.0 / (1.0 - dp1[n][0])
		for j := 1; j <= n; j++ {
			dp2[i] += dp1[n][j] / (1 - dp1[n][0]) * dp2[i+j]
		}
	}
