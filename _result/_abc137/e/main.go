...
func main() {
	...
	for i := 0; i < e; i++ {
		...
		edges2[s] = append(edges2[s], edge{to: t, cost: c})
	}
	graph := newgraph(v, edges)
	dist := graph.dijkstra(v - 1)
	ur := make([]bool, v)
	for i, v := range dist {
		if v == inf {
			ur[i] = true
	...
	for i := 0; i < e; i++ {
		...
		edges[s] = append(edges[s], edge{to: t, cost: -c + p})
	}

	graph = newgraph(v, edges)
	dist, ng := graph.bellmanFord(0)
	if ng {
		out(-1)
		return
	}
