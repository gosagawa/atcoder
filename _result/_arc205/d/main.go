	...
	for qi := 0; qi < q; qi++ {
		...
		dfs = func(v, p int) {
			...
			for _, nv := range edges[v] {
				...
				if tst[0] < st[nv.to][0] {
					tst = st[nv.to]
				} else if tst[0] == st[nv.to][0] && tst[1] > st[nv.to][1] {
					tst = st[nv.to]
				}
			}
			if alst == tst {
			} else {
				dst := [2]int{alst[0] - tst[0], alst[1] - tst[1]}
				if tst[0] > dst[0]+dst[1] {
					tst[0] -= dst[0] + dst[1]
					tst[1] = alst[0] + alst[1] - tst[0]
				} else {
					tst[0] = alst[0] % 2
					tst[1] = alst[0] + alst[1] - tst[0]
				}
