...
func main() {
	...
	for i := 0; i < n; i++ {
		if ss[i] < ts[i] {
			ts1 = append(ts1, [3]int{ss[i], ts[i], i})
		} else {
			ts2 = append(ts2, [3]int{ss[i], ts[i], i})
		}
	}
	sort2ar(ts1, opt2ar(0, asc))
	sort2ar(ts2, opt2ar(0, asc))
	dbg(ts1)
	dbg(ts2)
	for i := 0; i < len(ts1)-1; i++ {
		if ts1[i+1][1] < ts1[i][1] {
			out("No")
			return
		}
	}
	for i := len(ts2) - 1; i > 0; i-- {
		if ts2[i-1][1] > ts2[i][1] {
			out("No")
			return
		}
	}
	...
	out("Yes")
	rs := []int{}
	for i := len(ts1) - 1; i >= 0; i-- {
		rs = append(rs, ts1[i][2]+1)
	}
	for i := 0; i < len(ts2); i++ {
		rs = append(rs, ts2[i][2]+1)
	}
	outis(rs)
}
