...
func main() {
	...
	for i := 0; i < h; i++ {
		...
		if t > 0 {
			sum += t
			setDualEdge(edges, 0, i+1, 0)
			setDualEdge(edges, i+1, h+w+1, t)
		}
	}
	for j := 0; j < w; j++ {
		t := 0
		...
		if t > 0 {
			sum += t
			setDualEdge(edges, 0, h+j+1, t)
			setDualEdge(edges, h+j+1, h+w+1, 0)
		}
	}
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			if as[i][j] < 0 {
				setDualEdge(edges, h+j+1, i+1, inf)
			} else {
				setDualEdge(edges, h+j+1, i+1, as[i][j])
			}
		}
	}
