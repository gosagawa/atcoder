...
func main() {
	...
	a, b, d := ni3()
	r := math.Atan2(float64(b), float64(a)) * 180 / math.Pi
	t := r + float64(d)

	na := math.Sin(t/180*math.Pi) * math.Sqrt(float64(a)*float64(a)+float64(b)*float64(b))
	nb := math.Cos(t/180*math.Pi) * math.Sqrt(float64(a)*float64(a)+float64(b)*float64(b))
