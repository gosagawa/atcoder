...
func main() {
	...
	for i := 0; i < n-1; i++ {
		...
		edges[s] = append(edges[s], edge{to: t, cost: w})
		edges[t] = append(edges[t], edge{to: s, cost: w})
	}
	mp := make(map[[3]int]int)
	var dfs func(v, p, d int) int
	dfs = func(v, p, d int) int {
		if d == 1 && ds[v] == 0 {
			return -10000000000000000
		}
		if v2, ok := mp[[3]int{v, p, d}]; ok {
			return v2
		}
		r := [][2]int{}
		for _, nv := range edges[v] {
			if nv.to != p {
				r1 := dfs(nv.to, v, 0)
				r2 := dfs(nv.to, v, 1) + nv.cost
				if r1 >= r2 {
					r2 = r1
				}
				r = append(r, [2]int{r1, r2 - r1})
			}
		}

		sort2ar(r, opt2ar(1, desc))
		t := 0
		for i, v2 := range r {
			if i < ds[v]-d {
				t += v2[0] + v2[1]
