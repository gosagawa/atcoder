...
func main() {
	...
	txy := point{tx, ty}
	for i := 0; i < n; i++ {
		if pointDistDouble(sxy, point{xs[i], ys[i]}) == rs[i]*rs[i] {
			f = i
		}
		if pointDistDouble(txy, point{xs[i], ys[i]}) == rs[i]*rs[i] {
			t = i
		}
	}
	uf := newUnionFind(n)
	for i := 0; i < n; i++ {
		for j := 0; j < n; j++ {
			...
			if pointDistDouble(point{xs[i], ys[i]}, point{xs[j], ys[j]}) <= (rs[i]+rs[j])*(rs[i]+rs[j]) {
				if pointDistDouble(point{xs[i], ys[i]}, point{xs[j], ys[j]}) >= (rs[i]-rs[j])*(rs[i]-rs[j]) {
					uf.unite(i, j)
				}
			}
