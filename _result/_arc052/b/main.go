...
func main() {
	...
	for qi := 0; qi < q; qi++ {
		...
		for i := 0; i < n; i++ {
			...
			if xs[i] <= a {
				nr := rs[i] * float64(xts[i]-a) / float64(xts[i]-xs[i])
				t += nr * nr * math.Pi * float64(xts[i]-a) / 3
			} else {
				t += rs[i] * rs[i] * math.Pi * hs[i] / 3
			}
			if b <= xts[i] {
				nr := rs[i] * float64(xts[i]-b) / float64(xts[i]-xs[i])
				t -= nr * nr * math.Pi * float64(xts[i]-b) / 3
			}
		}
		outfl(t)
	}
