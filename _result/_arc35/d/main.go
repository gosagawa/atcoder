...
func main() {
	...
	mapping := func(f segfstruct, x segstruct) segstruct {
		...
		return segstruct{x.v + float64(f)*float64(x.size), x.size}
	}
	compostion := func(f segfstruct, g segfstruct) segfstruct {
		...
		return segfstruct(float64(f) + float64(g))
	}
	mx := int(1e6)*2 + 10
	fl := make([]float64, mx)
	for i := 1; i < mx; i++ {
		fl[i] = fl[i-1] + math.Log(float64(i))
	}
	dbg(math.Log(1), fl[:10])

	base := make([]segstruct, n)
	culc := func(i int) segstruct {
		dx := ps[i+1] - ps[i]
		dy := qs[i+1] - qs[i]

		return segstruct{fl[dx+dy] - fl[dx] - fl[dy], 1}
	}
	for i := 0; i < n-1; i++ {
		base[i] = culc(i)
	}
	...
	for qi := 0; qi < q; qi++ {
		...
		case 1:
			...
			qs[k-1] = b
			if k != 1 {
				lst.set(k-2, culc(k-2))
			}
			if k != n {
				lst.set(k-1, culc(k-1))
			}
		case 2:
			l1, r1, l2, r2 := ni4()
			v1 := lst.prod(l1-1, r1-1)
			v2 := lst.prod(l2-1, r2-1)
			dbg(v1.v, v2.v)
			if v1.v > v2.v {
				out("FIRST")
			} else {
