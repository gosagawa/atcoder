...
func main() {
	...
	for q.Len() > 0 {
		...
		for _, d := range dr {
			np := pointAdd(p, d)
			if !np.isValid(h, w) || !m[np.x][np.y] || dist[np.x][np.y] <= dist[p.x][p.y] {
				continue
			}
		...
		for _, d := range dr {
			np := pointAdd(p, d)
			if !np.isValid(h, w) || dist[np.x][np.y] <= dist[p.x][p.y]+1 {
				continue
			}
...
func (q *pointQueue) pushfront(pt point) {
	q.pt = append([]point{pt}, q.pt...)
}
