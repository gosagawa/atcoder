...
func main() {
	...
	as2 := [][3]int{}
	for i, v := range as[c1] {
		as2 = append(as2, [3]int{c1, v, i})
	}
	for i, v := range as[c2] {
		as2 = append(as2, [3]int{c2, v, i})
	}
	sort.Slice(as2, func(i, j int) bool { return as2[i][1] < as2[j][1] })
	for i := range as2 {
		if i > 0 {
			if as2[i][0] != as2[i-1][0] {
				o = min(o, as2[i][1]-as2[i-1][1])
			}
		}
	}
	...
	for j := 0; j < 2; j++ {
		...
		for i := range as2 {
			if i > 0 {
				if as2[i][0] != as2[i-1][0] {
					if o1 > as2[i][1]-as2[i-1][1] {
						o1 = as2[i][1] - as2[i-1][1]
						if as2[i][0] == c0 {
							usedidx = as2[i][2]
						} else {
							usedidx = as2[i-1][2]
