...
func main() {
	...
	ns2 := []point{}
	for i := 0; i < n; i++ {
		if ns[i].x == minx || ns[i].x == minx2 || ns[i].x == maxx || ns[i].x == maxx2 || ns[i].y == miny || ns[i].y == miny2 || ns[i].y == maxy || ns[i].y == maxy2 {
			ns2 = append(ns2, ns[i])
		}
	}
	r := []int{}
	for i := 0; i < len(ns2); i++ {
		for j := i + 1; j < len(ns2); j++ {
			r = append(r, max(abs(ns2[i].x-ns2[j].x), abs(ns2[i].y-ns2[j].y)))
		}
	}
	sortir(r)
	o = r[1]
