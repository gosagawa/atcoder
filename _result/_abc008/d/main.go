...
func main() {
	...
	for i := 0; i < n; i++ {
		xs[i]--
		ys[i]--
		txs = append(txs, max(0, xs[i]-1))
		txs = append(txs, xs[i])
		txs = append(txs, min(w-1, xs[i]+1))
		tys = append(tys, max(0, ys[i]-1))
		tys = append(tys, ys[i])
		tys = append(tys, min(h-1, ys[i]+1))
	}
	xsl, xm := cocom(txs)
	ysl, ym := cocom(tys)
	nw := len(xsl)
	nh := len(ysl)
	//out(xsl, ysl, nw, nh)
	dp := make([]map[int]int, nw*nh)
	for i := range dp {
		dp[i] = make(map[int]int)
	}

	var dfs func(f, t point) int
	dfs = func(f, t point) int {
		if f.x > t.x || f.y > t.y || !f.isValid(nw, nh) || !t.isValid(nw, nh) {
			return 0
		}
		if v, ok := dp[f.idx(nw)][t.idx(nw)]; ok {
			return v
		}
		r := 0
		for i := 0; i < n; i++ {
			tx := xm[xs[i]]
			ty := ym[ys[i]]
			if tx < f.x || t.x < tx || ty < f.y || t.y < ty {
				continue
			}
			tr := 0
			tr += xsl[tx] - xsl[f.x]
			...
			tr += ysl[ty] - ysl[f.y]
			tr += ysl[t.y] - ysl[ty]
			tr += dfs(f, point{tx - 1, ty - 1})
			tr += dfs(point{tx + 1, f.y}, point{t.x, ty - 1})
			tr += dfs(point{f.x, ty + 1}, point{tx - 1, t.y})
			tr += dfs(point{tx + 1, ty + 1}, t)
			maxs(&r, tr)
		}

		dp[f.idx(nw)][t.idx(nw)] = r
		//		out(f, t, r)
		return r
	}

	o = dfs(point{0, 0}, point{nw - 1, nh - 1}) + n
	out(o)
}
