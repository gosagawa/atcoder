...
func main() {
	...
	for e != nil {
		...
		if t.x == len(ss) && t.y == len(ts) {
			out("Yes")
			return
		}
		if v, ok := mp[[2]int{t.x, t.y}]; ok {
			if v < t.c {
				e = e.Next()
				continue
			}
		} else {
			mp[[2]int{t.x, t.y}] = t.c
		}

		if gi(ss, t.x) == gi(ts, t.y) && gi(ss, t.x) != inf {
			q.InsertAfter(pqst{t.x + 1, t.y + 1, t.c}, e)
		} else if t.c < k {
			q.PushBack(pqst{t.x + 1, t.y + 1, t.c + 1})
			q.PushBack(pqst{t.x + 1, t.y, t.c + 1})
			q.PushBack(pqst{t.x, t.y + 1, t.c + 1})
		}
