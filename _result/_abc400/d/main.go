...
func main() {
	...
	for e != nil {
		...
		for k := 0; k < 4; k++ {
			...
			if !np.isValid(h, w) || mp[np.x][np.y] == 1 || dist[np.x][np.y] != inf {
				continue
			}
		...
		for k := 0; k < 4; k++ {
			np2 := point{p.x + dx[k], p.y + dy[k]}
			if !np2.isValid(h, w) || mp[np2.x][np2.y] == 0 || dist[np2.x][np2.y] != inf {
				continue
			}
			dist[np2.x][np2.y] = dist[p.x][p.y] + 1
			q.PushBack(np2)
		}

		for k := 0; k < 4; k++ {
			np2 := point{p.x + dx[k]*2, p.y + dy[k]*2}
			if !np2.isValid(h, w) || mp[np2.x][np2.y] == 0 || dist[np2.x][np2.y] != inf {
				continue
			}
			dist[np2.x][np2.y] = dist[p.x][p.y] + 1
