...
func main() {
	...
	for i := 0; i < n; i++ {
		if as[i] == 1 && bs[i] == 1 {
			sl11 = append(sl11, cs[i])
		} else if as[i] == 1 && bs[i] == 0 {
			sl10 = append(sl10, cs[i])
		} else if as[i] == 0 && bs[i] == 1 {
			sl01 = append(sl01, cs[i])
		}
	}
	sorti(sl11)
	sorti(sl10)
	...
	s11sum := issum(sl11)
	cs11sum := newcusum(sl11)
	cs01sum := newcusum(sl01)
	cs10sum := newcusum(sl10)
	tct := len(sl10) + len(sl01)
	to += (tct) * s11sum
	mins(&o, to)
	dbg(to)
	for i := len(sl11) - 1; i >= 0; i-- {
		to += 2 * cs11sum.get(0, i+1)
		t1 := lowerBound(sl11[i], sl10)
		t2 := lowerBound(sl11[i], sl01)
		c1 := cs10sum.get(0, t1+1)
		c2 := cs01sum.get(0, t2+1)
		to += c1
		to += c2
		dbg(to)
		t3 := 1 + (t1 + 1) + (t2 + 1)
		to -= t3 * sl11[i]
		dbg(to, cs11sum.get(0, i+1), i, t1, t2, c1, c2, t3)
		mins(&o, to)
	}
	out(o)
}
