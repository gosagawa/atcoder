...
func main() {
	...
	for i := 0; i < n; i++ {
		l, r := ni2()
		if _, ok := mp[[2]int{l, r}]; ok {
			continue
		}
		mp[[2]int{l, r}] = i
		lrs = append(lrs, [2]int{l, r})
	}
	n = len(lrs)
	sort2ar(lrs, opt2ar(1, asc), opt2ar(0, asc))
	if n == 1 {
		out((lrs[0][1] - lrs[0][0] + 1) * 2)
		return
	}
	...
	switch len(tls) {
	case 1:
		sort2ar(lrs, opt2ar(0, desc))
		if lrs[0] == tls[0] {
			for i := 1; i < n; i++ {
				maxs(&o, lrs[0][1]-lrs[0][0]+1+lrs[i][1]-lrs[i][0]+1)
			}
		} else {
			tls = [][2]int{tls[0], lrs[0]}
			//			out(tls)
			sort2ar(lrs, opt2ar(1, asc), opt2ar(0, asc))
			if false {
				for _, v := range lrs {
			...
			for _, v := range lrs {
				...
				maxs(&o, tls[0][1]-tls[1][0]+1+v[1]-v[0]+1)
				if v[1] > tls[1][1] {
					maxs(&maxt2r, v[0])
				}
			}
			...
			for i, v := range lrs {
				...
				nv := lrs[i+1]
				if tls[0] == v {
					maxs(&o, v[1]-v[0]+1+max(nv[1]-maxt2r+1, 0))
					continue
				}
				maxs(&t1l, v[0])
				maxs(&o, max(t1r-t1l+1, 0)+max(nv[1]-maxt2r+1, 0))
			}
		}
