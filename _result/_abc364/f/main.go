...
func main() {
	...
	lrc := make([][3]int, q)
	for i := 0; i < q; i++ {
		lrc[i] = [3]int{ls[i], rs[i], cs[i]}
	}
	sort2ar(lrc, opt2ar(0, asc), opt2ar(1, asc))
	...
	mapping := func(f segfstruct, x segstruct) segstruct {
		...
		return segstruct(min(int(f), int(x)))
	}
	compostion := func(f segfstruct, g segfstruct) segfstruct {
		if f == id() {
			return g
		}
		return segfstruct(min(int(f), int(g)))
	}
	base := make([]segstruct, n)
	for i := 0; i < n; i++ {
		base[i] = segstruct(inf)
	}
	...
	for i := 0; i < q; i++ {
		l, r, c := lrc[i][0], lrc[i][1], lrc[i][2]
		o += c
		dbg(i, r, c)

		lst.applyrange(l-1, r-1, segfstruct(c))
	}
	for i := 0; i < n-1; i++ {
		t := int(lst.prod(i, i+1))
		dbg(t)
		if t == inf {
			out(-1)
			return
