...
func main() {
	...
	for i := 0; i < e; i++ {
		...
		ss--
		t--
		edges[ss] = append(edges[ss], edge{to: t})
		edges[t] = append(edges[t], edge{to: ss})
	}
	dp := make([][][2]int, k+1)
	for i := 0; i <= k; i++ {
		dp[i] = make([][2]int, v)
	...
	for i := 0; i < k; i++ {
		for j := 0; j < v; j++ {
			for _, nv := range edges[j] {
				if nv.to == x {
					dp[i+1][nv.to][0] = madd(dp[i+1][nv.to][0], dp[i][j][1])
					dp[i+1][nv.to][1] = madd(dp[i+1][nv.to][1], dp[i][j][0])
				} else {
					dp[i+1][nv.to][0] = madd(dp[i+1][nv.to][0], dp[i][j][0])
					dp[i+1][nv.to][1] = madd(dp[i+1][nv.to][1], dp[i][j][1])
				}
			}
		}
	}
