...
func main() {
	...
	for i := 0; i < n; i++ {
		...
		if used[i] && used[ps[i]] {
			if !uf.issame(i, ps[i]) {
				o = madd(o, mmul(mn[sc-2], mmul(mdiv(mmul(m, m-1), 2), mn[nd])))
				sc--
				uf.unite(i, ps[i])
			}
			continue
		...
		//		out(i+1, ps[i]+1, sc-1, nd, mdiv(mmul(m, m-1), 2), used[i], used[ps[i]])

		o = madd(o, mmul(mn[sc-1], mmul(mdiv(mmul(m, m-1), 2), mn[nd])))
		used[i] = true
		used[ps[i]] = true
		uf.unite(i, ps[i])
	}
