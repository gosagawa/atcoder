...
func main() {
	...
	temp := make([]segstruct, len(ts)-1)
	for i := 0; i < len(ts)-1; i++ {
		temp[i].y = ts[i+1] - ts[i]
	}
	//out(ts, temp)

	op := func(a, b segstruct) segstruct {
		a.x += b.x
		a.y += b.y
		return a
	}
	...
	composition := func(a, b segfstruct) segfstruct {
		if a.x == -1 {
			return b
		}
		return a
	...
	id := func() segfstruct {
		return segfstruct{-1, 0}
	}
	stree := newlazysegtree(len(temp), temp, op, e, mapping, composition, id)
	for i := 0; i < n; i++ {
		il := lowerBound(ls[i], ts) + 1
		ir := lowerBound(rs[i], ts) + 1
		X := rs[i] - ls[i]
		f := func(x segstruct) bool {
			return x.x <= X
		}
		rr := stree.maxright(il, f)
		t := stree.prod(il, rr)
		//out(il, ir, rr, t, X)
		X -= t.x
		stree.applyrange(il, rr, segfstruct{0, 0})
		...
		stree.applyrange(il, ir, segfstruct{1, 0})

		//out(stree.d[stree.size:])
