...
func main() {
	...
	for i := 0; i < n; i++ {
		m.AddEdge(0, i+1, 1, 0)
		m.AddEdge(n+i+1, n*2+1, 1, 0)
		for j := 0; j < n; j++ {
			d := int(pointDist(abs[i], cds[j]) * float64(1e14))
			mp[i][j] = d
			m.AddEdge(i+1, n+j+1, 1, d)
		}
	}
	dbg(mp)
	result := m.Slope(0, n*2+1, n)
	dbg(result)
	dbg(m.edges)
	rs := is(n, 0)
	for _, v := range m.edges {
		dbg(v)
		if v.flow == 1 && v.from >= 1 && v.from <= n {
			dbg(v.from, v.to, v.cost)
			rs[v.from-1] = v.to - n
		}
	}
...
func (m *McfGraph) Slope(s, t int, arg ...int) [][2]int {
	...
	for {
		...
		*/
		...
		if flow >= flowLimit {
			dbg("break flow", flow, flowLimit)
			break
		}

		if !m.dualRef(s, t) {
			dbg("break dualref", s, t)
			break
		}
		c := flowLimit - flow
