...
func main() {
	...
	h, w := ni2()
	mp := convidxi2s(nsi2s(h), map[string]int{".": 0, "#": 1, "E": 2})
	dist := i2s(h, w, 0)
	...
	ds := []string{"v", ">", "^", "<", "RD", "RU", "LD", "LU"}
	for e != nil {
		t := e.Value.(point)
		for k := 0; k < 4; k++ {
			np := point{t.x + dx[k], t.y + dy[k]}
			if np.isValid(h, w) && mp[np.x][np.y] == 0 && dist[np.x][np.y] == 0 {
	...
	for i := 0; i < h; i++ {
		...
		for j := 0; j < w; j++ {
			...
			for k := 0; k < 4; k++ {
				...
				if np.isValid(h, w) && mp[np.x][np.y] != 1 && dist[np.x][np.y] == dist[i][j]-1 {
					sl[j] = ds[k]
					break
				}
			}
