...
func main() {
	...
	for i := 0; i < m; i++ {
		...
		abs[i] = [2]int{a, b}
		ts = append(ts, [3]int{0, a, i})
		ts = append(ts, [3]int{1, b, i})
	}
	q := ni()
	rs := is(q, 0)
	cds := make([][2]int, q)
	for i := 0; i < q; i++ {
		...
		cds[i] = [2]int{c, d}
		ts = append(ts, [3]int{2, c, i})
		ts = append(ts, [3]int{3, d, i})
	}
	sort2ar(ts, opt2ar(1, asc))
	bit := newbit(n * 2)
	for _, t := range ts {
		switch t[0] {
		case 0:
		...
		case 1:
			...
			bit.add(abs[t[2]][1], -1)
		case 2:
			t1 := bit.rangesum(cds[t[2]][0], cds[t[2]][1])
			dbg(cds[t[2]][0], t[2], cds[t[2]], t1)
			rs[t[2]] += t1
		case 3:
			t2 := bit.rangesum(cds[t[2]][0], cds[t[2]][1])
			rs[t[2]] += t2
			dbg(cds[t[2]][1], t[2], cds[t[2]], t2)
		}
	}
