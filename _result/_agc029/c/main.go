...
func main() {
	...
	fn := func(c int) bool {
		...
		pop := func(j int) {
			...
			ts[len(ts)-1][1] = j - ts[len(ts)-1][0]
		}
		inc := func() bool {
			if len(ts) == 1 && ts[0][2] == c-1 {
				return false
			}
			t1 := 0
			t2 := 0
			if ts[len(ts)-1][2] == c-1 {
				t1 = ts[len(ts)-1][0]
				t2 = ts[len(ts)-1][1]
				ts = ts[:len(ts)-1]
			}
			...
			if ts[len(ts)-1][1] == 1 {
				...
				ts[len(ts)-1][1]--
				ni := ts[len(ts)-1][0] + ts[len(ts)-1][1]
				ts = append(ts, [3]int{ni, 1, ts[len(ts)-1][2] + 1})
			}
			if len(ts) > 1 && ts[len(ts)-1][2] == ts[len(ts)-2][2] {
				ts[len(ts)-2][1] += ts[len(ts)-1][1]
				ts = ts[:len(ts)-1]
			}
			if t1 > 0 {
				ts = append(ts, [3]int{t1, t2, 0})
			}
		...
		for i := 1; i < n; i++ {
			if ns[i] <= ns[i-1] {
				...
				ts = append(ts, [3]int{ns[i-1], ns[i] - ns[i-1], 0})
				if len(ts) > 1 && ts[len(ts)-1][2] == ts[len(ts)-2][2] {
					ts[len(ts)-2][1] += ts[len(ts)-1][1]
					ts = ts[:len(ts)-1]
				}
