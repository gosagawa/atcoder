...
func main() {
	...
	mp := i2s(pow2(maxpr)+1, pow2(maxpr)+1, 0)
	nxt := is(pow2(maxpr), 0)
	for i := 0; i < pow2(maxpr); i++ {
		for j := 1; j < m+1; j++ {
			add := 1
			for k := 0; k < maxpr; k++ {
				if !hasbit(i, k) {
	...
	mp[pow2(maxpr)-1][pow2(maxpr)] = 1
	mp[pow2(maxpr)][pow2(maxpr)] = 1
	flg := false
	if flg {
		for _, v := range mp {
			outis(v)
