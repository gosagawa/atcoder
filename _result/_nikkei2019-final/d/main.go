...
func main() {
	...
	buckets := make([]state, n/b+1)
	for i := 0; i <= n/b; i++ {
		buckets[i].size = min(b, n-i*b)
		buckets[i].lens = make([]int, buckets[i].size)
	}
	...
	for i := 0; i < m; i++ {
		...
		r--
		for j := l / b; j <= r/b; j++ {
			td := t - buckets[j].lastCut
			if l <= j*b && (j+1)*b-1 <= r {
				o += buckets[j].sum
				o += td * buckets[j].size
				buckets[j].reset = true
				buckets[j].sum = 0
			} else {
				for k := 0; k < buckets[j].size; k++ {
					...
					if j*b+k < l || j*b+k > r {
						...
						o += buckets[j].lens[k] + td
						buckets[j].sum -= buckets[j].lens[k]
						buckets[j].lens[k] = 0
					}
				}
				buckets[j].reset = false
			}
			buckets[j].lastCut = t
		}
		//		out(o, buckets)
