...
func main() {
	...
	for i := 0; i < e; i++ {
		...
		s--
		t--
		edges[s] = append(edges[s], edge{to: t, cost: 0})
		edges[t] = append(edges[t], edge{to: s, cost: 0})
	}
	used := make([]bool, v)
	r := [][2]int{}
	var dfs func(vv, p int)
	dfs = func(vv, p int) {
		if used[vv] {
			return
		}
		used[vv] = true
		for _, nv := range edges[vv] {
			if nv.to != p {
				if used[nv.to] {
					continue
				}
				r = append(r, [2]int{vv + 1, nv.to + 1})
				dfs(nv.to, vv)
			}
		}
	}
