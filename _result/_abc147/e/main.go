...
func main() {
	...
	for i := 0; i < h; i++ {
		dp[i] = make([][]bool, w)
		for j := 0; j < w; j++ {
			dp[i][j] = make([]bool, 12801)
		}
	}
	dp[0][0][as[0][0]-bs[0][0]+6400] = true
	dp[0][0][bs[0][0]-as[0][0]+6400] = true
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			diff := as[i][j] - bs[i][j]
			if i != 0 {
				for v, hv := range dp[i-1][j] {
					if !hv {
						continue
					}
					if diff+v >= 0 && diff+v <= 12800 {
						dp[i][j][diff+v] = true
					}
					if -diff+v >= 0 && -diff+v <= 12800 {
						dp[i][j][-diff+v] = true
