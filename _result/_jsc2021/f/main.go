...
func main() {
	...
	n, m, q := ni3()
	ts, xs, ys := ni3s(q)
	_, cym := cocom(append(ys, 0))
	var cbit, tbit [2]*bit
	cbit[0] = newbit(len(cym))
	cbit[1] = newbit(len(cym))
	tbit[0] = newbit(len(cym))
	tbit[1] = newbit(len(cym))
	cbit[0].add(0, n)
	cbit[1].add(0, m)
	nms := i2s(2, max(n, m), 0)
	...
	for i := 0; i < q; i++ {
		...
		*/
		...
		o -= cbit[1^t].rangesum(0, pyi) * py
		o -= tbit[1^t].rangesum(pyi, len(cym))
		...
		o += cbit[1^t].rangesum(0, nyi) * ny
		o += tbit[1^t].rangesum(nyi, len(cym))
