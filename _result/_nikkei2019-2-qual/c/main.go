...
func main() {
	...
	sort2ar(abs, opt2ar(1, asc), opt2ar(0, asc))
	for i := 0; i < n; i++ {
		abs[i][2] = i
		abs[i][3] = i
		s.Increment(abs[i][0])
		if _, ok := mp[abs[i][0]]; !ok {
			mp[abs[i][0]] = make(map[int]bool)
		}
		mp[abs[i][0]][i] = true
	}
	dbg(abs)
	...
	for i := 0; i < n; i++ {
		...
		key, hasV := s.LowerBound(abs[i][1] + 1)
		if !hasV {
			out("No")
			return
		}
		...
		dbg(i, j, abs[i], abs[j], key, hasV)
		delete(mp[abs[i][0]], i)
		delete(mp[abs[j][0]], j)
		abs[i][0], abs[j][0] = abs[j][0], abs[i][0]
		abs[i][3], abs[j][3] = abs[j][3], abs[i][3]
		mp[abs[j][0]][j] = true
		s.Decrement(abs[i][0])
		t++
	}
