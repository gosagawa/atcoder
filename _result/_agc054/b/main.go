...
func main() {
	...
	for i := 0; i < n; i++ {
		for j := 0; j < n; j++ {
			for k := 0; k < sum+1; k++ {
				if dp[i][j][k] != 0 {
					dp[i+1][j][k] = madd(dp[i+1][j][k], dp[i][j][k])
					dp[i+1][j+1][k+ws[i]] = madd(dp[i+1][j+1][k+ws[i]], dp[i][j][k])
				}
			}
		}
	}
	cb := newcombFactorial(n + 10)
	for i := 1; i < n; i++ {
		o = madd(o, mmul(dp[n][i][sum/2], mmul(cb.factorial(i), cb.factorial(n-i))))
	}
