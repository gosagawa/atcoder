...
func main() {
	...
	for qi := 0; qi < q; qi++ {
		...
		if t == 1 {
			...
			if c == 0 {
				lst.applyrange(i+1, n, segfstruct(2))
				bt.add(i, 2)
				lst.set(i, segstruct(inf))
			} else {
				lst.applyrange(i+1, n, segfstruct(-2))
				bt.add(i, -2)
				lst.set(i, segstruct(bt.get(0, i+1)))
			}
			ts[i] = c
		} else {
			l, r := ni2()
