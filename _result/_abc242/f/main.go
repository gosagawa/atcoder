...
func main() {
	...
	comb := newcombFactorial(2501)
	var dfs func(i, j, idx, th int) int
	dfs = func(i, j, idx, th int) int {
		if dp[idx][i][j] != -1 {
			return dp[idx][i][j]
		}
		//out(i, j, idx, th)
		...
		t := comb.combination(i*j, th)
		for k := 1; k <= i; k++ {
			for l := 1; l <= j; l++ {
				if k == i && l == j {
					continue
				}
				ct := mmul(comb.combination(i, k), comb.combination(j, l))
				t = madd(t, mmul(ct, -dfs(k, l, idx, th)))
			}
		}
		dp[idx][i][j] = t
		return t
	...
	for i := 1; i <= n; i++ {
		for j := 1; j <= n-i; j++ {
			for k := 1; k <= m; k++ {
				for l := 1; l <= m-k; l++ {
					nt := mmul(comb.combination(n, i), comb.combination(n-i, j))
					mt := mmul(comb.combination(m, k), comb.combination(m-k, l))
					t := mmul(nt, mt)
					t = mmul(t, dp[0][i][k])
					t = mmul(t, dp[1][j][l])
					o = madd(o, t)
