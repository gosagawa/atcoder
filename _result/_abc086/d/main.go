...
func main() {
	...
	cusum2db := newcusum2d(mpb)
	cusum2dw := newcusum2d(mpw)
	sum := cusum2db.get(0, 0, k, k)
	o = max(o, sum)
	o = max(o, n-sum)
	for i := 0; i < k; i++ {
		for j := 0; j < k; j++ {
			t := cusum2db.get(0, 0, k, k) - cusum2db.get(0, 0, i, k) - cusum2db.get(0, 0, k, j) + cusum2db.get(0, 0, i, j)*2
			t += cusum2dw.get(0, 0, i, k) + cusum2dw.get(0, 0, k, j) - cusum2dw.get(0, 0, i, j)*2
			o = max(o, t)
			o = max(o, n-t)
		}
	}
