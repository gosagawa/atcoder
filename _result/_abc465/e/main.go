...
func main() {
	...
	dp[0][1][s%3][tk][pow2(s)] = 1
	for i := 1; i < s; i++ {
		tk = 0
		if i == 3 {
			tk = 1
		}
		dp[0][0][i%3][tk][pow2(i)] = 1
	}
	dp[0][0][0][0][0] = 1
				...
				for l := 0; l < 2; l++ {
					for m := 0; m < pow2(10); m++ {
						...
						for ni := 0; ni < 10; ni++ {
							...
							dp[i][nj][nk][nl][m|pow2(ni)] += dp[i-1][j][k][l][m]
							dp[i][nj][nk][nl][m|pow2(ni)] %= mod
						}
					}
