...
func main() {
	...
	for i := 0; i < t; i++ {
		...
		sort.Slice(lrs, func(i, j int) bool { return lrs[i][0] < lrs[j][0] })
		indexedlrs := [][][2]int{}
		tl := 0
		ti := -1
		for _, v := range lrs {
			if tl != v[0] {
				indexedlrs = append(indexedlrs, [][2]int{})
				tl = v[0]
				ti++
			}
			indexedlrs[ti] = append(indexedlrs[ti], v)
		}
		...
		for {
			if h.Len() == 0 {
				if ti == len(indexedlrs) {
					break
				}
				c = indexedlrs[ti][0][0]
			}
			if ti != len(indexedlrs) && indexedlrs[ti][0][0] == c {
				for _, v := range indexedlrs[ti] {
					heap.Push(h, v[1])
				}
				ti++
			}
