...
func main() {
	...
	mp := i2s(h, w, 0)
	if sw {
		ts := convidxi2s(nsi2s(w), map[string]int{"?": 0, "1": 1, "2": 2, "3": 3})
		for i := 0; i < w; i++ {
			for j := 0; j < h; j++ {
				mp[j][i] = ts[i][j]
			}
		...
		mp = convidxi2s(nsi2s(h), map[string]int{"?": 0, "1": 1, "2": 2, "3": 3})
	}
	dp := make(map[[14]int]int)
	dp[[14]int{}] = 1

	mv := func(ln [14]int, i int) [14]int {
		for j := 0; j < 13; j++ {
			ln[j] = ln[j+1]
		}
		ln[13] = i
	...
	for i := 0; i < h*w; i++ {
		ndp := make(map[[14]int]int)
		x := i / w
		y := i % w
		for ln, v := range dp {
			used := make([]bool, 3)
			for j := 0; j < 3; j++ {
				if mp[x][y] != 0 && mp[x][y] != j+1 {
					used[j] = true
				}
			}
			...
			//			dbg(i, ln, used, ln[13], ln[14-w], mp[x][y])
