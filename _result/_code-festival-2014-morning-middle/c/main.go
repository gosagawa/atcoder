...
func main() {
	...
	me := newMatrixExponentation(2, base)
	r = slmatrixmul(r, me.getpow(n))
...
func i2sf(l, m int, def float64) [][]float64 {
	sl := make([][]float64, l)
	for i := 0; i < l; i++ {
		sl[i] = make([]float64, m)
		for j := 0; j < m; j++ {
...
func matrixmul(a, b [][]float64) [][]float64 {
	...
	n1 := r[0][0] / (r[0][0] + r[0][1])
	n2 := r[0][1] / (r[0][0] + r[0][1])
	r[0][0] = n1
	r[0][1] = n2
	r[1][0] = n2
	r[1][1] = n1
...
func slmatrixmul(a []float64, b [][]float64) []float64 {
	ar := len(a)
	bc := len(b)
	br := len(b[0])
	if ar != bc {
...
func matrixpow(n int, matrix [][]float64) [][]float64 {

	size := len(matrix)
	base := make([][][]float64, maxlogn)
	base[0] = matrix
...
func newMatrixExponentation(size int, base [][]float64) *matrixExponentation {
	me := &matrixExponentation{
		size: size,
	}

	me.base = make([][][]float64, maxlogn)
	me.base[0] = base
	for i := 0; i < maxlogn-1; i++ {
		me.base[i+1] = matrixmul(me.base[i], me.base[i])
	}
	me.id = i2sf(size, size, 0)
	for i := 0; i < size; i++ {
		me.id[i][i] = 1
	}
	return me
...
func (me *matrixExponentation) getpow(n int) [][]float64 {
	r := i2sf(me.size, me.size, 0)
	copy(r, me.id)
	for i := 0; i < maxlogn; i++ {
		if hasbit(n, i) {
