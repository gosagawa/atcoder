...
func main() {
	...
	for i, v := range tree.oiler {
		...
		if tree.oilerv[i] != -1 {
			fv[tree.oilerv[i]] = append(fv[tree.oilerv[i]], i)
		}
	}
	/*
		out(tree.oiler)
	...
	*/
	...
	for i := 0; i < q; i++ {
		...
		out(bs(0, len(sl)-1, f2) - bs(0, len(sl)-1, f1))
	}
	/*
		a := ni()
		s := ns()
...
func (t *tree) dfs(v, p, d int) {
	...
	t.parent[0][v] = p
	t.depth[v] = d
	t.oiler = append(t.oiler, v)
	t.oilerv = append(t.oilerv, d)
