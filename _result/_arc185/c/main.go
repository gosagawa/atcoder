...
func main() {
	...
	for i := 0; i < n; i++ {
		...
		fs[as[i]]++
		tis[as[i]] = append(tis[as[i]], i)
		if len(tis[as[i]]) == 2 {
			ds = append(ds, as[i])
		}
		if len(tis[as[i]]) == 3 && x3 == as[i] {
			sl := []int{tis[as[i]][0] + 1, tis[as[i]][1] + 1, tis[as[i]][2] + 1}
			sorti(sl)
			outf("%v %v %v", sl[0], sl[1], sl[2])
			return
		}
	}
	for _, v := range ds {
		t := x - v*2
		if t > 0 && t != v && fs[t] > 0 {
			sl := []int{tis[v][0] + 1, tis[v][1] + 1, tis[t][0] + 1}
			sorti(sl)
			outf("%v %v %v", sl[0], sl[1], sl[2])
			return
		}
	}
	gs := convolution(fs, fs)
	gs = gs[:x+1]
	dbg(fs)
	for i := range gs {
		if i%2 == 0 {
