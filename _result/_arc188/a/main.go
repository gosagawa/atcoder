...
func main() {
	...
	o := 0
	n, K := ni2()
	s := convidxis(nsis(), map[string]int{"?": 0, "A": 1, "B": 2, "C": 3})
	dp := [51][4][51][51][51]int{}
	dp[0][0][0][0][0] = 1
	set := func(j, k, l, m, p int) (int, int, int, int) {
		j ^= p
		switch j {
		case 1:
			k++
