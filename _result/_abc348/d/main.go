...
func main() {
	...
	h, w := ni2()
	mp := convidxi2s(nsi2s(h), map[string]int{".": 0, "#": 1, "S": 2, "T": 3})
	var s, g point
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			if mp[i][j] == 2 {
	...
	q := list.New()
	q.PushBack(point{s.x, s.y})
	st[s.x][s.y] = 0

	e := q.Front()
	for e != nil {
		p := e.Value.(point)
		if rce[p.x][p.y] > st[p.x][p.y] {
			st[p.x][p.y] = rce[p.x][p.y]
		}
		if st[p.x][p.y] == 0 {
			e = e.Next()
			continue
		...
		for i := 0; i < 4; i++ {
			...
			if st[np.x][np.y] < st[p.x][p.y]-1 {
				st[np.x][np.y] = st[p.x][p.y] - 1
				q.PushBack(np)
			}
		}
		e = e.Next()
