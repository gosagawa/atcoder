...
func main() {
	...
	h, w := ni2()
	as := make([][]int, h)
	as3 := make([]map[int][]int, h)
	rs := make([][2]int, h)
	tw := 0
	for i := 0; i < h; i++ {
		as[i] = nis(w)
		as3[i] = make(map[int][]int)
		mx := 0
		mi := inf
		for j, v := range as[i] {
			if v != 0 {
	...
	sort2ar(rs, opt2ar(0, asc), opt2ar(1, asc))
	for i := 0; i < h-1; i++ {
		if rs[i][0] == 0 {
			continue
		}
	...
	for i := 0; i < h; i++ {
		...
		sorti(as2)
		for j := 0; j < len(as2)-1; j++ {
			for _, v := range as3[i][as2[j]] {
				s := v
				t := twi
				edges[s] = append(edges[s], edge{to: t})
				deg[t]++
			}
			for _, v := range as3[i][as2[j+1]] {
				s := twi
				t := v
				edges[s] = append(edges[s], edge{to: t})
				deg[t]++
	...
	graph := newgraph(w, edges)
	isdag, _ := graph.topologicalSort(deg)
	if isdag {
		out("Yes")
		return
	} else {
