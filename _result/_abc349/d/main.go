...
func main() {
	...
	t2s := make([][2]int, 0)
	for i := 61; i >= 0; i-- {
		if (l+ts[i]-1)/ts[i] < r/ts[i] {
			t1 := (l + ts[i] - 1) / ts[i] * ts[i]
			t2 := t1 + ts[i]
			t2s = append(t2s, [2]int{t1, t2})
			break
		}
	}
	debug(t2s)
	...
	for i := 61; i >= 0; i-- {
		if s-ts[i] >= l {
			t2s = append(t2s, [2]int{s - ts[i], s})
			s = s - ts[i]
		}
	}
	sort2ar(t2s, opt2ar(0, asc))
	out(len(t2s))
	for _, v := range t2s {
		out2d(v[0], v[1])
	}
