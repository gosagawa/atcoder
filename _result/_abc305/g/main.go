...
func main() {
	...
	maxlogn := 62
	allList := make([][][]int, maxlogn+1)
	allList[0] = base
	culcbase := i2s(lpow, lpow, 0)
	for i := 0; i < lpow; i++ {
		culcbase[i][i] = 1
	}
	allList[0] = base
	for l := 0; l < maxlogn; l++ {
		allList[l+1] = sum(allList[l], allList[l])
	}
	for l := 0; l < maxlogn; l++ {
		if hasbit(n-lmax, l) {
			culcbase = sum(culcbase, allList[l])
		}
	}
	for i := 0; i < lpow; i++ {
		for j := 0; j < lpow; j++ {
			o += mmul(r[i], culcbase[i][j])
			o %= mod
		}
	}
	out(o)
