...
func main() {
	...
	n, k := ni2()
	divs := [][51]int{}
	var dfs func(i int, mx int, st [51]int)
	dfs = func(i int, mx int, st [51]int) {
		if i == n {
			divs = append(divs, st)
			return
		}
	...
	for _, v := range divs {
		...
		for j := 0; j < n+1; j++ {
			...
			scb = gcm(scb, j)
			c = mdiv(c, mpow(cf.factorial(j), v[j], mod))
			c = mdiv(c, cf.factorial(v[j]))
			c = mmul(c, mpow(cf.factorial(j-1), v[j], mod))
		}
		sc := mpow(scb, k, mod)
		o = madd(o, mmul(sc, c))

		debug(v[:n+1], scb, sc, c)
