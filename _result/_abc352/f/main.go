...
func main() {
	...
	for i := 0; i < m; i++ {
		...
		uf.unite(bs[i], as[i], cs[i])
	}
	ts := make([][]int, n)
	for i := 0; i < n; i++ {
		ts[uf.root(i)] = append(ts[uf.root(i)], uf.weight(i))
	}
	l1 := 0
	ts2 := is(n, 0)
	for i := range ts {
	...
	ans := is(n, -1)
	var dfs func(v, ci, i int) bool
	dfs = func(v, ci, i int) bool {
		if i == n {
			return true
		}
		if ci == i || ts2[i] == 0 {
	...
	for i := 0; i < n; i++ {
		if ans[uf.root(i)] != -1 {
			rs[i] = ans[uf.root(i)] - ts[uf.root(i)][0] + uf.weight(i) + 1
		}
	}
	outis(rs)
}
