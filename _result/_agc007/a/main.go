...
func main() {
	...
	for {
		...
		np3 := point{p.x - 1, p.y}
		np4 := point{p.x, p.y - 1}
		if lastp != np3 && np3.isValid(h, w) && mp[np3.x][np3.y] == 1 {
			dbg("np3")
			out("Impossible")
			return
		} else if lastp != np4 && np4.isValid(h, w) && mp[np4.x][np4.y] == 1 {
			dbg(p, np4, "np4")
			out("Impossible")
			return
		} else if np.isValid(h, w) && mp[np.x][np.y] == 1 {
			if np2.isValid(h, w) && mp[np2.x][np2.y] == 1 {
				out("Impossible")
				return
			}
			lastp = p
			p = np
		} else if np2.isValid(h, w) && mp[np2.x][np2.y] == 1 {
			lastp = p
			p = np2
		} else {
			if p.x == h-1 && p.y == w-1 {
