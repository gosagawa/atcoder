...
func main() {
	...
	for i := 0; i < n; i++ {
		x1, y1, d1 := ni3()
		x2, y2, d2 := ni3()
		ns[i] = [2][3]int{{x1, y1, d1}, {x2, y2, d2}}
		xs[i*2] = x1
		xs[i*2+1] = x2
		ys[i*2] = y1
		ys[i*2+1] = y2
	...
	ycs, ym := cocom(ys)
	dcs, dm := cocom(ds)
	xyds := make([][][]int, len(xcs)+1)
	for i := 0; i < len(xcs)+1; i++ {
		xyds[i] = make([][]int, len(ycs)+1)
		for j := 0; j < len(ycs)+1; j++ {
			xyds[i][j] = make([]int, len(dcs)+1)
		}
	}
	for i := 0; i < n; i++ {
		xi1 := xm[ns[i][0][0]]
	...
	for i := 0; i < len(xcs); i++ {
		for j := 0; j < len(ycs); j++ {
			for k := 0; k < len(dcs); k++ {
				if xyds[i][j][k] >= K {
					o += (xcs[i+1] - xcs[i]) * (ycs[j+1] - ycs[j]) * (dcs[k+1] - dcs[k])
				}
			}
		}
	}
