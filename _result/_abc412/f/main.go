...
func main() {
	...
	for i := n - 1; i >= 0; i-- {
		rest -= as[i]
		dbg(rest, mdiv(sum, sum-rest))
		// a(n-1)=2/6 a(n) + 4/6 a(n-1) + 1
		// a(n-2)=2/6 a(n) + 3/6 a(n-1) + 1/6 a(n-2) + 1
		dp[i] = mmul(mdiv(sum, sum-rest), 1+other)
		other += mmul(dp[i], mdiv(as[i], sum))
	}
	dbg(dp)
	o = dp[ai]
