...
func main() {
	...
	dp := make([][]int, n+1)
	for i := range dp {
		dp[i] = make([]int, pow2(d*2+1))
	}
	dp[0][t] = 1
	for i := 0; i < n; i++ {
		next := 0
		if i+d+1 < n && ns[i+d+1] == -1 {
		} else {
			next = pow2(2 * d)
		}
		for j := 0; j < pow2(d*2+1); j++ {
			if used[i] {
				if j%2 == 1 {
					dp[i+1][j/2+next] = madd(dp[i+1][j/2+next], dp[i][j])
				}
			} else {
				for k := 0; k < d*2+1; k++ {
					if !hasbit(j, k) {
						nj := j + pow2(k)
						if nj%2 == 1 {
							dp[i+1][nj/2+next] = madd(dp[i+1][nj/2+next], dp[i][j])
						}
					}
				}
			}
