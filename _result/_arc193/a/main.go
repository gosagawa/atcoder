...
func main() {
	...
	rss := is(n*2+2, inf)
	for i := 0; i < n; i++ {
		mins(&lss[ls[i]-1], ws[i])
		mins(&rss[rs[i]+1], ws[i])
	}
	dbg(lss)
	dbg(rss)
	for i := n * 2; i >= 0; i-- {
	...
	for qi := 0; qi < q; qi++ {
		...
		s--
		t--
		if rs[s] < ls[t] || rs[t] < ls[s] {
			out(ws[s] + ws[t])
			continue
		}
		o := inf
		mins(&o, lss[max(rs[s], rs[t])]+ws[s]+ws[t])
		mins(&o, rss[min(ls[s], ls[t])]+ws[s]+ws[t])
		mins(&o, lss[rs[s]]+rss[ls[t]]+ws[s]+ws[t])
		mins(&o, rss[ls[s]]+lss[rs[t]]+ws[s]+ws[t])
