...
func main() {
	...
	for i := h + w - 1; i >= 0; i-- {
		for j := 0; j < h; j++ {
			...
			ptw := point{pt.x, pt.y + 1}
			pth := point{pt.x + 1, pt.y}
			pthw := point{pt.x + 1, pt.y + 1}
			st := 0
			if ptw.isValid(h, w) && mp[ptw.x][ptw.y] == 0 && stm[ptw.x][ptw.y] == 0 {
				st = 1
			}
			if pth.isValid(h, w) && mp[pth.x][pth.y] == 0 && stm[pth.x][pth.y] == 0 {
				st = 1
			}
			if pthw.isValid(h, w) && mp[pthw.x][pthw.y] == 0 && stm[pthw.x][pthw.y] == 0 {
				st = 1
			}
			stm[pt.x][pt.y] = st
		}
