...
func main() {
	...
	dp[0][ss[0]^1][0] = cs[0]
	for i := 1; i < n; i++ {
		mins(&dp[i][ss[i]][1], dp[i-1][ss[i]^1][1])
		if dp[i-1][ss[i]][1] != inf {
			mins(&dp[i][ss[i]^1][1], dp[i-1][ss[i]][1]+cs[i])
		}

		mins(&dp[i][ss[i]][1], dp[i-1][ss[i]][0])
		if dp[i-1][ss[i]^1][0] != inf {
			mins(&dp[i][ss[i]^1][1], dp[i-1][ss[i]^1][0]+cs[i])
		}

		mins(&dp[i][ss[i]][0], dp[i-1][ss[i]^1][0])
		if dp[i-1][ss[i]][0] != inf {
			mins(&dp[i][ss[i]^1][0], dp[i-1][ss[i]][0]+cs[i])
		}
	}
	o = min(dp[n-1][0][1], dp[n-1][1][1])
