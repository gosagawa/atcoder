...
func main() {
	...
	op := func(a segstruct, b segstruct) segstruct {
		...
		return segstruct{mv, mc, a.size + b.size}
	}
	e := func() segstruct {
	...
	mapping := func(f segfstruct, x segstruct) segstruct {
		...
		return segstruct{x.v + int(f), x.mc, x.size}
	}
	...
	for i := 0; i < n; i++ {
		...
		debug(f, i, t)
		ps[f] = append(ps[f], [2]int{i, t})
		ms[i] = append(ms[i], [2]int{i, t})
	}
	for i := 0; i < n; i++ {
		debug(ps[i])
		for _, v := range ps[i] {
			lst.applyrange(v[0], v[1], segfstruct(1))
		}
		t := lst.allprod()
		o += t.size - t.mc
		debug(t)
