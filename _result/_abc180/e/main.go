...
func main() {
	...
	getDist := func(i, j int) int {
		return abs(xs[i]-xs[j]) + abs(ys[i]-ys[j]) + max(0, zs[i]-zs[j])
	}
	for i := 0; i < n; i++ {
		for j := 0; j < n; j++ {
			c := getDist(i, j)
			edges[i][j] = edge{to: j, cost: c}
		}
	}
	...
	for i := 0; i < n; i++ {
		dp[1<<i][i] = edges[i][0].cost
	}
	for bit := 1; bit < nm; bit++ {
		for i := 0; i < n; i++ {
			if bit&(1<<i) == 0 {
				continue
			}
			...
			for j := 0; j < n; j++ {
				...
				if dp[bit2][j] != inf {
					dp[bit][i] = min(dp[bit][i], dp[bit2][j]+edges[i][j].cost)
				}
			}
		}
	}
	o = inf
	for i := 0; i < n; i++ {
		o = min(o, dp[nm-1][i]+edges[0][i].cost)
	}
