...
func main() {
	...
	for i := 0; i < 100; i++ {
		...
		if i != 99 {
			s = i
			t = i + 1
			edges[s] = append(edges[s], edge{to: t, cost: y})
			edges[t] = append(edges[t], edge{to: s, cost: y})
		}
		if i != 99 {
			s = i + 100
			t = i + 101
		...
		if i != 0 {
			s = i
			t = i + 99
			edges[s] = append(edges[s], edge{to: t, cost: x})
			edges[t] = append(edges[t], edge{to: s, cost: x})
		}
		s = i
		t = i + 100
		edges[s] = append(edges[s], edge{to: t, cost: x})
