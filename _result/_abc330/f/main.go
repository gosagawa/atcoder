...
func main() {
	...
	fb := func(c int) bool {
		f1 := func(c2 int) int {
			...
			ti := lowerBound(c2, xs)
			if ti != -1 {
				r += c2*(ti+1) - xss.get(ti)
			}
			ti = lowerBound(c2+c+1, xs)
			rm := 0
			if ti != -1 {
				rm = xss.get(ti)
			}
			r += xss.get(n-1) - rm - (c2+c)*(n-ti-1)
			return r
		}
		_, tk1 := bs3i(0, int(1e9), f1)

		f2 := func(c2 int) int {
			r := 0
			ti := lowerBound(c2, ys)
			if ti != -1 {
				r += c2*(ti+1) - yss.get(ti)
			}
			ti = lowerBound(c2+c+1, ys)
			rm := 0
			if ti != -1 {
				rm = yss.get(ti)
			}
			r += yss.get(n-1) - rm - (c2+c)*(n-ti-1)
			return r
		}
		_, tk2 := bs3i(0, int(1e9), f2)
		return tk1+tk2 > k
	}
	o = bs(0, int(1e10), fb) + 1
