...
func main() {
	...
	ccs := make(map[int]int)
	gcs := make(map[int]int)
	swap := func(ar *[36]int, f, t int) {
		ar[f], ar[t] = ar[t], ar[f]
	}
	getHash := func(ar *[36]int) int {
		sl := make([]int, 36)
		for i := 0; i < 36; i++ {
			sl[i] = ar[i]
		}
		rh := newrh(sl, mod)
		return rh.getAllHash()
	}

	checkHash := func(t, hash, c int) bool {
		if t == 0 {
			if _, ok := ccs[hash]; ok {
				return false
			}
	...
	var bfs func(st [36]int, t, p, c int)
	bfs = func(st [36]int, t, p, c int) {
		type state struct {
			st [36]int
			p  int
			c  int
		...
		q := list.New()
		hash := getHash(&st)
		if checkHash(t, hash, c) {
			q.PushBack(state{st, p, c})
		}
		e := q.Front()
		for e != nil {
			v := e.Value.(state)
			...
			if v.p-w >= 0 {
				swap(&v.st, v.p, v.p-w)
				hash := getHash(&v.st)
				if checkHash(t, hash, v.c+1) {
					q.PushBack(state{v.st, v.p - w, v.c + 1})
				}
				swap(&v.st, v.p-w, v.p)
			}
			if v.p+w < h*w {
			...
			if v.p%w != 0 {
				...
				hash := getHash(&v.st)
				if checkHash(t, hash, v.c+1) {
					q.PushBack(state{v.st, v.p - 1, v.c + 1})
				}
				swap(&v.st, v.p-1, v.p)
			}
			if (v.p+1)%w != 0 {
...
func (r *rh) initReverseHash() {
	...
	r.shr[0] = r.sh[r.sl]
	for i := 0; i < r.sl-1; i++ {
		r.shr[i+1] = calcmodrh(r.shr[i] + mulrh(r.si[i], (mod2611-r.t[r.sl-i-1])))
	}
}
