...
func main() {
	...
	for i := 0; i < e; i++ {
		...
		s--
		t--
		edges[s] = append(edges[s], edge{idx: idx, to: t})
		iedges[idx] = edge{from: s, to: t}
		idx++
	}
	...
	for {
		...
		for _, v := range edges[t] {
			...
			q = append(q, v.to)
			dist[v.to] = dist[t] + 1
			route[v.to] = append(route[t], v.idx)
		}
	}
	bdist := dist[v-1] - 1
	ue := make(map[int]bool)
	for _, v := range route[v-1] {
		ue[v] = true
	}

	bws := func(s, t, ng int) int {
		q := []int{}
		q = append(q, s)
		dist := make([]int, v)
		dist[s] = 1
