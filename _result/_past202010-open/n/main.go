...
func main() {
	...
	mp := convidxi2s(nsi2s(18), map[string]int{"0": 0, "1": 1, "?": 2})
	mp = append(mp, []int{0, 0, 0, 0, 0, 0})
	dp := i2s(pow2(6), pow2(6), 0)
	for i := 0; i < pow2(6); i++ {
		isValid := true
		for j := 0; j < 6; j++ {
			if hasbit(i, j) {
	...
	for k := 0; k < 18; k++ {
		...
		for i := 0; i < pow2(6); i++ {
			...
			for i2 := 0; i2 < pow2(6); i2++ {
				for j2 := 0; j2 < pow2(6); j2++ {
					if dp[i2][j2] == 0 {
						continue
					}
					if check(i2, j2, i) {
