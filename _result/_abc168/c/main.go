...
func main() {
	...
	hx := float64(a) * math.Cos((float64((15-h)%12*30)-float64(m)*30/60)*math.Pi/180)
	hy := float64(a) * math.Sin((float64((15-h)%12*30)-float64(m)*30/60)*math.Pi/180)
	mx := float64(b) * math.Cos(float64((75-m)%60*6)*math.Pi/180)
	my := float64(b) * math.Sin(float64((75-m)%60*6)*math.Pi/180)
	o = math.Sqrt((hx-mx)*(hx-mx) + (hy-my)*(hy-my))
