...
func main() {
	...
	ei4 := make(map[[2]int]int)
	dfs = func(v, p, d int) {
		ap[v] = append(ap[v], len(etd))
		etd = append(etd, d)
		etv = append(etv, v)
	...
	*/
	...
	sd := newstree(len(etd), stmin)
	si := newstree(len(etd), stadd)
	ei3 := make(map[[2]int]int)
	for i := 0; i < len(etd); i++ {
		sd.set(i, etd[i])
		if i != len(etd)-1 {
			si.set(i, ei[[2]int{etv[i], etv[i+1]}]*ei4[[2]int{etv[i], etv[i+1]}])
			ei3[[2]int{etv[i], etv[i+1]}] = i
		}
	}
	q := ni()
	for qi := 0; qi < q; qi++ {
		...
		case 1:
			...
			i--
			s, t := ei2[i][0], ei2[i][1]
			si.set(ei3[[2]int{s, t}], w*ei4[[2]int{s, t}])
			si.set(ei3[[2]int{t, s}], w*ei4[[2]int{t, s}])
		case 2:
			u, v := ni2()
