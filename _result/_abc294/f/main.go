...
func main() {
	...
	n, m, k := ni3()
	abs := make([][2]float64, n)
	cds := make([][2]float64, m)
	for i := 0; i < n; i++ {
		abs[i][0] = float64(ni())
		abs[i][1] = float64(ni())
	}
	...
	o := bsfl(0.0, 100.0, 100, func(c float64) bool {
		...
		tcds := make([]float64, m)
		for i := 0; i < m; i++ {
			tcds[i] = 100*cds[i][0] - c*(cds[i][0]+cds[i][1])
			tcds[i] /= c - 100
		}
		sort.Slice(tcds, func(i, j int) bool {
			return tcds[i] < tcds[j]
		})
		to := 0
		for i := 0; i < n; i++ {
			t := 100*abs[i][0] - c*(abs[i][0]+abs[i][1])
			t /= c - 100
			ti := bs(0, m-1, func(c2 int) bool {
				return t+tcds[c2] <= 0.0
			})
			to += ti + 1
		}
		return to >= k
