...
func main() {
	...
	for i := 0; i < m; i++ {
		...
		uf.unite(s, t)
		edges[s] = append(edges[s], edge{to: t, cost: i})
		edges[t] = append(edges[t], edge{to: s, cost: i})
		ec[s]++
		ec[t]++
	}
	sort.Slice(q, func(i, j int) bool {
		return ec[q[i]] < ec[q[j]]
	})
	...
	for len(q) != 0 {
		...
		var dfs func(s, i int) bool
		dfs = func(s, i int) bool {
			if ns[i] == s {
				return true
			}
			if used[i] {
			...
			for _, nv := range edges[i] {
				r := dfs(s, nv.to)
				if r {
					ns[nv.to], ns[i] = ns[i], ns[nv.to]
					rs = append(rs, itoa(nv.cost+1))
					return true
				}
			}
			return false
