...
func main() {
	...
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			...
			if (i+j)%2 == 0 {
				for k := 0; k < 4; k++ {
					pt := point{i + dx[k], j + dy[k]}
					if pt.isValid(h, w) && mp[pt.x][pt.y] != 1 {
						//out(i*w+j+1, pt.x*w+pt.y+1)
						setDualEdge(edges, i*w+j+1, pt.x*w+pt.y+1, 1)
					}
				}
				if mp[i][j] == 2 {
					setDualEdge(edges, sts, i*w+j+1, 1)
					setDualEdge(edges, st, gls, 1)
					tc++
				} else {
					setDualEdge(edges, st, i*w+j+1, 1)
				}
			} else {
				if mp[i][j] == 2 {
					setDualEdge(edges, sts, gl, 1)
					setDualEdge(edges, i*w+j+1, gls, 1)
					tc++
				} else {
					setDualEdge(edges, i*w+j+1, gl, 1)
				}
			}
		}
	}
	setDualEdge(edges, gl, st, inf)
	graph := newgraph(h*w+4, edges)
	o := graph.dinic()
	if o == tc {
		out("Yes")
		return
