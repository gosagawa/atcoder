...
func main() {
	...
	for i := 0; i < m; i++ {
		lines := []Line{}
		lines = append(lines, NewLineByTwoPoint(as[i], bs[i], as[i], bs[i]+1))
		lines = append(lines, NewLineByTwoPoint(as[i], bs[i], as[i]+1, bs[i]))
		lines = append(lines, NewLineByTwoPoint(as[i], bs[i], as[i]+1, bs[i]+1))
		lines = append(lines, NewLineByTwoPoint(as[i], bs[i], as[i]+1, bs[i]-1))

		d := 0
		t1 := bs[i] - as[i]
		t2 := n - 1 - bs[i] - as[i]
		ds := []int{n - 1, n - 1, n - abs(t1) - 1, n - abs(t2) - 1}
		bpt := point{as[i], bs[i]}
		for _, v := range ds {
			d += v
		}
		...
		for k := 0; k < 4; k++ {
			...
			for j := 0; j < i; j++ {
				...
				lines2 := []Line{}
				lines2 = append(lines2, NewLineByTwoPoint(as[j], bs[j], as[j], bs[j]+1))
				lines2 = append(lines2, NewLineByTwoPoint(as[j], bs[j], as[j]+1, bs[j]))
				lines2 = append(lines2, NewLineByTwoPoint(as[j], bs[j], as[j]+1, bs[j]+1))
				lines2 = append(lines2, NewLineByTwoPoint(as[j], bs[j], as[j]+1, bs[j]-1))
				for l := 0; l < 4; l++ {
					if isSame(lines[k], lines2[l]) {
						hasSame = true
						continue
					} else if isParallel(lines[k], lines2[l]) {
						continue
					}
					x, y, ok := crossPoint(lines[k], lines2[l])
					pt := point{x, y}
					if ok && pt.isValid(n, n) {
						if pt == bpt {
							hasbpt = true
						} else {
							mp[pt] = true
