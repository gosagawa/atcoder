...
func main() {
	...
	op := func(a segstruct, b segstruct) segstruct {
		return segstruct{
			size:  a.size + b.size,
			Asum:  madd(a.Asum, b.Asum),
			Bsum:  madd(a.Bsum, b.Bsum),
			ABsum: madd(a.ABsum, b.ABsum),
		}
	}
	e := func() segstruct {
		return segstruct{0, 0, 0, 0}
	}
	id := func() segfstruct {
		return segfstruct{inf, inf}
	}
	mapping := func(f segfstruct, x segstruct) segstruct {
		...
		return segstruct{
			size:  x.size,
			Asum:  madd(x.Asum, mmul(f.Aadd, x.size)),
			Bsum:  madd(x.Bsum, mmul(f.Badd, x.size)),
			ABsum: madd(madd(madd(x.ABsum, mmul(f.Aadd, x.Bsum)), mmul(f.Badd, x.Asum)), mmul(mmul(f.Aadd, f.Badd), x.size)),
		}
	}
	compostion := func(f segfstruct, g segfstruct) segfstruct {
		...
		return segfstruct{
			Aadd: madd(f.Aadd, g.Aadd),
			Badd: madd(f.Badd, g.Badd),
		}
	}
	...
	for qi := 0; qi < q; qi++ {
		...
		case 1:
			l, r, x := ni3()
			lst.applyrange(l-1, r, segfstruct{x, 0})
		case 2:
			l, r, x := ni3()
			lst.applyrange(l-1, r, segfstruct{0, x})
		case 3:
			l, r := ni2()
			out(lst.prod(l-1, r).ABsum)
		}
	}
