...
func main() {
	...
	edges := make([][]edge, n)
	edgesc := is(n, 0)
	get := func(i, l int) (int, int, int) {
		if debug {
			return as[i], bs[i], cs[i]
		}
		a := as[i] * (l + 1) % mod % 2
		b := bs[i] * (l + 1) % mod % n
		c := cs[i] * (l + 1) % mod % n
	...
	culc := func(u, v int) int {
		...
		for x := 0; x < 2; x++ {
			...
			for i := edgesc[u]; i < len(edges[u]); i++ {
				p := edges[u][i].to
				cs = append(cs, p)
				if ts[p] {
					return p
	...
	for i := 0; i < ct; i++ {
		...
		setps()
		dbg(ps)
		for j := i * sq; j < min((i+1)*sq, q); j++ {
			t, u, v := get(j, ans)
			dbg(t+1, u+1, v+1)
			switch t {
			case 0:
				uf.unite(u, v)
				edges[u] = append(edges[u], edge{to: v})
				edges[v] = append(edges[v], edge{to: u})
			case 1:
				ans = culc(u, v) + 1
				out(ans)
			}
