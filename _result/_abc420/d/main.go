...
func main() {
	...
	o := 0
	h, w := ni2()
	mp := convidxi2s(nsi2s(h), map[string]int{".": 0, "#": 1, "o": 2, "x": 3, "?": 4, "S": 5, "G": 6})
	mp2 := make([][][]int, 2)
	mp2[0] = i2s(h, w, 0)
	mp2[1] = i2s(h, w, 0)
	var st, gl point
	...
	for e != nil {
		...
		// Do something
		for k := 0; k < 4; k++ {
			np := point{t[1] + dx[k], t[2] + dy[k]}
			if np.isValid(h, w) && mp2[t[0]][np.x][np.y] != 1 {
				if mp2[t[0]][np.x][np.y] == 4 {
					if dist[t[0]^1][np.x][np.y] == inf {
						dist[t[0]^1][np.x][np.y] = dist[t[0]][t[1]][t[2]] + 1
						q.PushBack([3]int{t[0] ^ 1, np.x, np.y})
					}
				} else {
					if dist[t[0]][np.x][np.y] == inf {
						dist[t[0]][np.x][np.y] = dist[t[0]][t[1]][t[2]] + 1
						q.PushBack([3]int{t[0], np.x, np.y})
	...
	o = min(dist[0][gl.x][gl.y], dist[1][gl.x][gl.y])
	if o == inf {
		out(-1)
		return
	}
