...
func main() {
	...
	r := make([][2]int, n)
	var dfs func(pfi, pti, ifi, iti int)
	hasAns := true
	dfs = func(pfi, pti, ifi, iti int) {

		ci := iis[ps[pfi]-1]
		lc := ci - ifi
		rc := iti - ci
		//out(pfi, pti, ifi, iti, ci, lc, rc)
		if ci < ifi || iti < ci {
			hasAns = false
			return
		}
		...
		if lc == 0 {
			...
			r[ps[pfi]-1][0] = ps[pfi+1]
			dfs(pfi+1, pfi+lc, ifi, ci-1)
		}
		if rc == 0 {
			r[ps[pfi]-1][1] = 0
		} else {
			r[ps[pfi]-1][1] = ps[pfi+lc+1]
			dfs(pfi+lc+1, pti, ci+1, iti)
		}
	}
	dfs(0, n-1, 0, n-1)
	if !hasAns {
