...
func main() {
	...
	as, bs := ni2s(p)
	cs, ds := ni2s(q)
	base := issum(bs) + issum(ds)
	source := 0
	sink := p + q + 1
	m := NewMcfGraph(sink + 1)
	for i := 0; i < p; i++ {
		m.AddEdge(source, i+1, 1, 0)
	}
	for j := 0; j < q; j++ {
		m.AddEdge(p+j+1, sink, 1, 0)
	}
	for i := 0; i < p; i++ {
		for j := 0; j < q; j++ {
			if ss[i][j] == 0 {
			...
			ws := as[i] + cs[j] - bs[i] - ds[j]
			if ws <= 0 {
				continue
			}
			m.AddEdge(i+1, p+j+1, 1, inf-ws)
		}
	}

	result := m.Slope(source, sink, p)
	dbg(result)
	o = base
	for _, v := range result {
		maxs(&o, base+inf*v[0]-v[1])
	}
	out(o)
}
