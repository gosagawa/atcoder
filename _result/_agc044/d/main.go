...
func main() {
	...
	for qi := 0; qi < q; qi++ {
		...
		for !pq.IsEmpty() {
			...
			for i := -4; i <= 4; i++ {
				if v.y+i >= 0 && (v.y+i)%2 == 0 && abs(i) < 2 {
					co := v.x + a + abs(i)*d
					if mp[(v.y+i)/2] == 0 || mp[(v.y+i)/2] > co {
						mp[(v.y+i)/2] = co
						heap.Push(pq, pqst{x: co, y: (v.y + i) / 2})
					}
				}
				if v.y+i >= 0 && (v.y+i)%3 == 0 && abs(i) < 3 {
					co := v.x + b + abs(i)*d
					if mp[(v.y+i)/3] == 0 || mp[(v.y+i)/3] > co {
						mp[(v.y+i)/3] = co
						heap.Push(pq, pqst{x: co, y: (v.y + i) / 3})
					}
				}
				if v.y+i >= 0 && (v.y+i)%5 == 0 {
					co := v.x + c + abs(i)*d
					if mp[(v.y+i)/5] == 0 || mp[(v.y+i)/5] > co {
						mp[(v.y+i)/5] = co
						heap.Push(pq, pqst{x: co, y: (v.y + i) / 5})
					}
				}
			}
			if v.y <= (inf-v.x)/d {
				...
				if mp[0] == 0 || mp[0] > co {
					mp[0] = co
					heap.Push(pq, pqst{x: v.x + v.y*d, y: 0})
				}
			}
		}
		out(r)
