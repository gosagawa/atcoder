...
func solve() {
	...
	for i := 0; i < n; i++ {
		...
		if ls[i] > rs[i] {
			srs = append(srs, [2]int{ks[i], abs(rs[i] - ls[i])})
		} else {
			sls = append(sls, [2]int{n - ks[i], abs(rs[i] - ls[i])})
		}
	}
	sort2ar(sls, opt2ar(0, asc), opt2ar(1, desc))
	sort2ar(srs, opt2ar(0, asc), opt2ar(1, desc))
	sum := func(sl [][2]int) {
		ih := newIntHeap(asc)
		for _, v := range sl {
			ih.Push(v[1])
