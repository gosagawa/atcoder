...
func main() {
	...
	sx, sy := ni2()
	xs, ys := ni2s(n)
	dist := func(i, j, x, y int) float64 {
		return pointDist(point{i, j}, point{x, y})
	}
	o += dist(sx, sy, xs[0], ys[0])
	o += dist(sx, sy, xs[n-1], ys[n-1])
	for i := 0; i < n-1; i++ {
		o += dist(xs[i], ys[i], xs[i+1], ys[i+1])
	}
	debug(o)

	check := func(i int) float64 {
		t1 := dist(xs[i], ys[i], xs[i+1], ys[i+1])
		t2 := dist(xs[i], ys[i], sx, sy)
		t2 += dist(xs[i+1], ys[i+1], sx, sy)
		return t2 - t1
	}

	s := newstree(n+k, stmin)
	for i := 0; i < k; i++ {
		s.set(i, 0)
	}
	for i := 0; i < n-1; i++ {
		r1 := s.query(i, i+k)
		s.set(i+k, r1+check(i))
	}
	o += s.query(n-1, n+k)
...
func newstree(n int, minmax streeculctype) *stree {
	...
	case stmin:
		...
		s.cmp = func(i, j float64) float64 {
			return minf(i, j)
		}
	}
	return s
...
func (s *stree) add(i int, x float64) {
	i += s.n - 1
	s.b[i] += x
...
func (s *stree) set(i int, x float64) {
	i += s.n - 1
	s.b[i] = x
...
func (s *stree) query(a, b int) float64 {
	return s.querysub(a, b, 0, 0, s.n)
}

func (s *stree) querysub(a, b, k, l, r int) float64 {
	if r <= a || b <= l {
		return s.def
	}
	if a <= l && r <= b {
...
func (s *stree) debug() {
	...
	for i := 0; i < 2*s.n-1; i++ {
		...
		if s.b[i] == inf {
			...
			l = append(l, fmt.Sprintf("%f", s.b[i]))
		}
	}
	out(strings.Join(l, " "))
}
