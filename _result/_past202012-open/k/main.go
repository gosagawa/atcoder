...
func main() {
	...
	o := 0.0
	mp := convidxi2s(nsi2s(4), map[string]int{".": 0, "#": 1})
	gl := 0
	for i := 0; i < 4; i++ {
		for j := 0; j < 4; j++ {
			if mp[i][j] == 1 {
	...
	dp := make([]float64, pow2(16))
	for i := 0; i < pow2(16); i++ {
		dp[i] = float64(inf)
	}
	dp[0] = 0.0
	dx := []int{1, 0, -1, 0, 0, 1, -1, -1}
	dy := []int{0, 1, 0, -1, 0, -1, 1, -1}
	for l := 1; l < pow2(16); l++ {
		tmp := i2s(4, 4, 0)
		for j := 0; j < 16; j++ {
			if hasbit(l, j) {
		...
		for i := 0; i < 4; i++ {
			for j := 0; j < 4; j++ {
				...
				t := 1.0
				for k := 0; k < 5; k++ {
					np := point{i + dx[k], j + dy[k]}
					ti := np.x*4 + np.y
					if np.isValid(4, 4) && hasbit(l, ti) && mp[np.x][np.y] == 1 && dp[l-pow2(ti)] != float64(inf) {
						t += dp[l-pow2(ti)] / 5.0
					} else {
						stay++
					}
				...
				if l == 17 {
					dbg(i, j, stay, 5.0/float64(5-stay))
				}
				minsf(&dp[l], t*(5.0/float64(5-stay)))
			}
		}
	}
	o = dp[gl]
