...
func main() {
	...
	for i := 0; i < n-1; i++ {
		l, r := ni2()
		evs = append(evs, [4]int{l, 0, i, 0})
		evs = append(evs, [4]int{r, 2, i, 0})
		s.Increment(i)
	}
	s.Increment(-1)
	s.Increment(n - 1)
	for i := 0; i < q; i++ {
		a, b := ni2()
		b--
		evs = append(evs, [4]int{a, 1, i, b})
	}
	sort2ar(evs, opt2ar(0, asc), opt2ar(1, asc))
	rs := is(q, 0)
	for i := 0; i < len(evs); i++ {
		t, j, b := evs[i][1], evs[i][2], evs[i][3]
		switch t {
		case 0:
			s.Decrement(j)
		case 1:
			from, _ := s.LowerBound(b)
			to, _ := s.UpperBound(b - 1)
			//			dbg(j, b, from, to)
			rs[j] = to - from
		case 2:
			s.Increment(j)
