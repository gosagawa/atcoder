...
func main() {
	...
	for i := 0; i < n; i++ {
		s := ns()
		sts[i].val = s
		sts[i].valtrimmed = strings.TrimLeft(s, "0")
		sts[i].tl = len(s)
		sts[i].vl = len(sts[i].valtrimmed)
		sts[i].zerol = sts[i].tl - sts[i].vl
	}
	sort.Slice(sts, func(i, j int) bool {
		if sts[i].vl != sts[j].vl {
			return sts[i].vl < sts[j].vl
		}
		if sts[i].valtrimmed != sts[j].valtrimmed {
			return sts[i].valtrimmed < sts[j].valtrimmed
		}
		return sts[i].zerol > sts[j].zerol
	})
	dbg(sts)
	for _, v := range sts {
		out(v.val)
