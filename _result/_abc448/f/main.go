...
func main() {
	...
	mx := int(1e7) * 2
	block := mx / sqrt(n)
	pts := make([][][3]int, mx/block+1)
	for i := 0; i < n; i++ {
		pts[ys[i]/block] = append(pts[ys[i]/block], [3]int{xs[i], ys[i], i})
	}
	for i := 0; i < mx/block+1; i++ {
		if i%2 == 0 {
			sort2ar(pts[i], opt2ar(0, asc), opt2ar(1, asc))
		} else {
			sort2ar(pts[i], opt2ar(0, desc), opt2ar(1, asc))
		}
	}
	d := func(x1, y1, x2, y2 int) int {
	...
	for i := 0; i < mx/block+1; i++ {
		for _, v := range pts[i] {
			...
			o += d(last[0], last[1], v[0], v[1])
		}
	}
	dbg(i1)
	o += d(last[0], last[1], first[0], first[1])
	rs := is(n, 0)
	for i := 0; i < n; i++ {
		rs[i] = trs[(i+i1-1)%n]
	}
