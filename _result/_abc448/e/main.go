...
func main() {
	...
	maxlogn := 60
	next := make([]int, maxlogn+1)
	next[0] = 1
	m2 := m * 10007
	for i := 0; i < maxlogn; i++ {
		next[i+1] = next[i]*mpow(10, pow2(i), m2) + next[i]
		next[i+1] %= m2
	}
	dbg(next[:10])
	...
	rep := func(l int) int {
		...
		for i := 0; i < maxlogn; i++ {
			...
			t = t*mpow(10, pow2(i), m2) + next[i]
			t %= m2
		}
		return t
	}
	dbg(rep(1), rep(2), rep(3), rep(4), rep(5), rep(6), rep(7), rep(8), rep(9), rep(10))
