...
func main() {
	...
	bit3 := newbit(n + 1)
	for i := 0; i < n; i++ {
		bit1.add(i+1, mmul(mmul(i+1, i+1), ns[i]))
		bit2.add(i+1, mmul(i+1, ns[i]))
		bit3.add(i+1, ns[i])
	}
	for i := 0; i < q; i++ {
		t := ni()
		...
		case 1:
			x, v := ni2()
			bit1.add(x, mmul(mmul(x, x), (v-ns[x-1])))
			bit2.add(x, mmul(x, (v-ns[x-1])))
			bit3.add(x, madd(v, -ns[x-1]))
			ns[x-1] = v
		case 2:
			x := ni()
			o := 0
			o = madd(o, mdiv(bit1.sum(x), 2))
			o = madd(o, -mdiv(mmul(bit2.sum(x), (x*2+3)%mod), 2))
			o = madd(o, mdiv(mmul(bit3.sum(x), mmul(x+1, x+2)), 2))
			out(o)
		}
	}
}
