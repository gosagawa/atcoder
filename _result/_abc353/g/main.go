...
func main() {
	...
	for i := 0; i < n; i++ {
		base1[i] = segstruct{-inf, i}
		base2[i] = segstruct{-inf, i}
	}
	base1[0] = segstruct{0, 0}
	base2[0] = segstruct{-c * n, 0}
	...
	for i := 0; i < m; i++ {
		ts[i]--
		v1 := lst1.prod(0, ts[i]+1)
		v2 := lst2.prod(ts[i], n)
		debug(v1, v2)
		t1 := dp[v1.index] - abs(v1.index-ts[i])*c + ps[i]
		t2 := dp[v2.index] - abs(v2.index-ts[i])*c + ps[i]
		debug(t1, t2)
		dp[ts[i]] = max(t1, t2)
		debug(dp)
		lst1.set(ts[i], segstruct{dp[ts[i]] + ts[i]*c, ts[i]})
		lst2.set(ts[i], segstruct{dp[ts[i]] + (n-ts[i])*c, ts[i]})
	}
	for i := 0; i < n; i++ {
		maxs(&o, dp[i])
	}
