...
func main() {
	...
	ty += k
	if k == 1 {
		o = abs(sx-tx) + abs(sy-ty)
		out(o)
		return
	}
	if [2]int{sx / k, sy / k} == [2]int{tx / k, ty / k} {
		o = abs(sx-tx) + abs(sy-ty)
	}
	iss := func(x, y int) bool {
		t := x/k + y/k
		return t%2 == 0
	}
	ds := [4][2]int{{1, 0}, {0, 1}, {-1, 0}, {0, -1}}
	if !iss(sx, sy) {
		ds = [4][2]int{{0, 0}, {0, 0}, {0, 0}, {0, 0}}
	}
	dt := [4][2]int{{1, 0}, {0, 1}, {-1, 0}, {0, -1}}
	if !iss(tx, ty) {
		dt = [4][2]int{{0, 0}, {0, 0}, {0, 0}, {0, 0}}
	}
	coststob := func(x, y int, mv [2]int) int {
		switch mv {
		case [2]int{1, 0}:
			return k - x%k
		case [2]int{-1, 0}:
	...
	for i := 0; i < 4; i++ {
		for j := 0; j < 4; j++ {
			t1 := coststob(sx, sy, ds[i])
			t2 := coststob(tx, ty, dt[j])
			nsx := (sx + ds[i][0]*k) / k
			nsy := (sy + ds[i][1]*k) / k
			ntx := (tx + dt[j][0]*k) / k
			nty := (ty + dt[j][1]*k) / k
			...
			if k == 2 {
				...
				d := max(abs(nsx-ntx), abs(nsy-nty))
				t3 = d * 2
			}
			mins(&o, t1+t2+t3)
			debug(i, j, t1, t2, t3, ds[i], ds[j])
		}
	}
