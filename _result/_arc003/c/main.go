...
func main() {
	...
	cm := make([][]float64, 10)
	for i := 1; i < 10; i++ {
		cm[i] = append(cm[i], float64(i))
		for j := 1; j < n*m; j++ {
			t := float64(i) * math.Pow(0.99, float64(j))
			if t > math.Pow(0.1, 11) {
				cm[i] = append(cm[i], t)
			} else {
				break
			}
	...
	for e != nil {
		...
		for _, d := range dr {
			np := pointAdd(p, d)
			if !np.isValid(n, m) || mp[np.x][np.y] == -1 || dist[np.x][np.y] != -1 {
				continue
			}
	...
	o = bsfl(0.0, 9.0, 100, func(c float64) bool {
		...
		for e != nil {
			...
			for _, d := range dr {
				...
				if (np.x != g[0] || np.y != g[1]) && c > culc(mp[np.x][np.y], dist[p.x][p.y]+1) {
					continue
				}
				dist[np.x][np.y] = dist[p.x][p.y] + 1
				q.PushBack(np)
