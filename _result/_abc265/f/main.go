...
func main() {
	...
	for i := 0; i < n; i++ {
		var dp2 [1001][1001]int
		var dp3 [1001][1001]int
		var ndp [1001][1001]int
		for j := 0; j <= d; j++ {
			for k := 0; k <= d; k++ {
				dp2[j][k] = madd(dp2[j][k], dp[j][k])
				dp3[j][k] = madd(dp3[j][k], dp[j][k])
				if j > 0 && k > 0 {
					dp2[j][k] = madd(dp2[j][k], dp2[j-1][k-1])
				}
				if j > 0 && k < d {
					dp3[j][k] = madd(dp3[j][k], dp3[j-1][k+1])
				}
			}
		}
		...
		for j := 0; j <= d; j++ {
			for k := 0; k <= d; k++ {
				...
				p2y := k - pq
				if p1x > 0 && p1y > 0 {
					ndp[j][k] = madd(ndp[j][k], dp2[p1x-1][p1y-1])
				}
				if p2x > 0 && p2y > 0 {
					ndp[j][k] = madd(ndp[j][k], dp2[p2x-1][p2y-1])
				}
				if p2y < 0 {
					p2x += p2y
					p2y = 0
				...
				ndp[j][k] = madd(ndp[j][k], dp3[p2x][p2y])
				if p1x != 0 && p1y != d {
					ndp[j][k] = madd(ndp[j][k], -dp3[p1x-1][p1y+1])
				}
			}
		}
		if i == -1 {
