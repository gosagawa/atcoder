...
func main() {
	...
	for i := 0; i < n; i++ {
		for {
			...
			isValid := true
			lkey, lhas := s.LowerBound(as[t] + 1)
			//			dbg("lkey,lhas", lkey, lhas)
			if lhas && abs(lkey-as[t]) < d {
				isValid = false
			}
			rkey, rhas := s.UpperBound(as[t] - 1)
			if rhas && abs(rkey-as[t]) < d {
				isValid = false
			}
			//			dbg("rkey,rhas", rkey, rhas)
			if isValid {
				s.Insert(as[t], 1)
				t++
			} else {
