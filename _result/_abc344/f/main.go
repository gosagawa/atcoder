...
func main() {
	...
	dp := make([][]map[int][2]int, n)
	for i := 0; i < n; i++ {
		dp[i] = make([]map[int][2]int, n)
		for j := 0; j < n; j++ {
			dp[i][j] = make(map[int][2]int)
		}
	}

	set := func(i, j, mx int, nv [2]int) {
		if v, ok := dp[i][j][mx]; ok {
			if nv[0] < v[0] || (nv[0] == v[0] && nv[1] > v[1]) {
				dp[i][j][mx] = nv
			}
		} else {
			dp[i][j][mx] = nv
	...
	dp[0][0][ps[0][0]] = [2]int{0, 0}
	for i := 0; i < n; i++ {
		for j := 0; j < n; j++ {
			if j != n-1 {
				r := rs[i][j]
				for mx, v := range dp[i][j] {
					t := max((r-v[1]+mx-1)/mx, 0)
					set(i, j+1, max(mx, ps[i][j+1]), [2]int{v[0] + t, v[1] + mx*t - r})
				}
			}
			if i != n-1 {
				d := ds[i][j]
				for mx, v := range dp[i][j] {
					t := max((d-v[1]+mx-1)/mx, 0)
					set(i+1, j, max(mx, ps[i+1][j]), [2]int{v[0] + t, v[1] + mx*t - d})
				}
			}
		}
	}
	for _, v := range dp[n-1][n-1] {
		mins(&o, v[0]+(n-1)*2)
	}
