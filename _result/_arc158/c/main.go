...
func main() {
	...
	n := ni()
	ns := nis(n)
	convert := func(n, i int) (int, int) {
		return (n / pow10(i)) % 10, n % pow10(i)
	}
	...
	for i := 0; i < 16; i++ {
		ts1[i] = make([][2]int, n)
		for j := 0; j < n; j++ {
			ts1[i][j][0], ts1[i][j][1] = convert(ns[j], i)
		}
		sort2ar(ts1[i], opt2ar(1, asc))
	}
	ts3 := is(20, 0)
	for i := 0; i < 20; i++ {
		ts3[i] = i % 10
	...
	for i := 0; i < 16; i++ {
		...
		tc := make([]int, 10)
		for j := 0; j < n; j++ {
			ts[ts1[i][j][0]] = append(ts[ts1[i][j][0]], ts1[i][j][1])
		}
		for k := 0; k < 10; k++ {
			tc[k] = len(ts[k])
			if tc[k] == 0 {
			...
			for j := 0; j < n; j++ {
				for {
					...
					if ts[k][tc[k]-1-ti] >= pow10(i)-ts1[i][j][1] {
						ti++
						if ti == tc[k] {
							break
						}
				...
				o += (tc[k] - ti) * ts3[ts1[i][j][0]+k]
				o += ti * ts3[ts1[i][j][0]+k+1]
				//				out(k, j, ti, o)
			}
		}
	}
