...
func main() {
	...
	if len(ms) < 6 {
		...
		ms2 = append(ms2, ms[:3]...)
		ms2 = append(ms2, ms[len(ms)-3:]...)
	}
	if len(ps) < 6 {
		ps2 = append(ps2, ps...)
	} else {
		ps2 = append(ps2, ps[:3]...)
		ps2 = append(ps2, ps[len(ps)-3:]...)
	}
	allsearch := func(sl []int) {
		if len(sl) < 3 {
			return
		}
		for i := 0; i < len(sl); i++ {
			for j := i + 1; j < len(sl); j++ {
				for k := j + 1; k < len(sl); k++ {
					t := float64(sl[i]+sl[j]+sl[k]) / float64(sl[i]) / float64(sl[j]) / float64(sl[k])
					if t < omi {
						omi = t
					}
					if t > omx {
