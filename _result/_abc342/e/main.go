...
func main() {
	...
	for !pq.IsEmpty() {
		...
		for _, ni := range es[v.x] {
			...
			ll := 0
			if ls[ni]+(ds[ni]*ks[ni])+cs[ni] < v.y {
				ll = ls[ni] + (ds[ni] * ks[ni])
			} else {
				t := (v.y - cs[ni] - ls[ni]) / ds[ni]
				ll = ls[ni] + (ds[ni] * t)
			}
			heap.Push(pq, pqst{x: as[ni], y: ll})
		}
	}
	for _, v := range dp {
		if v == inf {
