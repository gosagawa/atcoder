...
func main() {
	...
	epq := make([]*eraseableHeap, n*2)
	for i := 0; i < n*2; i++ {
		epq[i] = newEraseableHeap(desc)
	}
	for i := 0; i < n; i++ {
		epq[i+n].push(as[i])
	}

	rangeUpdate := func(l, r, t, x int) {
		l += n
		r += n
		for l < r {
			if l&1 == 1 {
	...
	for qi := 0; qi < q; qi++ {
		...
		case 1:
			...
			l--
			rangeUpdate(l, r, t, x)
			history = append(history, [3]int{l, r, x})
		case 2:
			c := ni()
			c--
			l, r, x := history[c][0], history[c][1], history[c][2]
			rangeUpdate(l, r, t, x)
			history = append(history, [3]int{})
		case 3:
			x := ni()
			x--
			out(pointAccess(x))
...
type eraseableHeap struct {
	ih *IntHeap
	dl *IntHeap
}

func newEraseableHeap(order sortOrder) *eraseableHeap {
	return &eraseableHeap{
		ih: newIntHeap(order),
		dl: newIntHeap(order),
	}
}

func (h *eraseableHeap) push(v int) {
	h.ih.Push(v)
}

func (h *eraseableHeap) erase(v int) {
	h.dl.Push(v)
}

func (h *eraseableHeap) top() int {
	for h.dl.Len() > 0 && h.ih.Top() == h.dl.Top() {
		h.ih.Pop()
		h.dl.Pop()
	}
	if h.ih.Len() == 0 {
