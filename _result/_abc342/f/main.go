...
func main() {
	...
	for i := 0; i <= n; i++ {
		...
		if i < l {
			ds[i+1] += ys[i] / d
			ds[min(i+id+1, n+1)] -= ys[i] / d
			if i+id > n {
				ys[n+1] += ys[i] / d * float64(i+id-n)
			}
			ys[i] = 0
		}
	}
	...
	debug(ys)
	xws := make([]float64, n+1)
	for i := 0; i <= n; i++ {
		if i == 0 {
			xws[i] = ys[n+1]
			continue
		}
		xws[i] = yc.get(i-1) + ys[n+1]
	}
	debug(xws)
	xs := make([]float64, n+1)
	rws := make([]float64, n+1)
	for i := n; i >= 0; i-- {
		mw := 0.0
		ri := n - i
		if i != n {
			mw = rws[ri] - rws[max(ri-id, 0)]
		}
		wr := mw / d
		if wr < xws[i] {
			wr = xws[i]
...
func newcusum(sl []float64) *cusum {
	c := &cusum{}
	c.l = len(sl)
	c.s = make([]float64, len(sl)+1)
	for i, v := range sl {
		c.s[i+1] = c.s[i] + v
	}
	return c
...
// get sum f <= i && i <= t
func (c *cusum) getRange(f, t int) float64 {
	if f > t || f >= c.l {
		return 0
	}
...
// get sum 0 to i
func (c *cusum) get(i int) float64 {
	return c.s[i+1]
}
