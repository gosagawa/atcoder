...
func main() {
	...
	xs, ys := ni2s(n)
	ps := make([][][2]int, 2)
	ps[0] = make([][2]int, limHalf)
	ps[1] = make([][2]int, limHalf)
	psl := make([]int, 2)
	var dfs func(idx, end, i, x, y int)
	dfs = func(idx, end, i, x, y int) {
		if i == end {
			ti := psl[idx]
			psl[idx]++
			ps[idx][ti] = [2]int{x, y}
	...
	sort.Slice(ps[0], func(i, j int) bool {
		return ps[0][i][0] < ps[0][j][0]
	})
	sort.Slice(ps[1], func(i, j int) bool {
		return ps[1][i][0] < ps[1][j][0]
	})
	/*
		out(ps[0])
		out("")
	...
	*/
	...
	for i := 0; i < psl[0]; i++ {
		for {
			if kb < psl[1] && ps[1][kb][0] <= ps[0][i][0]+d {
				m.Insert(ps[1][kb][1], 1)
				kb++
			} else {
				break
			}
		...
		for {
			if ka < psl[1] && ps[1][ka][0] < ps[0][i][0]-d {
				m.Delete(ps[1][ka][1])
				ka++
			} else {
				break
