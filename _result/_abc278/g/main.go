...
func main() {
	...
	cutst := func(st [][2]int, a int) [][2]int {
		ci := 0
		for i, v := range st {
			if v[0] <= a && a <= v[1] {
				ci = i
				if a != v[0] {
					st = append(st, [2]int{v[0], a - 1})
				}
				if a+l-1 != v[1] {
					st = append(st, [2]int{a + l, v[1]})
				}
				break
			}
		}
...
func delIdx(pos int, sl [][2]int) [][2]int {
