...
func main() {
	...
	mp1 := i2s(h-h1+1, w-w1+1, 0)
	mp2 := i2s(h-h2+1, w-w2+1, 0)
	for i := 0; i < h-h1+1; i++ {
		for j := 0; j < w-w1+1; j++ {
			mp1[i][j] = cusum2d.get(i, j, i+h1-1, j+w1-1)
		}
	}
	for i := 0; i < h-h2+1; i++ {
		for j := 0; j < w-w2+1; j++ {
			mp2[i][j] = cusum2d.get(i, j, i+h2-1, j+w2-1)
		}
	}
	dbg(mp1)
	dbg(mp2)
	mp3 := i2s(h-h2+1, w-w1+1, 0)
	for i := 0; i < h-h2+1; i++ {
		s := NewRBMAP()
		for j := 0; j < w1-w2+1; j++ {
			s.Increment(mp2[i][j])
		...
		for j := 1; j < w-w1+1; j++ {
			s.Decrement(mp2[i][j-1])
			s.Increment(mp2[i][j+w1-w2])
			mp3[i][j] = s.Max()
		}
	}
	dbg(mp3)
	mp4 := i2s(h-h1+1, w-w1+1, 0)
	for j := 0; j < w-w1+1; j++ {
		s := NewRBMAP()
		for i := 0; i < h1-h2+1; i++ {
			s.Increment(mp3[i][j])
		...
		for i := 1; i < h-h1+1; i++ {
			s.Decrement(mp3[i-1][j])
			s.Increment(mp3[i+h1-h2][j])
			mp4[i][j] = s.Max()
		}
	}
	for i := 0; i < h-h1+1; i++ {
		for j := 0; j < w-w1+1; j++ {
			maxs(&o, mp1[i][j]-mp4[i][j])
		}
	}
	dbg(mp4)
