...
func main() {
	...
	mpc := make(map[[2]int]int)
	for i := 0; i < n; i++ {
		t := gcd(abs(xs[i]), abs(ys[i]))
		xs[i] /= t
		ys[i] /= t
		mp[[2]int{xs[i], ys[i]}] = i
		mpc[[2]int{xs[i], ys[i]}]++
	}
	sl := make([]point, len(mp))
	slc := make([]int, len(mp))
	i := 0
	for v := range mp {
		sl[i] = point{x: v[0], y: v[1]}
		i++
	}
	sort.Slice(sl, func(i, j int) bool {
		return sl[i].x*sl[j].y < sl[i].y*sl[j].x
	})
	mpi := make(map[point]int)
	for i, v := range sl {
		slc[i] = mpc[[2]int{v.x, v.y}]
		mpi[v] = i
	}
	sl = append(sl, sl...)
	slc = append(slc, slc...)
	...
	for qi := 0; qi < q; qi++ {
		...
		a--
		b--
		ai := mpi[point{x: xs[a], y: ys[a]}]
		bi := mpi[point{x: xs[b], y: ys[b]}]
		if ai > bi {
			bi += len(mpc)
		}
		o := bt.get(ai, bi+1)
