...
func main() {
	...
	for i := 0; i < m; i++ {
		...
		a--
		b--
		mint := int(math.Sqrt(float64(d))+0.5) - 1
		edges[a] = append(edges[a], edge{to: b, c: c, d: d, mint: mint})
		edges[b] = append(edges[b], edge{to: a, c: c, d: d, mint: mint})
	}

	graph := newgraph(n, edges)
	graph.setStart(state{node: 0})
...
func (e edge) getCost(t int) int {
	if t < e.mint {
		return e.mint - t + e.c + int(e.d/(e.mint+1))
	}
	return e.c + int(e.d/(t+1))
}
...
func (g *graph) dijkstra() []int {
	...
	for !que.IsEmpty() {
		...
		for _, edge := range g.edges[st.node] {
			newScore := st.score + edge.getCost(st.score)
