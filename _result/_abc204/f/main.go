...
func main() {
	...
	check := func(st [6]int) {
		...
		mp1[st] = t1
		mp2[t2] = append(mp2[t2], st)
	}
	var dfs func(c int, st [6]int)
	dfs = func(c int, st [6]int) {
		for i := 1; i <= 5; i++ {
			st[c] = i
			if c == h-1 {
				check(st)
	...
	me := newMatrixExponentation(ph, mv)
	r := slmatrixmul(base, me.getpow(w))
	o = r[0]
