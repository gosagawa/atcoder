...
func main() {
	...
	for i := 0; i < q; i++ {
		...
		tli := -1
		if len(rs[t]) > 0 {
			tli = upperBound(s-1, rs[t])
			if tli < len(rs[t]) {
				tl = rs[t][tli]
			}
		}
		dbg(s, t, ls[s], tr, tri, rs[t], tl, tli)
		if tr == t {
			hasV := false
			if tri >= 1 || tli < len(rs[t])-1 {
				hasV = true
			}
			streel := upperBound(s, tls)
			streer := lowerBound(t+1, tls)
			if streel < m {
				r := st.query(streel, streer+1)
				dbg(streel, streer, r)
				if r <= t {
					hasV = true
				}
