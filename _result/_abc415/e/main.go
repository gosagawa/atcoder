...
func main() {
	...
	fn := func(c int) bool {
		...
		for e != nil {
			t := e.Value.(point)
			if checked[t.x][t.y] == 1 {
				e = e.Next()
				continue
			}
			checked[t.x][t.y] = 1
			if dp[t.x][t.y] == -inf || dp[t.x][t.y]+mp[t.x][t.y] < ps[t.x+t.y] {
				e = e.Next()
				continue
			}
			if t.x+1 < h {
				maxs(&dp[t.x+1][t.y], dp[t.x][t.y]+mp[t.x][t.y]-ps[t.x+t.y])
				q.PushBack(point{t.x + 1, t.y})
			}
			if t.y+1 < w {
				maxs(&dp[t.x][t.y+1], dp[t.x][t.y]+mp[t.x][t.y]-ps[t.x+t.y])
				q.PushBack(point{t.x, t.y + 1})
			}

			e = e.Next()
		}
		return dp[h-1][w-1] == -inf || dp[h-1][w-1]+mp[h-1][w-1] < ps[h+w-2]
	}
	o = bs(-1, int(1e16), fn)
	o++
