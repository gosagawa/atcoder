...
func main() {
	...
	qs := is(n+1, 0)
	for i := 0; i < n; i++ {
		qs[i] = mdiv(1, mmul((n-i), cf.combination(n, i)))
	}
	for k, tr := range trs {
		sorti(tr)
		pre := is(len(tr)+1, 0)
		for i := 0; i < len(tr); i++ {
			pre[i+1] = madd(pre[i], tr[i])
		}
		for i, tl := range tls {
			for _, sl := range tl {

				c := lowerBound(x-sl, tr)
				c++
				p := madd(mmul(c, madd(asum, -sl)), -pre[c])
				o += mmul(qs[i+k], p)
				o %= mod
			}
