...
func solve() {
	o := 0
	n := ni()
	ss := convidxis(nsis(), map[string]int{"S": 0, "R": 1})
	xs := nis(n)
	ys := nis(n - 1)
	dp := i2s(2, n, -inf)
	if ss[0] == 0 {
	...
	for i := 1; i < n; i++ {
		if ss[i] == 0 {
			maxs(&dp[0][i], dp[0][i-1])
			maxs(&dp[0][i], dp[1][i-1]+ys[i-1])
			maxs(&dp[1][i], dp[0][i-1]-xs[i])
			maxs(&dp[1][i], dp[1][i-1]-xs[i])
		} else {
			maxs(&dp[0][i], dp[0][i-1]-xs[i])
			maxs(&dp[0][i], dp[1][i-1]-xs[i]+ys[i-1])
			maxs(&dp[1][i], dp[0][i-1])
			maxs(&dp[1][i], dp[1][i-1])
		}
	}
	o = max(dp[0][n-1], dp[1][n-1])
