...
func main() {
	...
	n, m := ni2()
	ps := nis(n)
	buyableMax := func(p, mx int) int {
		return bs(0, int(1e9), func(c int) bool {
			return safemul((c-1)*2+1, p) <= mx
		})
	}
	...
	fn := func(c int) bool {
		ts := 0
		for i := 0; i < n; i++ {
			ct := buyableMax(ps[i], c)
			//			dbg(i, ct, ts)
			sum := safemul(ct*ct, ps[i])
			ts = safeadd(ts, sum)
			if ts > m {
				return false
			}
	...
	buym := bs(0, int(1e18), fn)
	ts := 0
	for i := 0; i < n; i++ {
		ct := buyableMax(ps[i], buym)
		o += ct
		ts += ct * ct * ps[i]
		dbg(i, ct, ct*ct*ps[i])
	}
