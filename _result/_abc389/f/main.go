...
func main() {
	...
	base := make([]segstruct, q)
	for i := 0; i < q; i++ {
		base[i] = segstruct(qis[i][0])
	}
	...
	for i := 0; i < n; i++ {
		f := lst.maxleft(q, func(v segstruct) bool {
			return int(v) >= ls[i]
		})
		t := lst.maxleft(q, func(v segstruct) bool {
			return int(v) > rs[i]
		})
		dbg(f, t, ls[i], rs[i])
		dbg(lst.d)
		lst.applyrange(f, t, 1)
