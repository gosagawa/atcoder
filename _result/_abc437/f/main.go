...
func main() {
	...
	n, q := ni2()
	xs, ys := ni2s(n)
	xmaxs := newstree(n, stmax)
	xmins := newstree(n, stmin)
	ymaxs := newstree(n, stmax)
	ymins := newstree(n, stmin)
	for i := 0; i < n; i++ {
		xmaxs.set(i, xs[i]+ys[i])
		xmins.set(i, xs[i]+ys[i])
		ymaxs.set(i, -xs[i]+ys[i])
		ymins.set(i, -xs[i]+ys[i])
	...
	for i := 0; i < q; i++ {
		...
		case 2:
			...
			nx := x + y
			ny := -x + y
			xmax := xmaxs.query(l, r)
			xmin := xmins.query(l, r)
			ymax := ymaxs.query(l, r)
			ymin := ymins.query(l, r)
			d1 := abs(xmax - nx)
			d2 := abs(xmin - nx)
			d3 := abs(ymax - ny)
			d4 := abs(ymin - ny)
			t := max(max(d1, d2), max(d3, d4))
			dbg(nx, ny, "-", xmin, xmax, ymin, ymax, "-", d1, d2, d3, d4, t)
			out(t)
		}
