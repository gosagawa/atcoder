...
func main() {
	...
	sts := []st{}
	sts = append(sts, st{})
	sts[0].mp = make(map[int]int)
	for i := 0; i < n; i++ {
		x, y := xs[i], ys[i]
		xi := xis[x]
		if v, ok := sts[xi].mp[y]; ok {

			sts[v].idxs = append(sts[v].idxs, i+1)
			xis[i+1] = v
		} else {
			sts = append(sts, st{})
			sts[len(sts)-1].mp = make(map[int]int)

			sts[len(sts)-1].idxs = append(sts[len(sts)-1].idxs, i+1)
			sts[xi].mp[y] = len(sts) - 1
			xis[i+1] = len(sts) - 1
		}
	}
	dbg(xis)
	...
	dfs = func(v int) {
		sorti(sts[v].idxs)
		rs = append(rs, sts[v].idxs...)
		ts := is(len(sts[v].mp), 0)
		j := 0
		for i := range sts[v].mp {
			ts[j] = i
			j++
		}
		sorti(ts)
