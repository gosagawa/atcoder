...
func solve() {
	n := ni()
	mp := convidxis(nsis(), map[string]int{"G": 0, "B": 1, "R": 2})
	tedges := make([][]edge, n)
	us, vs := ni2s(n - 1)
	for i := 0; i < n-1; i++ {
		us[i]--
		vs[i]--
		s, t := us[i], vs[i]
		tedges[s] = append(tedges[s], edge{to: t})
		tedges[t] = append(tedges[t], edge{to: s})
	}
	divide := is(n, 0)
	var dfs func(v, p int)
	dfs = func(v, p int) {
		for _, nv := range tedges[v] {
			if nv.to == p {
				continue
			}
			divide[nv.to] = divide[v] ^ 1
			dfs(nv.to, v)
		}
	}
	dfs(0, -1)
	...
	dbg(graph.edges)
	mp2 := make(map[[2]int]bool)
	for i, es := range graph.edges {
		for _, v := range es {
			if i > source && i < sink && v.to > source && v.to < sink {
				if divide[(i-1)/3] == 1 || divide[(v.to-1)/3] == 0 || v.cost != 0 {
					continue
				}
				mp2[[2]int{i, v.to}] = true
			}
		}
	}
	dbg(mp2)
