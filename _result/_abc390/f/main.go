...
func main() {
	...
	for i := 0; i < n; i++ {
		ais[as[i]-1] = append(ais[as[i]-1], i)
		aids[as[i]-1] = append(aids[as[i]-1], i)
		if as[i] != 1 {
			aids[as[i]-2] = append(aids[as[i]-2], i)
		}
	}
	for i := 0; i < n; i++ {
		ais[i] = append(ais[i], n)
		aids[i] = append(aids[i], n)
	}
	ais = append(ais, []int{-1, n})
	slsum := func(sl []int) int {
		t := 0
