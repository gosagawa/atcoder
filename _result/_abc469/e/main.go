...
func main() {
	...
	o := 0.0
	n, k := ni2()
	mp := convidxis(nsis(), map[string]int{"x": 0, "o": 1})
	ts := is(n+1, 0)
	for i := 0; i < n; i++ {
		ts[i+1] = ts[i] + mp[i]
	}
	ts2 := is(n+1, 0)
	for i := 0; i < n+1; i++ {
		ts2[i] = lowerBound(ts[i]-k+1, ts)
	}
	ts3 := make([]float64, n)
	ts4 := make([]float64, n+1)
	ts5 := make([]float64, n+1)
	f := func(c float64) bool {
		for i := 0; i < n; i++ {
			ts3[i] = float64(mp[i]) - c
		}
		for i := 0; i < n; i++ {
			ts4[i+1] = ts4[i] + ts3[i]
		}
		for i := 0; i < n; i++ {
			ts5[i+1] = minf(ts4[i+1], ts5[i])
		}
		//		dbg(ts3)
		//		dbg(ts4)
		//		dbg(ts5)
		for i := 0; i < n+1; i++ {
			...
			//			dbg(i, ts2[i], ts4[i], ts5[ts2[i]], ts4[i]-ts5[ts2[i]])
			if ts4[i]-ts5[ts2[i]] >= 0 {
				return true
			}
		}
