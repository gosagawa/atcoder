...
func main() {
	...
	for i := 0; i < q; i++ {
		b, k := ni2()
		sx, sy, gx, gy := ni4()
		o := abs(gx-sx)*k + abs(gy-sy)*k
		slxy := [][3]int{}
		slxy = append(slxy, [3]int{sx - sx%b, sy, sx % b * k})
		slxy = append(slxy, [3]int{sx - sx%b + b, sy, (b - sx%b) * k})
		slxy = append(slxy, [3]int{sx, sy - sy%b, sy % b * k})
		slxy = append(slxy, [3]int{sx, sy - sy%b + b, (b - sy%b) * k})

		glxy := [][3]int{}
		glxy = append(glxy, [3]int{gx - gx%b, gy, gx % b * k})
		glxy = append(glxy, [3]int{gx - gx%b + b, gy, (b - gx%b) * k})
		glxy = append(glxy, [3]int{gx, gy - gy%b, gy % b * k})
		glxy = append(glxy, [3]int{gx, gy - gy%b + b, (b - gy%b) * k})

		check := func(f, t [3]int) int {
			if f[0]%b == 0 && f[0] == t[0] {
				return abs(f[1]-t[1]) + f[2] + t[2]
			}
			if f[1]%b == 0 && f[1] == t[1] {
				return abs(f[0]-t[0]) + f[2] + t[2]
			}
			...
			if f[0]%b == 0 {
				fp = append(fp, [3]int{f[0], f[1] - f[1]%b, f[2] + f[1]%b})
				fp = append(fp, [3]int{f[0], f[1] - f[1]%b + b, f[2] + b - f[1]%b})
			}
			if f[1]%b == 0 {
				fp = append(fp, [3]int{f[0] - f[0]%b, f[1], f[2] + f[0]%b})
				fp = append(fp, [3]int{f[0] + b - f[0]%b, f[1], f[2] + b - f[0]%b})
			}
			...
			if t[0]%b == 0 {
				tp = append(tp, [3]int{t[0], t[1] - t[1]%b, t[2] + t[1]%b})
				tp = append(tp, [3]int{t[0], t[1] - t[1]%b + b, t[2] + b - t[1]%b})
			}
			if t[1]%b == 0 {
				tp = append(tp, [3]int{t[0] - t[0]%b, t[1], t[2] + t[0]%b})
				tp = append(tp, [3]int{t[0] + b - t[0]%b, t[1], t[2] + b - t[0]%b})
			}
			sub := inf
			for j := 0; j < 2; j++ {
				for k := 0; k < 2; k++ {
					sub = min(sub, abs(fp[j][0]-tp[k][0])+abs(fp[j][1]-tp[k][1])+fp[j][2]+tp[k][2])
		...
		for j := 0; j < 4; j++ {
			for k := 0; k < 4; k++ {
				//				out(slxy[j], glxy[k], check(slxy[j], glxy[k]))
				mins(&o, check(slxy[j], glxy[k]))
			}
		}
