...
func main() {
	...
	mapping := func(f segfstruct, x segstruct) segstruct {
		...
		nv := madd(x.v, mmul(x.pos, f.v))
		npos := mmul(x.pos, f.pos)
		//debug("map", x, f, nv, npos)
		return segstruct{nv, npos}
	}
	compostion := func(f segfstruct, g segfstruct) segfstruct {
		...
		nv := madd(g.v, mmul(g.pos, f.v))
		npos := mmul(f.pos, g.pos)
		//debug("comp", f, g, nv, npos)
		return segfstruct{nv, npos}
	}
	...
	for i := m - 1; i >= 0; i-- {
		...
		nv := mmul(mdiv(1, lr), xs[i])
		npos := mdiv(lr-1, lr)
		debug(i, xs[i], nv, npos)
		lst.applyrange(l, r+1, segfstruct{nv, npos})
	}
	r := make([]int, n)
	for i := 0; i < n; i++ {
		r[i] = madd(lst.get(i).v, mmul(as[i], lst.get(i).pos))
	}
	outis(r)
