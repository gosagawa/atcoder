...
func main() {
	...
	for i := 0; i < n; i++ {
		for j := 0; j < n; j++ {
			a := ni()
			edges[i] = append(edges[i], edge{to: j, cost: a})
		}
	}

	graph := newgraph(n, edges)
	graph.defaultScore = inf
	o, hasRoad := graph.floydWarshallEx()
	if !hasRoad {
		out(-1)
		return
	}
...
func (g *graph) floydWarshallEx() (int, bool) {
	...
	for k := 0; k < g.size; k++ {
		for i := 0; i < g.size; i++ {
			for j := 0; j < g.size; j++ {
				if score[i][j] == inf || score[i][k] == inf || score[k][j] == inf {
					continue
				}
