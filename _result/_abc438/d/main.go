...
func main() {
	...
	for i := 0; i < n; i++ {
		...
		maxs(&dp1[i][0], dp1[i-1][0]+as[i])
		maxs(&dp1[i][1], dp1[i-1][0]+bs[i])
		maxs(&dp1[i][1], dp1[i-1][1]+bs[i])
	}
	for i := n - 1; i >= 0; i-- {
		if i == n-1 {
			dp2[i][0] = cs[i]
		...
		maxs(&dp2[i][0], dp2[i+1][0]+cs[i])
		maxs(&dp2[i][1], dp2[i+1][0]+bs[i])
		maxs(&dp2[i][1], dp2[i+1][1]+bs[i])
	}
	for i := 0; i < n; i++ {
		maxs(&o, dp1[i][1]+dp2[i][1]-bs[i])
	}
