...
func main() {
	...
	for i := 0; i < e; i++ {
		...
		s--
		t--
		edges[s] = append(edges[s], edge{to: t, cost: c, span: k})
		edges[t] = append(edges[t], edge{to: s, cost: c, span: k})
	}
...
func (g *graph) dijkstra(start int) []int {
	...
	for !que.IsEmpty() {
		...
		for _, edge := range g.edges[st.node] {
			newScore := st.score + edge.cost + (edge.span-st.score%edge.span)%edge.span
