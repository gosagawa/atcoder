...
func main() {
	...
	for i := 0; i < n; i++ {
		...
		for j := 0; j < ds[i]; j++ {
			temp[uf.root(i)] = append(temp[uf.root(i)], i)
		}
	}
	...
	for i := 0; i < n; i++ {
		if len(temp[i]) == 1 {
			c1 = append(c1, temp[i][0])
		} else if len(temp[i]) > 1 {
			c2 = append(c2, temp[i])
		}
	}
	ans := [][2]int{}
	...
	for _, v := range c2 {
		for i := 0; i < len(v)-1; i++ {
			...
			uf.unite(v[i], c1[c1idx])
			ans = append(ans, [2]int{v[i], c1[c1idx]})
			c1idx++
		}
	...
	uf.unite(c1[c1idx], c1[c1idx+1])
	ans = append(ans, [2]int{c1[c1idx], c1[c1idx+1]})
	...
	for _, v := range ans {
		out(fmt.Sprintf("%d %d", v[0]+1, v[1]+1))
	}
}
