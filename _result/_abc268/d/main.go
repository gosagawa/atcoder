...
func main() {
	...
	for {
		...
		var dfs func(c int, pc int, s string) string
		dfs = func(c int, pc int, s string) string {
			if c == 0 {
				return dfs(c+1, pc, ss[is[0]])
			} else if c != n-1 {
				for i := 0; i <= pc; i++ {
					r := dfs(c+1, pc-i, s+tt[i]+ss[is[c]])
					if r != "" {
						return r
					}
				}
