...
func main() {
	...
	for ti := 0; ti < t; ti++ {
		tsx, tsy, tgx, tgy := ni4()
		asx, asy, agx, agy := ni4()
		ts := pointf{float64(tsx), float64(tsy)}
		tg := pointf{float64(tgx), float64(tgy)}
		as := pointf{float64(asx), float64(asy)}
		ag := pointf{float64(agx), float64(agy)}
		...
		if td > ad {
			td, ad = ad, td
			ts, tg, as, ag = as, ag, ts, tg
		}
		tm := pointf{
			x: as.x + (ag.x-as.x)*td/ad,
			y: as.y + (ag.y-as.y)*td/ad,
		}

		fn1 := func(p float64) float64 {
			...
			return pointfDist(tp, ap)
		}
		fn2 := func(p float64) float64 {

			ap := pointf{
				x: tm.x + (ag.x-tm.x)*p,
				y: tm.y + (ag.y-tm.y)*p,
			}
			return pointfDist(tg, ap)
		}

		to1 := bs3fl(0.0, 1.0, 100, fn1)
		to2 := bs3fl(0.0, 1.0, 100, fn2)
		//		dbg(td, ad, to1, to2)
		//		dbg(fn(0), fn(to1), fn(td), fn(ad))
		o := fn1(to1)
		minsf(&o, fn2(to2))
		outfl(o)
	}
...
func pointfDistDouble(a, b pointf) float64 {
	return (a.x-b.x)*(a.x-b.x) + (a.y-b.y)*(a.y-b.y)
}
