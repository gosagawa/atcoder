...
func main() {
	...
	for i := 0; i < n; i++ {
		base[i] = segstruct{as[i], 1}
	}

	// 区間加算・区間和取得
	op := func(a segstruct, b segstruct) segstruct {
		return segstruct{min(a.v, b.v), a.size + b.size}
	}
	e := func() segstruct {
	...
	mapping := func(f segfstruct, x segstruct) segstruct {
		...
		return segstruct{x.v + int(f), x.size}
	}
	...
	for i := 0; i < q; i++ {
		...
		l--
		o := (r - l - bt.get(l, r)) * k
		lst.applyrange(l, r, segfstruct(-k))
		for {
			ti := lst.maxright(0, func(s segstruct) bool {
				return s.v >= 0
			})
			if ti == n || lst.get(ti).v >= 0 {
				break
			}
			o += lst.get(ti).v
			lst.set(ti, segstruct{inf, 0})
			bt.add(ti, 1)
		}
		out(o)
	}
