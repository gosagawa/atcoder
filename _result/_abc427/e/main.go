...
func main() {
	...
	o := -1
	h, w := ni2()
	mp := convidxi2s(nsi2s(h), map[string]int{".": 0, "#": 1, "T": 2})
	t := point{}
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			if mp[i][j] == 2 {
	...
	mv := func(dx, dy int, mp [][]int) [][]int {
		nmp := i2s(h, w, 0)
		for i := 0; i < h; i++ {
			for j := 0; j < w; j++ {
				pt := point{i + dx, j + dy}
				if pt.isValid(h, w) {
					nmp[i+dx][j+dy] = mp[i][j]
				}
			}
		}
		return nmp
	}
	dp := make(map[[144]int]int)
	fst := mptost(mp)
