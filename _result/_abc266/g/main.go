...
func main() {
	...
	o := 0
	r, g, b, k := ni4()
	cb := newcombFactorial(3000010)
	for i := k; i <= min(r, g); i++ {
		tr := r - i
		tg := g - i
		t := cb.factorial(tr + tg + b + i)
		t = mmul(t, cb.factorialInv(tr))
		t = mmul(t, cb.factorialInv(tg))
		t = mmul(t, cb.factorialInv(b))
		t = mmul(t, cb.factorialInv(k))
		t = mmul(t, cb.factorialInv(i-k))
		if (i-k)%2 == 0 {
			o = madd(o, t)
		} else {
			o = madd(o, -t)
...
func (c *combFactorial) factorialInv(n int) int {
	return c.facinv[n]
}
