...
func main() {
	...
	dbg(ns)
	used := make([]bool, 1000001)
	for i := 0; i < n; i++ {
		if ns[i] <= 1000 {
			t := cns.getRange(i+1, n-1)
			t -= ts[ns[i]]
			o += t / ns[i]
		} else {
			if !used[ns[i]] {
				o += ns2[ns[i]] * (ns2[ns[i]] - 1) / 2
			}
			used[ns[i]] = true

			c := 1
			for j := ns[i]; j <= 1000000; j += ns[i] {
				t := 0
				if c == 1 {
					t = cns2.getRange(j+1, min(j-1+ns[i], 1000000))
				} else {
					t = cns2.getRange(j, min(j-1+ns[i], 1000000))
				}
