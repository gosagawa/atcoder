...
func main() {
	...
	sort2ar(lrs, opt2ar(0, asc), opt2ar(1, desc))
	outans := func(sl [][2]int) {
		out(len(sl))
		ks := is(m, 0)
		for _, v := range sl {
			ks[v[0]] = v[1]
	...
	if lrs[0][0] == 1 && lrs[0][1] == n {
		outans([][2]int{[2]int{lrs[0][2], 1}})
		return
	}
	if m == 1 {
		out(-1)
	...
	for i := 0; i < m-1; i++ {
		if lrs[i][0] == lrs[i+1][0] {
			if lrs[i][1] > lrs[i+1][1] {
				outans([][2]int{[2]int{lrs[i][2], 1}, [2]int{lrs[i+1][2], 2}})
				return
			} else {
				outans([][2]int{[2]int{lrs[i][2], 2}, [2]int{lrs[i+1][2], 1}})
				return
			}
		} else if lrs[i][1] >= lrs[i+1][1] {
			outans([][2]int{[2]int{lrs[i][2], 1}, [2]int{lrs[i+1][2], 2}})
			return
		}
	}
	...
	rmin := lrs[0]
	rmax := lrs[m-1]
	if lmax[2] != rmin[2] && lmax[0] > rmin[1] {
		outans([][2]int{[2]int{lmax[2], 2}, [2]int{rmin[2], 2}})
		return
	}
	if lmin[0] == 1 && rmax[1] == n {
		if lmin[1] < rmax[0] {
			outans([][2]int{[2]int{lmin[2], 2}, [2]int{rmax[2], 2}})
			return
		} else {
			outans([][2]int{[2]int{lmin[2], 1}, [2]int{rmax[2], 1}})
			return
		}
	}
	if m == 2 {
	...
	outans([][2]int{[2]int{lrs[0][2], 2}, [2]int{lrs[1][2], 1}, [2]int{lrs[2][2], 2}})
...
func sort2ar(sl [][3]int, setters ...Sort2ArOption) {
	args := &Sort2ArOptions{}
