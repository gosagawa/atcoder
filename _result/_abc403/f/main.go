...
func main() {
	...
	n := ni()
	number := make(map[int]st)
	number[1] = st{l: 1, str: "1"}
	number[11] = st{l: 2, str: "11"}
	number[111] = st{l: 3, str: "111"}
	number[1111] = st{l: 4, str: "1111"}
	expr := make(map[int]st)
	term := make(map[int]st)
	factor := make(map[int]st)
	var dfsexpr func(v int) st
	var dfsterm func(v int) st
	var dfsfactor func(v int) st
	dfsexpr = func(v int) st {
		if r, ok := expr[v]; ok {
			return r
		}
		tst := dfsterm(v)
		for i := 1; i < v; i++ {
			st1 := dfsexpr(i)
			st2 := dfsterm(v - i)
			if tst.l > st1.l+st2.l+1 {
				tst.l = st1.l + st2.l + 1
				tst.str = st1.str + "+" + st2.str
			}
		}
		expr[v] = tst
		return tst
	}
	dfsterm = func(v int) st {
		if r, ok := term[v]; ok {
			return r
		}
		tst := dfsfactor(v)
