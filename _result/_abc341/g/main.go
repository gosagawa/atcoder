...
func main() {
	...
	ns = append([]int{0}, ns...)
	cns := newcusum(ns)
	ps := []point{point{x: n, y: cns.get(n)}}
	rs := make([]float64, n)

	culc := func(a, b point) float64 {
		if a.x > b.x {
			a, b = b, a
		}
		return float64(b.y-a.y) / float64(b.x-a.x)
	}
	cmp := func(p1, p2, p3 point) bool {
		return (p3.y-p1.y)*(p2.x-p1.x) >= (p2.y-p1.y)*(p3.x-p1.x)
	}

	for i := n - 1; i >= 0; i-- {
		np := point{x: i, y: cns.get(i)}
		for {
			if len(ps) < 2 {
				break
			}
			if cmp(np, ps[len(ps)-1], ps[len(ps)-2]) {
				ps = ps[:len(ps)-1]
			} else {
				break
			}
		...
		ps = append(ps, np)
		debug(ps)
		rs[i] = culc(ps[len(ps)-2], ps[len(ps)-1])
	}
	for _, v := range rs {
		outfl(v)
	}
