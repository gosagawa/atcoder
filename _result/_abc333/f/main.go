...
func main() {
	...
	for i := 2; i <= n; i++ {
		for j := i - 1; j >= 1; j-- {
			debug(i-1, i-j)
			dp[i] = madd(dp[i], mmul(dp[j], cf.combination(i-1, i-j)))
		}
		dp[i] = mdiv(dp[i], madd(mpow(2, i, mod), -1))
	}
	rs := is(n, 0)
	for i := 0; i < n; i++ {
		if i == 0 {
		...
		for j := 0; j <= i; j++ {
			rs[i] = madd(rs[i], mmul(cf.combination(i, j), dp[n-j]))
		}
		rs[i] = mdiv(rs[i], mpow(2, i, mod))
	}
	outis(rs)
}
