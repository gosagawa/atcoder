...
func main() {
	...
	for _, a := range as {
		for _, i := range mp[a] {
			dist[i] = max(rmax[rs[i]], cmax[cs[i]])
		}
		for _, i := range mp[a] {
			rmax[rs[i]] = max(rmax[rs[i]], dist[i]+1)
			cmax[cs[i]] = max(cmax[cs[i]], dist[i]+1)
		}
	}
	for _, v := range dist {
		out(v)
...
func sorti2ar2k(sl [][2]int, key1, key2 int) {
	sort.Slice(sl, func(i, j int) bool {
...
func sort2ar(sl [][2]int, key1 int) {
	sort.Slice(sl, func(i, j int) bool {
		return sl[i][key1] < sl[j][key1]
	})
}
