...
func main() {
	...
	anst := [9]int{}
	ans := 0
	used := make([]bool, 1000000000)

	for i := 0; i < 8; i++ {
		p := ni()
		pos += p * int(math.Pow10(i))
		anst[p-1] = p
	}

	for i := 0; i < 9; i++ {
		ans += anst[i] * int(math.Pow10(i))
	}

	q := list.New()
	q.PushBack(cs{pos, 0, 8})
	e := q.Front()
	for e != nil {
		s := e.Value.(cs)
		if used[s.base] {
		...
		for _, v := range edges[s.emp] {
			nbase := s.base
			nbase += s.base % int(math.Pow10(v.to+1)) / int(math.Pow10(v.to)) * int(math.Pow10(s.emp))
			nbase -= s.base%int(math.Pow10(v.to+1)) - s.base%int(math.Pow10(v.to))
			q.PushBack(cs{nbase, s.dist + 1, v.to})
		}
		e = e.Next()
	}
	out(-1)
