...
func main() {
	...
	bs2[0] = 1
	for i := 0; i < n; i++ {
		bs1[i+1] = mmul(bs1[i], 2)
		bs2[i+1] = mmul(bs2[i], 10)
		bs2[i+1] = madd(bs2[i+1], bs1[i])
	}
	for i, vs := range s {
		v := int(vs - '0')
		//		out(v, bs1[i], bs2[n-i-1], mmul(mmul(v, bs1[i]), bs2[n-i-1]))
		o = madd(o, mmul(mmul(v, bs1[i]), bs2[n-i-1]))
	}
	//	out(bs1)
	//	out(bs2)
