	...
	dfs = func(v, p int) {
		for _, nv := range edges[v] {
			...
			for i := k; i >= 0; i-- {
				for j := k; j >= 0; j-- {
					if i+j <= k {
						maxs(&dp[v][0][i+j], dp[v][0][i]+dp[nv.to][0][j])
						maxs(&dp[v][1][i+j], dp[v][0][i]+dp[nv.to][1][j])
						maxs(&dp[v][0][i+j], dp[v][0][i]+dp[nv.to][2][j])

						maxs(&dp[v][1][i+j], dp[v][1][i]+dp[nv.to][0][j])
						maxs(&dp[v][1][i+j], dp[v][1][i]+dp[nv.to][2][j])

						maxs(&dp[v][2][i+j], dp[v][2][i]+dp[nv.to][0][j])
						maxs(&dp[v][2][i+j], dp[v][2][i]+dp[nv.to][1][j])
						maxs(&dp[v][2][i+j], dp[v][2][i]+dp[nv.to][2][j])

					}
					if i+j > 0 && i+j-1 <= k {
						maxs(&dp[v][2][i+j-1], dp[v][1][i]+dp[nv.to][1][j])
					}
				}
			}
		}
		for i := k; i >= 0; i-- {
			if i+1 <= k {
				maxs(&dp[v][1][i+1], dp[v][0][i]+as[v])
			}
			maxs(&dp[v][1][i], dp[v][1][i]+as[v])
			maxs(&dp[v][2][i], dp[v][2][i]+as[v])
		}
	}
	dfs(0, -1)
	for i := 0; i < 3; i++ {
