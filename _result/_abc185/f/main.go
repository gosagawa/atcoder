...
func main() {
	...
	/*
		out(bits[0].sum(0), bits[1].sum(0), bits[2].sum(0))
		out(bits[0].sum(1), bits[1].sum(1), bits[2].sum(1))
		out(bits[0].sum(2), bits[1].sum(2), bits[2].sum(2))
		out(bits[0].sum(3), bits[1].sum(3), bits[2].sum(3))
	*/
	for i := 0; i < q; i++ {
		t, x, y := ni3()
		switch t {
		...
		case 2:
			...
			o := 0
			for j := 0; j < max2n; j++ {
				//	out(j, bits[j].sum(y+1), bits[j].sum(x))
				if (bits[j].sum(y+1)-bits[j].sum(x))%2 != 0 {
					o |= (1 << j)
				}
			}
			out(o)
