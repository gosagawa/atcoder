...
func main() {
	...
	for i := 0; i < n; i++ {
		for j := 0; j < m; j++ {
			...
			for k := 0; k < 4; k++ {
				...
				if np.isValid(n, m) && mp[np.x][np.y] != -62 {
					s := i*m + j
					t := np.x*m + np.y
					edges[s] = append(edges[s], edge{to: t, cost: 2})
				}
			}
			if mp[i][j] >= 0 && mp[i][j] < 26 {
				s := i*m + j
				t := n*m + mp[i][j]
				edges[s] = append(edges[s], edge{to: t, cost: 1})
				edges[t] = append(edges[t], edge{to: s, cost: 1})
			}
		}
	}
	dist := is(n*m+26, inf)
	...
	for e != nil {
		...
		for _, ne := range edges[t] {
			if dist[ne.to] > dist[t]+ne.cost {
				dist[ne.to] = dist[t] + ne.cost
				q.PushBack(ne.to)
			}
		}
