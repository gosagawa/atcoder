...
func main() {
	...
	for i := 0; i < e; i++ {
		...
		s--
		t--
		edges[s] = append(edges[s], edge{to: t, cost: c1})
		edges[t] = append(edges[t], edge{to: s, cost: c1})
		edges2[s] = append(edges2[s], edge{to: t, cost: c2})
		edges2[t] = append(edges2[t], edge{to: s, cost: c2})
	}

	graph := newgraph(v, edges)
	dist := graph.dijkstra(st)

	graph = newgraph(v, edges2)
	dist2 := graph.dijkstra(to)
	tc := make([]int, v)
	for i := 0; i < v; i++ {
		tc[i] = dist[i] + dist2[i]
	}
