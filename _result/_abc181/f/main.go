...
func main() {
	...
	ds := [][3]int{}
	setds := func(x1, y1, x2, y2 int) {
		ds = append(ds, [3]int{is[x1][y1], is[x2][y2], pointDistDouble(point{x1, y1}, point{x2, y2})})
	}
	for i := 0; i < n; i++ {
		setds(xs[i], ys[i], xs[i], 0)
		setds(xs[i], ys[i], xs[i], 200)
	}
	for i := 0; i < n; i++ {
		for j := i + 1; j < n; j++ {
			setds(xs[i], ys[i], xs[j], ys[j])
		}
	}
	sort2ar(ds, opt2ar(2, asc))
	for _, v := range ds {
		uf.unite(v[0], v[1])
		if uf.issame(0, 201) {
			o = v[2]
