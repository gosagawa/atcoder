...
func main() {
	...
	fc := as[0]*bs[0] + bs[0]*cs[0] + cs[0]*as[0]
	heap.Push(pq, pqst{c: fc, x: 0, y: 0, z: 0})
	for i := 0; i < k; i++ {
		v := heap.Pop(pq).(pqst)
		dbg(v)
		o = v.c
		if v.x != n-1 {
			if !used[[3]int{v.x + 1, v.y, v.z}] {
				used[[3]int{v.x + 1, v.y, v.z}] = true
				heap.Push(pq, pqst{c: as[v.x+1]*bs[v.y] + bs[v.y]*cs[v.z] + cs[v.z]*as[v.x+1], x: v.x + 1, y: v.y, z: v.z})
			}
		}
		if v.y != n-1 {
			if !used[[3]int{v.x, v.y + 1, v.z}] {
				used[[3]int{v.x, v.y + 1, v.z}] = true
				heap.Push(pq, pqst{c: as[v.x]*bs[v.y+1] + bs[v.y+1]*cs[v.z] + cs[v.z]*as[v.x], x: v.x, y: v.y + 1, z: v.z})
			}
		}
		if v.z != n-1 {
			if !used[[3]int{v.x, v.y, v.z + 1}] {
				used[[3]int{v.x, v.y, v.z + 1}] = true
				heap.Push(pq, pqst{c: as[v.x]*bs[v.y] + bs[v.y]*cs[v.z+1] + cs[v.z+1]*as[v.x], x: v.x, y: v.y, z: v.z + 1})
			}
		}
	}
