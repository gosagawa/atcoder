...
func main() {
	...
	for i := n - 1; i >= 0; i-- {
		if i == n-1 {
			p1 := mmul(mdiv(i, n), mdiv(i, n)) // + 1
			p2 := mmul(mdiv(i, n), mdiv(1, n)) // + 1
			//p3 := mdiv(1, n)                   // + 0
			...
			p1 := mmul(mdiv(i, n), mdiv(i, n))
			p2 := mmul(mdiv(i, n), mdiv(n-i, n))
			p3 := mmul(mdiv(n-i, n), mdiv(i+1, n))
			p4 := mmul(mdiv(n-i, n), mdiv(n-i-1, n))

			pm := mdiv(1, madd(1, -p1))

			dp[i] = madd(dp[i], p1)
			dp[i] = madd(dp[i], mmul(p2, madd(dp[i+1], 1)))
			dp[i] = madd(dp[i], mmul(p3, dp[i+1]))
			dp[i] = madd(dp[i], mmul(p4, dp[i+2]))
			dp[i] = mmul(dp[i], pm)
		}
	}
	outf("%d %d", dp[0], dp[1])
