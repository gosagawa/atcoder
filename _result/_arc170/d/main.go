...
func main() {
	...
	for qi := 0; qi < t; qi++ {
		...
		mpdel := make([][]int, n)
		for i := 0; i < n; i++ {
			t1 := lowerBound(bs[i], as)
			ts := [][2]int{}
			for j := max(0, t1-1); j <= min(n-1, t1+2); j++ {
				ts = append(ts, [2]int{abs(as[j] - bs[i]), j})
			}
			sort2ar(ts, opt2ar(0, asc))
			mpadd[ts[0][1]] = append(mpadd[ts[0][1]], ts[1][0])
			mpdel[ts[0][1]] = append(mpdel[ts[0][1]], ts[0][0])
			s.Increment(ts[0][0])
		}

		for i := 0; i < n; i++ {
			if as[i] > bs[0] {
				t1 := upperBound(as[i]-bs[0], as)
				t2 := lowerBound(as[i]+bs[0], as)
				if t1 == i && t2 == i {
					continue
				}
			}
			for _, v := range mpdel[i] {
				s.Decrement(v)
			}
			for _, v := range mpadd[i] {
				s.Increment(v)
			}
			lv, haskey := s.LowerBound(inf)
			if haskey && lv < as[i] {
				aw = true
			}
			for _, v := range mpdel[i] {
