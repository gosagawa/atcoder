...
func main() {
	...
	for i := 0; i < n; i++ {
		x1, y1, x2, y2 := ni4()
		ns[i] = [2]point{{x1, y1}, {x2, y2}}
		xs[i*2] = x1
		xs[i*2+1] = x2
		ys[i*2] = y1
		ys[i*2+1] = y2
	}
	ns[n] = [2]point{{0, 0}, {w, h}}
	xs[n*2] = 0
	xs[n*2+1] = w
	ys[n*2] = 0
	ys[n*2+1] = h
	...
	xm := uniquem(xs)
	ym := uniquem(ys)
	xys := make([][]int16, len(xm)+1)
	for i := 0; i < len(xm)+1; i++ {
		xys[i] = make([]int16, len(ym)+1)
	}
	for i := 0; i <= n; i++ {
		xi1 := xm[ns[i][0].x]
		xi2 := xm[ns[i][1].x]
	...
	for i := 0; i < len(xm)-1; i++ {
		for j := 0; j < len(ym)-1; j++ {
			...
			for len(q) != 0 {
				...
				for _, d := range dr {
					np := pointAdd(p, d)
					if !np.isValid(w, h) || xys[np.x][np.y] != 1 {
						continue
					}
					q = append(q, np)
				}
...
func uniquem(sl []int) map[int]int {
