...
func main() {
	...
	for i := 0; i < e; i++ {
		...
		edges2 = append(edges2, edge{from: s, to: t, cost: c})
	}
	o := 0

	graph := newgraph(vi, edges)
	dist, _ := graph.floydWarshall()
	for _, v := range edges2 {
		if dist[v.from][v.to] < v.cost {
			o++
		} else if dist[v.from][v.to] == v.cost {
			for i := 0; i < vi; i++ {
				if i == v.from || i == v.to {
					continue
				}
				if dist[v.from][i]+dist[v.to][i] == v.cost {
					o++
					break
				}
			}
