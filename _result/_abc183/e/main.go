	...
	for hi := 0; hi < h; hi++ {
		for wi := 0; wi < w; wi++ {
			...
			if hi+i >= h || m[hi+i][wi] {
				...
				dist[hi+i][wi] = madd(dist[hi+i][wi], dist[hi][wi])
				dist[hi+i][wi] = madd(dist[hi+i][wi], sumx[hi][wi])
				sumx[hi+i][wi] = madd(sumx[hi+i][wi], dist[hi][wi])
				sumx[hi+i][wi] = madd(sumx[hi+i][wi], sumx[hi][wi])
			}
			if wi+i >= w || m[hi][wi+i] {
			} else {
				dist[hi][wi+i] = madd(dist[hi][wi+i], dist[hi][wi])
				dist[hi][wi+i] = madd(dist[hi][wi+i], sumy[hi][wi])
				sumy[hi][wi+i] = madd(sumy[hi][wi+i], dist[hi][wi])
				sumy[hi][wi+i] = madd(sumy[hi][wi+i], sumy[hi][wi])
			}

			nh := hi + i
			nw := wi + i
			if nh >= h || nw >= w || m[nh][nw] {
			} else {
				dist[nh][nw] = madd(dist[nh][nw], dist[hi][wi])
				dist[nh][nw] = madd(dist[nh][nw], sumxy[hi][wi])
				sumxy[nh][nw] = madd(sumxy[nh][nw], dist[hi][wi])
				sumxy[nh][nw] = madd(sumxy[nh][nw], sumxy[hi][wi])
			}
		}
	}
