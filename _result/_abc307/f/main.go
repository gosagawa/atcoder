...
func main() {
	...
	pq2 := newpq([]compFunc{func(p, q interface{}) int {
		if p.(pqst).y != q.(pqst).y {
			// get from bigger
			if p.(pqst).y > q.(pqst).y {
				//if p.(pqst).y < q.(pqst).y {
	...
	for i := 0; i < k; i++ {
		for j, v := range edges[as[i]-1] {
			if r[j] == -1 {
				heap.Push(pq, pqst{x: j, y: v})
			}
			delete(edges[j], as[i]-1)
		}
	}
	for i := 0; i < d; i++ {
		x := xs[i]
		for !pq.IsEmpty() {
			...
			if st.y <= x {
				...
				heap.Push(pq2, pqst{x: st.x, y: x - st.y})
			} else {
				heap.Push(pq, st)
				break
			}
		...
		for !pq2.IsEmpty() {
			...
			//out("2", edges[st.x])
			r[st.x] = i + 1
			for j, v := range edges[st.x] {
				if r[j] != -1 {
					continue
				}
				if v <= st.y {
					heap.Push(pq2, pqst{x: j, y: st.y - v})
				} else {
					heap.Push(pq, pqst{x: j, y: v})
				}
				delete(edges[j], st.x)
