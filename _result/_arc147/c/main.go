...
func main() {
	...
	sl, mp := cocom(lrs)
	s := newlazystree(len(sl)+1, stmax, stadd)
	for i := 0; i < n; i++ {
		s.rc(0, mp[ls[i]], -1)
		s.rc(mp[rs[i]]+1, len(sl)+1, 1)
		//		s.debug2()
	}
	abmin := inf
	abminis := []int{}
	...
	for _, abmini := range abminis {
		to := 0
		mp2 := make(map[int]int)
		for i := 0; i < n; i++ {
			if sl[abmini] < ls[i] {
				mp2[ls[i]]++
			} else if sl[abmini] > rs[i] {
				mp2[rs[i]]++
			} else {
				mp2[sl[abmini]]++
			}
		...
		for i := 1; i < len(sl2); i++ {
			mp2sl2 := mp2[sl2[i]]
			to += (sl2[i] - base) * c * mp2sl2
			to -= (sl2[i] - base) * (n - c - mp2sl2) * mp2sl2
			c += mp2sl2
		}
		mins(&o, to)
	}
