...
func main() {
	...
	n := ni()
	xds := ni2a(n)
	sort2ar(xds, opt2ar(0, desc))
	ts := []int{}
	for i := 0; i < n; i++ {
		ts = append(ts, xds[i][0])
		ts = append(ts, xds[i][0]+xds[i][1])
	}
	_, mp := cocom(ts)
	for i := 0; i < n; i++ {
		xds[i][0], xds[i][1] = mp[xds[i][0]], mp[xds[i][0]+xds[i][1]]
	}
	debug(xds)
	dp := i2s(n+1, 2, 0)
	dp[0][0] = 1
	...
	debug(ids)
	s := newstree(len(mp), stmax)
	for i, v := range xds {
		s.set(v[0], v[1]-1)
		mx := s.query(v[0], v[1])
		s.set(v[0], mx)
		ti := n - upperBound(mx, ids)
		debug(i, ti, mx)
		dp[i+1][0] = madd(dp[i][0], dp[i][1])
		dp[i+1][1] = madd(dp[ti][0], dp[ti][1])
		debug(dp[i+1])
	}
	o = madd(dp[n][0], dp[n][1])
	out(o)
}
