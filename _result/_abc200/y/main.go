...
func main() {
	...
	if k <= ml {
		i := bs(2, n, func(c int) bool {
			return c*(c-1)*(c-2)/6 < k
		}) - 1
		k -= (i + 1) * i * (i - 1) / 6
		for j := 0; j < i; j++ {
			if k <= i-j {
		...
		subTotal := func(x int) int {
			t := n * (n + 1) / 2 * x
			if x > 2 {
				x--
				t += -x*(x+1)*(2*x+1)/6 + (n-1)*x*(x+1)/2
			}
			return t
		}

		i := bs(0, n, func(c int) bool {
			return subTotal(c) < k
		})
		k -= subTotal(i)
		i++
		...
		for j := 0; j < n; j++ {
			...
			if j < i-1 {
				//out(n-i+1+j, k, i, j)
				if k <= n-i+1+j {
					//out(i, j, k, n+i+1, n+i-j-1, max(1, i-j-1))
					//t := min(n,n - k - j + 1)
					r[0] = itoa(j + 1)
					r[1] = itoa(max(1, i-j-1) + k)
					r[2] = itoa(n + i + 1 - j - 1 - max(1, i-j-1) - k)
					break
				}
				//out(k, n-i+1+j)
				k -= n - i + 1 + j
	...
	if isR {
		r[0] = itoa(n + 1 - atoi(r[0]))
		r[1] = itoa(n + 1 - atoi(r[1]))
		r[2] = itoa(n + 1 - atoi(r[2]))
	}
	out(strings.Join(r, " "))
	//	}
