...
func main() {
	...
	if omod != -1 {
		out("Yes")
		oxs := make([]string, len(ox))
		for i := range ox {
			ox[i]++
			oxs[i] = strconv.Itoa(ox[i])
		}
		oys := make([]string, len(oy))
		for i := range oy {
			oy[i]++
			oys[i] = strconv.Itoa(oy[i])
		}
		out(fmt.Sprintf("%v %v", len(ox), strings.Join(oxs, " ")))
		out(fmt.Sprintf("%v %v", len(oy), strings.Join(oys, " ")))
	} else {
		out("No")
	}
	/*
