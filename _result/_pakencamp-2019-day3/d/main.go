...
func main() {
	...
	for i := 1; i < n; i++ {
		dp[i] = make([]int, 4)
		dp[i][2] = dp[i-1][1] + 5 - m[i][2]
		dp[i][3] = dp[i-1][1] + 5 - m[i][3]
		dp[i][1] = dp[i-1][2] + 5 - m[i][1]
		dp[i][3] = min(dp[i][3], dp[i-1][2]+5-m[i][3])
		dp[i][1] = min(dp[i][1], dp[i-1][3]+5-m[i][1])
		dp[i][2] = min(dp[i][2], dp[i-1][3]+5-m[i][2])
	}
	o = min(min(dp[n-1][1], dp[n-1][2]), dp[n-1][3])
