...
func solve() {
	...
	/*
		getXf := func(l1, l2 [3]int) float64 {
			a1, b1, c1 := l1[0], l1[1], l1[2]
			a2, b2, c2 := l2[0], l2[1], l2[2]
			return float64(b2*c1-b1*c2) / float64(a1*b2-a2*b1)
		}
	*/
	getX := func(l1, l2 [3]int) int {
		a1, b1, c1 := l1[0], l1[1], l1[2]
		a2, b2, c2 := l2[0], l2[1], l2[2]
		return (b1*c2-b2*c1-1)/(a2*b1-a1*b2) + 1
	}

	for _, v := range abs {
		aj, bj, cj := v[0], v[1], v[2]
		if len(co) >= 1 {
			ai, bi, _ := co[len(co)-1][0], co[len(co)-1][1], co[len(co)-1][2]
			if ai*bj == aj*bi {
				continue
			}
		}
		...
		if len(x) == 0 {
			...
			xj := getX(co[len(co)-2], v)
			dbg("xj", xj)
			x = append(x, xj)
		}
		xLim = min(xLim, (cj+aj-1)/aj)
	}
	x = append(x, inf)
	dbg(co)
	dbg(x)
	var floorSum func(n, m, a, b int) int
	floorSum = func(n, m, a, b int) int {
		a1, a2 := a/m, a%m
		ans := n * (n - 1) / 2 * a1
		if a2 == 0 {
			return ans + (b/m)*n
		...
		b1, b2 := b/m, b%m
		y := (a2*n + b2) / m
		ans += n*(y+b1) - floorSum(y, a2, m, m+a2-b2-1)
		return ans
	}

	clamp := func(x, l, r int) int {
		return min(max(x, l), r)
	}
	o := 0
	for i, v := range co {
		ai, bi, ci := v[0], v[1], v[2]
		le, ri := clamp(x[i], 1, xLim), clamp(x[i+1], 1, xLim)
		dbg(le, ri)
		n, m, a, b := ri-le, bi, ai, ci-1-ai*(ri-1)
		o += floorSum(n, m, a, b%m) + (b/m)*n
	}
	out(o)
}
