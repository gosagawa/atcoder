...
func main() {
	...
	for qi := 0; qi < t; qi++ {
		...
		af := mpow(10, a-1, mod)
		at := madd(mpow(10, a, mod), -1)
		ac := madd(madd(at, -af), 1)
		debug(af, at, ac)

		bf := mpow(10, b-1, mod)
		bt := madd(mpow(10, b, mod), -1)
		bc := madd(madd(bt, -bf), 1)
		debug(bf, bt, bc)
		if a == b {

			t1 := madd(af, ac)
			t2 := madd(madd(ac, -af), 1)
			t3 := mmul(t1, t2)
			t4 := mdiv(t3, 2)
			debug(t1, t2, t3, t4)

			t5 := mmul(madd(af, -1), ac)
			nup := madd(t4, t5)
			nst := madd(mmul(ac, ac), -nup)
			debug(t5, nup, nst)
		...
		t1 := madd(af, at)
		t2 := madd(madd(at, -af), 1)
		t3 := mmul(t1, t2)
		t4 := mdiv(t3, 2)
		debug(t1, t2, t3, t4)

		nup := t4
		nst := madd(mmul(ac, bc), -nup)
		debug(nup, nst)
