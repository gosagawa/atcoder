...
func main() {
	...
	n := ni()
	xs, ys, cs := ni3s(n)
	makeRect := func(xi, yi, ci int, t float64) (float64, float64, float64, float64) {
		x := float64(xi)
		y := float64(yi)
		c := float64(ci)
		fx := x - t/c
	...
	check2 := func(xf1, xt1, xf2, xt2 float64) (float64, float64, bool) {
		if xf1 > xf2 {
			xf1, xf2 = xf2, xf1
			xt1, xt2 = xt2, xt1
		}
	...
	check := func(c float64) bool {
		xf1, yf1, xt1, yt1 := makeRect(xs[0], ys[0], cs[0], c)
		for i := 1; i < n; i++ {
			xf2, yf2, xt2, yt2 := makeRect(xs[i], ys[i], cs[i], c)
			nxf1, nxt1, ok := check2(xf1, xt1, xf2, xt2)
			if !ok {
				return true
			}
			nyf1, nyt1, ok := check2(yf1, yt1, yf2, yt2)
			if !ok {
				return true
			}
			xf1, xt1, yf1, yt1 = nxf1, nxt1, nyf1, nyt1
		}

		return false
	}
	o = bsfl(0.0, float64(1e11), 100, check)
	outfl(o)
}
