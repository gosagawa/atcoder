...
func main() {
	...
	dist1 := graph.dijkstra(0)
	dist2 := graph.dijkstra(v - 1)
	dbg(dist1)
	dbg(dist2)
	...
	for i := 0; i < e; i++ {
		s, t, c := ss[i], ts[i], cs[i]
		rs[i] = "No"
		if dist1[s]+c+dist2[t] == dist1[v-1] {
			edges2[s] = append(edges2[s], edge{from: s, to: t, cost: i})
			edges2[t] = append(edges2[t], edge{from: t, to: s, cost: i})
		} else if dist1[t]+c+dist2[s] == dist1[v-1] {
			edges2[s] = append(edges2[s], edge{from: s, to: t, cost: i})
			edges2[t] = append(edges2[t], edge{from: t, to: s, cost: i})
		} else {
			rs[i] = "No"
