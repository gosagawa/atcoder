	...
	for qi := 0; qi < t; qi++ {
		...
		for i := 0; i < np; i++ {
			...
			for j := 0; j < k+1; j++ {
				if hasbit(n, np-i-1) {
					if j != k {
						dp[1][i+1][j+1] = madd(dp[1][i+1][j+1], dp[1][i][j])
						dpv[1][i+1][j+1] = madd(dpv[1][i+1][j+1], dpv[1][i][j]+dp[1][i][j]*i2%mod)
						dp[0][i+1][j+1] = madd(dp[0][i+1][j+1], dp[0][i][j])
						dpv[0][i+1][j+1] = madd(dpv[0][i+1][j+1], dpv[0][i][j]+dp[0][i][j]*i2%mod)
					}
					dp[0][i+1][j] = madd(dp[0][i+1][j], dp[1][i][j])
					dpv[0][i+1][j] = madd(dpv[0][i+1][j], dpv[1][i][j]+dp[1][i][j]*0)
					dp[0][i+1][j] = madd(dp[0][i+1][j], dp[0][i][j])
					dpv[0][i+1][j] = madd(dpv[0][i+1][j], dpv[0][i][j]+dp[0][i][j]*0)
				} else {
					dp[1][i+1][j] = madd(dp[1][i+1][j], dp[1][i][j])
					dpv[1][i+1][j] = madd(dpv[1][i+1][j], dpv[1][i][j]+dp[1][i][j]*0)
					if j != k {
						dp[0][i+1][j+1] = madd(dp[0][i+1][j+1], dp[0][i][j])
						dpv[0][i+1][j+1] = madd(dpv[0][i+1][j+1], dpv[0][i][j]+dp[0][i][j]*i2%mod)
					}
			...
			dbg(i, dp[0][i+1], dp[1][i])
			dbg(i, dpv[0][i+1], dpv[1][i])
		}

		o := dpv[0][np][k] + dpv[1][np][k]
		o %= mod
		out(o)
	}
}
