...
func main() {
	...
	qis := make([][][2]int, n)
	for i := 0; i < q; i++ {
		qis[qs[i]-1] = append(qis[qs[i]-1], [2]int{ls[i], i})
	}
	dp1 := i2s(11, lmax+1, 0)
	dp2 := i2s(pow2(10), 2, 0)
	rs := is(q, 0)
	...
	dfs = func(i int) {
		...
		if hs[i] < nh {
			th := hs[i]
			for j := 1; j <= lmax; j++ {
				dp1[th+1][j] = max(dp1[th+1][j-1], dp1[th][j])
				/*
					maxs(&dp1[th+1][j], dp1[th][j])
					if j > 0 {
						maxs(&dp1[th+1][j], dp1[th+1][j-1])
					}
				*/
				if j >= w {
					maxs(&dp1[th+1][j], dp1[th][j-w]+v)
				}
			}
			for _, v := range qis[i] {
				rs[v[1]] = dp1[th+1][v[0]]
			...
			ii := hs[i] - nh
			for j := 0; j < pow2(ii); j++ {
				dp2[j+pow2(ii)][0] = dp2[j][0] + v
				dp2[j+pow2(ii)][1] = dp2[j][1] + w
				for _, v := range qis[i] {
					d := v[0] - dp2[j][1]
					if d >= 0 {
						maxs(&rs[v[1]], dp2[j][0]+dp1[nh][d])
					}
					d = v[0] - dp2[j+pow2(ii)][1]
					if d >= 0 {
						maxs(&rs[v[1]], dp2[j+pow2(ii)][0]+dp1[nh][d])
					}
				}
			}
		}
