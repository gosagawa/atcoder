...
func main() {
	...
	n, x, y := ni3()
	rects := make([][]rect, n+1)
	rects[0] = append(rects[0], rect{point{0, 0}, point{x - 1, y - 1}})
	for ti := 0; ti < n; ti++ {
		t := ns()
		a, b := ni2()
		if t == "X" {
			for _, v := range rects[ti] {
				if v.t.x < a {
					rects[ti+1] = append(rects[ti+1], rect{point{v.f.x, v.f.y - b}, point{v.t.x, v.t.y - b}})
				} else if v.f.x >= a {
					rects[ti+1] = append(rects[ti+1], rect{point{v.f.x, v.f.y + b}, point{v.t.x, v.t.y + b}})
				} else {

					rects[ti+1] = append(rects[ti+1], rect{point{v.f.x, v.f.y - b}, point{a - 1, v.t.y - b}})
					rects[ti+1] = append(rects[ti+1], rect{point{a, v.f.y + b}, point{v.t.x, v.t.y + b}})
				}
			...
			for _, v := range rects[ti] {
				if v.t.y < a {
					rects[ti+1] = append(rects[ti+1], rect{point{v.f.x - b, v.f.y}, point{v.t.x - b, v.t.y}})
				} else if v.f.y >= a {
					rects[ti+1] = append(rects[ti+1], rect{point{v.f.x + b, v.f.y}, point{v.t.x + b, v.t.y}})
				} else {
					rects[ti+1] = append(rects[ti+1], rect{point{v.f.x - b, v.f.y}, point{v.t.x - b, a - 1}})
					rects[ti+1] = append(rects[ti+1], rect{point{v.f.x + b, a}, point{v.t.x + b, v.t.y}})
				}
			}
	...
	if logic == 1 {
		...
		match := func(i, j int) bool {
			...
			jr := rects[n][j]
			if ir.t.x == jr.f.x-1 {
				if ir.t.y >= jr.f.y && ir.f.y <= jr.t.y {
					return true
				}
				if jr.t.y >= ir.f.y && jr.f.y <= ir.t.y {
					return true
				}
			}
			if ir.t.y == jr.f.y-1 {
				if ir.t.x >= jr.f.x && ir.f.x <= jr.t.x {
					return true
				}
				if jr.t.x >= ir.f.x && jr.f.x <= ir.t.x {
					return true
				}
			}
			return false
		...
		for i := 0; i < tn; i++ {
			mp2[uf.root(i)] += (rects[n][i].t.x - rects[n][i].f.x + 1) * (rects[n][i].t.y - rects[n][i].f.y + 1)
		}
		rs := []int{}
		for _, v := range mp2 {
			rs = append(rs, v)
		...
		xs := []int{}
		ys := []int{}
		for _, v := range rects[n] {
			xs = append(xs, v.f.x)
			xs = append(xs, v.f.x-1)
			xs = append(xs, v.t.x)
			xs = append(xs, v.t.x+1)
			ys = append(ys, v.f.y)
			ys = append(ys, v.f.y-1)
			ys = append(ys, v.t.y)
			ys = append(ys, v.t.y+1)
		}
		xsl, xmp := cocom(xs)
		//		dbg(xsl, xmp)
		ysl, ymp := cocom(ys)
		//		dbg(ysl, ymp)
		mp := i2s(len(xsl), len(ysl), 0)
		for _, v := range rects[n] {
			if _, ok := xmp[v.f.x]; !ok {
				panic("")
			}
			if _, ok := xmp[v.t.x]; !ok {
				panic("")
			}
			if _, ok := ymp[v.f.y]; !ok {
				panic("")
			}
			if _, ok := ymp[v.t.y]; !ok {
				panic("")
			}
			for xi := xmp[v.f.x]; xi <= xmp[v.t.x]; xi++ {
				for yi := ymp[v.f.y]; yi <= ymp[v.t.y]; yi++ {
					mp[xi][yi] = 1
				}
			}
		}
		uf := newUnionFind(len(xsl) * len(ysl))
		dx := []int{1, 0, -1, 0, 1, 1, -1, -1}
		dy := []int{0, 1, 0, -1, 1, -1, 1, -1}
		for xi := 0; xi < len(xsl); xi++ {
			for yi := 0; yi < len(ysl); yi++ {
				if mp[xi][yi] == 0 {
					continue
				}
				for k := 0; k < 4; k++ {
					np := point{xi + dx[k], yi + dy[k]}
					if np.isValid(len(xsl), len(ysl)) && mp[xi][yi] == mp[np.x][np.y] {
						uf.unite(xi*len(ysl)+yi, np.x*len(ysl)+np.y)
					}
				}
			}
		}
		...
		for xi := 0; xi < len(xsl); xi++ {
			for yi := 0; yi < len(ysl); yi++ {
				...
				size := (xsl[xi+1] - xsl[xi]) * (ysl[yi+1] - ysl[yi])
				//	dbg(xi, yi, uf.root(xi*len(ysl)+yi), size)
				mp2[uf.root(xi*len(ysl)+yi)] += size
			}
		}
		rs := []int{}
		for _, v := range mp2 {
