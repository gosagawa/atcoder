...
func (g *graph) dijkstra(start int) [][2]int {

	used := make([]bool, g.size)
	score := make([][2]int, g.size)
	for i := 0; i < g.size; i++ {
		score[i] = [2]int{inf, inf}
	}
	score[start] = [2]int{0, 0}
	for i := 0; i < g.size; i++ {
		minDist := [2]int{inf, inf}
		minV := -1
		...
		for j := 0; j < g.size; j++ {
			...
			if score[j][0] > minDist[0] {
				continue
			}
			if score[j][0] == minDist[0] && score[j][1] > minDist[1] {
				continue
			}
			minDist = score[j]
			minV = j
		...
		for _, edge := range g.edges[minV] {
			nscore := [2]int{score[minV][0], score[minV][1] + edge.cost}
			if nscore[1] > l {
				nscore[0]++
				nscore[1] = edge.cost
			}
			if score[edge.to][0] < nscore[0] {
				continue
			}
			if score[edge.to][0] == nscore[0] && score[edge.to][1] < nscore[1] {
				continue
			}
			score[edge.to] = nscore
		}
		used[minV] = true
	}
	/*
