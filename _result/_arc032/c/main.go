...
func main() {
	...
	sort2ar(ns, opt2ar(1, desc))
	mx := ns[0][1]
	sort2ar(ns, opt2ar(0, asc))
	st := newlazystree(mx+1, stmax, stmax)
	//	out(ns)
	for i := n - 1; i >= 0; i-- {
		v := st.get(ns[i][1])
		st.rc(0, ns[i][0]+1, v+1)
		//		st.debug2()
	}
	ct := st.get(0)
	ti := inf
	...
	r := []int{}
	for i := 0; i < n; i++ {
		//		out(ns[i], ti, tii, from)
		if st.get(ns[i][0]) != ct && tii != inf {
			r = append(r, ti)
			from = ns[tii][1]
			ti = inf
			tii = inf
		...
		if ns[i][0] >= from && st.get(ns[i][1]) == ct-1 {
			if ns[i][2] < ti {
				ti = ns[i][2]
				tii = i
			}
