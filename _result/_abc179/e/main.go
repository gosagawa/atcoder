...
func main() {
	...
	n -= nonloopCount + loopCount
	loopSum := 0
	for i := loopFrom; i < loopFrom+loopCount; i++ {
		loopSum += vals[i]
	}
	//	out(loopFrom, vals[loopFrom], loopCount, nonloopCount, loopSum, n, n%loopCount)
	o += (n / loopCount) * loopSum

	for i := loopFrom; i < loopFrom+n%loopCount; i++ {
		o += vals[i]
	}
