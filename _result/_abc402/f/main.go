...
func main() {
	...
	for i := 0; i < n; i++ {
		for j := 0; j < n; j++ {
			mp[i][j] = mp[i][j] * ts[i+j] % m
		}
	}
	dbg(mp)
	ts1 := make([][]int, n)
	var dfs func(x, y, p int)
	dfs = func(x, y, p int) {
		if x >= n || y >= n {
			return
		}
		if x+y == n-1 {
			ts1[x] = append(ts1[x], p)
			return
		}
	...
	dfs(0, 0, 0)
	ts2 := make([][]int, n)
	var dfs2 func(x, y, p int)
	dfs2 = func(x, y, p int) {
		if x < 0 || y < 0 {
			return
		}
		if x+y == n-1 {
			ts2[x] = append(ts2[x], p)
			return
		}
	...
	for i := 0; i < n; i++ {
		...
		for _, v := range ts1[i] {
			tv := (v + mp[i][y]) % m
			r := lowerBound(m-tv, ts2[i])
			if r == -1 {
				r = lowerBound(m, ts2[i])
			}
			maxs(&o, (tv+ts2[i][r])%m)
		}
	}
