...
func main() {
	...
	mapping := func(f segfstruct, x segstruct) segstruct {
		...
		return segstruct{int(f) * x.size, x.size}
	}
	...
	for qi := 0; qi < q; qi++ {
		...
		c -= t
		dbg(t, c)
		ti := bs(0, n-1, func(i int) bool {
			return int(lst.get(i).v) < c
		})
		cc := int(lst.prod(t, t+1).v)
		...
		cv := int(lst.prod(l, r).v)
		lst.applyrange(l, r, segfstruct(c))
		nv := int(lst.prod(l, r).v)
		to := abs(cv - nv)
		dbg(l, r, cv, nv, to)
		o += to
	}
