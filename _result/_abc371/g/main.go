...
func main() {
	...
	for _, lp := range lps {
		...
		tm := int(tmp.Mod(t, big.NewInt(int64(lpl))).Int64())
		dbg("tc,tm", tc, tm)
		for {
			ci := int(tmp.Mod(tc, big.NewInt(int64(lpl))).Int64())
			dbg(ci, i)
			if used[ci] {
				break
			}
			...
			i++
			tc.Add(tc, big.NewInt(int64(tm)))
		}
		c.Add(c, tmp.Mul(t, big.NewInt(int64(mi))))
		t.Mul(t, big.NewInt(int64(i)))
		ti := 0
		for i, v := range lp {
			if mv == as[v] {
				ti = i
