...
func main() {
	...
	dps := make([][]int, len(n)+1)
	dpc := make([][]int, len(n)+1)
	for i := 0; i <= len(n); i++ {
		dps[i] = make([]int, 1<<10)
		dpc[i] = make([]int, 1<<10)
	}
	...
	for i := 0; i < len(n); i++ {
		for j := 1; j < (1 << 10); j++ {
			for k := 0; k < 10; k++ {
				dpc[i+1][j|1<<k] = madd(dpc[i+1][j|1<<k], dpc[i][j])
				dps[i+1][j|1<<k] = madd(dps[i+1][j|1<<k], dps[i][j]*10+dpc[i][j]*k)
			}
		}
		if i != 0 {
			for k := 1; k < 10; k++ {
				dpc[i+1][1<<k] = madd(dpc[i+1][1<<k], 1)
				dps[i+1][1<<k] = madd(dps[i+1][1<<k], k)
			}
			for k := 0; k < nsl[i]; k++ {
				dpc[i+1][digits|1<<k] = madd(dpc[i+1][digits|1<<k], 1)
				dps[i+1][digits|1<<k] = madd(dps[i+1][digits|1<<k], spre*10+k)
			}
		} else {
			for k := 1; k < nsl[i]; k++ {
				dpc[i+1][digits|1<<k] = madd(dpc[i+1][digits|1<<k], 1)
		...
		digits |= 1 << nsl[i]
		spre = madd(spre*10, nsl[i])

	}
	for j := 0; j < (1 << 10); j++ {
		if j&mask == mask && dps[len(n)][j] != 0 {
			o = madd(o, dps[len(n)][j])
		}
	}
	if digits&mask == mask {
		o = madd(o, spre)
