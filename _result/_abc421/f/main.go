...
func main() {
	...
	q := ni()
	sts := []st{}
	sts = append(sts, st{pre: -1, next: -1, v: 0})
	vis := is(q+1, 0)
	vis[0] = 0
	for i := 0; i < q; i++ {
		t := ni()
		...
		case 1:
			...
			if sts[ti].next == -1 {
				sts[ti].next = len(sts)
				sts = append(sts, st{pre: ti, next: -1, v: i + 1})
			} else {
				tnext := sts[ti].next
				sts[ti].next = len(sts)
				sts[tnext].pre = len(sts)
				sts = append(sts, st{pre: ti, next: tnext, v: i + 1})
			}
		case 2:
			x, y := ni2()
			ci := vis[x]
