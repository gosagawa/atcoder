...
func main() {
	...
	v := n*2 + 2
	edges := make([][]edge, v)
	setDualEdge(edges, 0, n+b, 2)
	setDualEdge(edges, n+a, v-1, 1)
	setDualEdge(edges, n+c, v-1, 1)
	for i := 0; i < n; i++ {
		setDualEdge(edges, i+1, n+i+1, 1)
	}
	for i := 0; i < m; i++ {
		setDualEdge(edges, n+us[i], vs[i], 1)
		setDualEdge(edges, n+vs[i], us[i], 1)
	}
