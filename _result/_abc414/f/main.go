...
func solve() {
	...
	for i := 0; i < n-1; i++ {
		...
		s--
		t--
		edges[s] = append(edges[s], edge{to: t, rev: i * 2})
		edges[t] = append(edges[t], edge{to: s, rev: i*2 + 1})
	}
	used := make([][]bool, (n-1)*2)
	for i := range used {
		used[i] = make([]bool, k)
	}
	dp := i2s(n, k, inf)
	dp[0][0] = 0
	q := list.New()
	q.PushBack([4]int{0, 0, 0, -1})
	e := q.Front()
	nc := i2s(n, k, 0)
	for e != nil {
		t := e.Value.([4]int)
		v, tk, c, p := t[0], t[1], t[2], t[3]
		if nc[v][tk] >= 2 {
			e = e.Next()
			continue
		}
		...
		for _, nv := range edges[v] {
			...
			ntk := (tk + 1) % k
			mins(&dp[nv.to][ntk], c+1)
			q.PushBack([4]int{nv.to, ntk, c + 1, v})
		}
		e = e.Next()
	}
	rs := is(n-1, 0)
