...
func main() {
	...
	for _, v := range dist1 {
		dbg(v)
		if v+1 > moc {
			dbg(v, n, dist2sum.get(0, n), (v+1)*n)
			o += dist2sum.get(0, n) + (v+1)*n
			continue
		}
		t := upperBound(moc-v-1, dist2)
		dbg(t)
		dbg(v, n, dist2sum.get(0, n), (v+1)*n)
		o += t*(moc-v-1) + dist2sum.get(t, n) + (v+1)*n
	}
	out(o)
