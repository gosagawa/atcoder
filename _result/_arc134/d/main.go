...
func main() {
	...
	for i, v := range ns {
		cns[i] = mp[v]
		if i < n {
			numis = append(numis, [2]int{cns[i], i})
			minf = min(minf, cns[i])
		} else {
			if cns[i-n] == minf && cns[i] <= minf && cns[i] < minn {
				minn = cns[i]
				minni = i
			}
		}
	...
	if minn != inf {
		out(fmt.Sprintf("%d %d", ns[minni-n], ns[minni]))
		return
	}

	r := []int{}
	sort2ar(numis, opt2ar(0, asc), opt2ar(1, asc))
	t := inf
	ci := -1
	hu := false
	chu := false
	for i := 0; i < n; i++ {
		...
		r = append(r, numis[i][1])
		ci = numis[i][1]
		if t == inf {
			t = cns[numis[i][1]+n]
		} else if !chu {
			if t == cns[numis[i][1]+n] {
			} else if t < cns[numis[i][1]+n] {
				hu = true
				chu = true
			} else {
				chu = true
	...
	rs := make([]string, len(r)*2)
	for i, v := range r {
		rs[i] = itoa(ns[v])
	}
	for i, v := range r {
		rs[i+len(r)] = itoa(ns[v+n])
	}
