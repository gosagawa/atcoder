...
func main() {
	...
	cs := []int{}
	for i := 0; i < n; i++ {
		fs[i] = timeToMilliseconds(ns())
		ts[i] = timeToMilliseconds(ns())
		cs = append(cs, fs[i])
		cs = append(cs, fs[i]-1)
		cs = append(cs, ts[i])
		cs = append(cs, ts[i]-1)
		cs = append(cs, ts[i]+1000)
	}
	sl, mp := cocom(cs)
	dbg(sl, mp)
...
func timeToMilliseconds(timeStr string) int {
