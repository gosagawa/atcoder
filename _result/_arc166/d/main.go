...
func main() {
	...
	pqa := newpq([]compFunc{func(p, q interface{}) int {
	...
	pqb := newpq([]compFunc{func(p, q interface{}) int {
		if p.(pqst).x != q.(pqst).x {
			// get from bigger
			// if p.(pqst).x > q.(pqst).x {
			if p.(pqst).x < q.(pqst).x {
	...
	for i := 0; i < n+1; i++ {
		c := ys[i+1] - ys[i]
		if c > 0 {
			heap.Push(pqa, pqst{x: i, y: c})
		} else if c < 0 {
			heap.Push(pqb, pqst{x: i, y: -c})
		}
	}
	for !pqa.IsEmpty() {
		va := heap.Pop(pqa).(pqst)
		vb := heap.Pop(pqb).(pqst)
		debug(va, vb, pqa.Len(), pqb.Len(), xs[vb.x]-xs[va.x])
		if xs[va.x] != -inf && xs[vb.x+1] != inf {
			mins(&o, xs[vb.x+1]-xs[va.x]-2)
		}
		use := min(va.y, vb.y)
		va.y -= use
		vb.y -= use
