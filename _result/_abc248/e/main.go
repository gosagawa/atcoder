...
func main() {
	...
	mp := make(map[[2]int]*unionFind)
	xcs := make(map[int]map[int]bool)
	ycs := make(map[int]map[int]bool)
	okmp := make(map[[2]int]map[int]bool)
	for i := 0; i < n; i++ {
		for j := i + 1; j < n; j++ {
			tx := xs[j] - xs[i]
			ty := ys[j] - ys[i]
			...
			cd := gcd(abs(tx), abs(ty))
			d := [2]int{tx / cd, ty / cd}
			if _, ok := mp[d]; !ok {
				mp[d] = newUnionFind(n)
				okmp[d] = make(map[int]bool)
			}
			mp[d].unite(i, j)
			if mp[d].size(i) >= k {
				okmp[d][mp[d].root(i)] = true
			}
