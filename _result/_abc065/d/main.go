...
func main() {
	...
	edges := make([][]edge, 1)
	edges[0] = make([]edge, 4*(n-1))
	c := 0

	sort.Slice(ns, func(i, j int) bool { return ns[i][1] < ns[j][1] })
	for i := 0; i < n-1; i++ {
		edges[0][c] = edge{from: ns[i][0], to: ns[i+1][0], cost: ns[i+1][1] - ns[i][1]}
		c++
	}
	sort.Slice(ns, func(i, j int) bool { return ns[i][2] < ns[j][2] })
	for i := 0; i < n-1; i++ {
		edges[0][c] = edge{from: ns[i][0], to: ns[i+1][0], cost: ns[i+1][2] - ns[i][2]}
		c++
	}
