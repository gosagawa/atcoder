...
func main() {
	...
	mp := make([][2]int, n)
	for i := 0; i < n; i++ {
		mp[i] = [2]int{rs[i] - 1, cs[i] - 1}
	}
	mp = append(mp, [2]int{0, 0})
	mp = append(mp, [2]int{h - 1, w - 1})
	n += 2

	sort2ar(mp, opt2ar(0, asc), opt2ar(1, asc))
	dbg(h, w, mp)
	lis := is(n, inf)
	id := is(n, 0)
	pre := is(n, 0)
	for i := 0; i < n; i++ {
		t := upperBound(mp[i][1], lis)
		lis[t] = mp[i][1]
		id[t] = i
		if t > 0 {
			pre[i] = id[t-1]
	...
	ans := []int{}
	ans = append(ans, id[t-1])
	for i := 1; i < t; i++ {
		ni := pre[ans[len(ans)-1]]
		ans = append(ans, ni)
	}
	reversei(ans)
	dbg(ans)
	...
	anss := []string{}
	for i := 1; i < len(ans); i++ {
		dbg(mp[ans[i]][0], mp[ans[i-1]][0], mp[ans[i]][1], mp[ans[i-1]][1])
		for j := 0; j < mp[ans[i]][0]-mp[ans[i-1]][0]; j++ {
			anss = append(anss, "D")
		}
		for j := 0; j < mp[ans[i]][1]-mp[ans[i-1]][1]; j++ {
			anss = append(anss, "R")
		}
	}
	outsj(anss)
