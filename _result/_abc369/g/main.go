...
func main() {
	...
	dfs = func(v, p int) {
		...
		for _, nv := range edges[v] {
			...
			dfs(nv.to, v)
			if sts[v][0] < sts[nv.to][0]+nv.cost {
				sts[v][1] = sts[nv.to][1]
				sts[v][0] = sts[nv.to][0] + nv.cost
			}
		}
	}
	dfs(0, -1)
	...
	heap.Init(pq)
	heap.Push(pq, pqst{x: sts[0][0], y: sts[0][1]})
	r := 0
	used := make([]bool, n)
	for i := 0; i < n; i++ {
		if pq.IsEmpty() {
		...
		for {
			...
			for _, nv := range edges[cy] {
				...
				heap.Push(pq, pqst{x: sts[nv.to][0] + nv.cost, y: sts[nv.to][1]})
			}
			cy = ps[cy]
			if cy == -1 {
				break
