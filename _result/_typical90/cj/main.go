...
func main() {
	...
	r := make([][2]int, 8889)
	var f func(i, sum, s1, s2 int) (int, int, int)
	f = func(i, sum, s1, s2 int) (int, int, int) {
		//out(i, sum, s1, s2)
		if i == n {
			return 0, 0, 0
		}
		...
		if useable {
			...
			if r[nsum][0] == 0 && r[nsum][1] == 0 {
				r[nsum] = [2]int{ns1, ns2}
				r1, r2, s := f(i+1, nsum, ns1, ns2)
				if r1 != 0 || r2 != 0 {
					return r1, r2, s
				}
			} else {
		...
		if sum != 0 && (r[sum][0] != s1 || r[sum][1] != s2) {
			return s1, s2, sum
		}
		if sum != 0 {
			r[sum] = [2]int{s1, s2}
		}
		r1, r2, s := f(i+1, sum, s1, s2)
		if r1 != 0 || r2 != 0 {
			return r1, r2, s
		}
		return 0, 0, 0
	...
	ra1 := itor(r1, r2)
	ra2 := itor(r[sum][0], r[sum][1])
	out(len(ra1))
	out(strings.Join(ra1, " "))
	out(len(ra2))
	out(strings.Join(ra2, " "))
