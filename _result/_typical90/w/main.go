...
func main() {
	...
	h, w := ni2()
	dp := make([]map[int]bool, 2*w+2)
	mvb := make([]map[int]int, w)
	mvw := make([]map[int]int, w)
	mvi := make(map[int]bool)
	...
	for i := 0; i < w; i++ {
		mvb[i] = make(map[int]int)
		mvw[i] = make(map[int]int)
	}
	for i := 0; i < 2*w+1; i++ {
	...
	mvwi := make([][][2]int, w)
	for i := 0; i < w; i++ {
		mvbi[i] = make([][2]int, len(mvb[i]))
		k := 0
		for j, v := range mvb[i] {
			mvbi[i][k] = [2]int{idrs[j], idrs[v]}
			k++
		}
		k = 0
		mvwi[i] = make([][2]int, len(mvw[i]))
		for j, v := range mvw[i] {
			mvwi[i][k] = [2]int{idrs[j], idrs[v]}
			k++
		}
	}
	...
	dp2 := make([][]int, 2)
	dp2[0] = make([]int, idxsl)
	dp2[1] = make([]int, idxsl)
	dp2[0][0] = 1
	for i := 0; i < h*w; i++ {
		dpf := i % 2
		dpt := (i + 1) % 2
		dp2[dpt] = make([]int, idxsl)
		nidx := i % w
		for _, v := range mvwi[nidx] {
			dp2[dpt][v[1]] = madd(dp2[dpt][v[1]], dp2[dpf][v[0]])
		}
		if m[i] {
			for _, v := range mvbi[nidx] {
				dp2[dpt][v[1]] = madd(dp2[dpt][v[1]], dp2[dpf][v[0]])
			}
		}
	}
	for _, v := range dp2[h*w%2] {
		o = madd(o, v)
	}
