...
func main() {
	...
	largeslen := len(larges)

	link := make([][]bool, v)
	for i := 0; i < v; i++ {
		link[i] = make([]bool, largeslen)
	}
	largers := make([]int, v)
	for i, v := range larges {
	...
	updates := isl(v, -1)
	updatelarges := isl(largeslen, -1)
	for i := 0; i < q; i++ {
		last := updates[xs[i]]
		for j := 0; j < largeslen; j++ {
			if link[xs[i]][j] {
				last = max(last, updatelarges[j])
			}
		}
		if last == -1 {
			out(1)
		...
		if len(edges[xs[i]]) < b {
			updates[xs[i]] = i
			for _, nv := range edges[xs[i]] {
				updates[nv.to] = i
			}
		} else {
			updatelarges[largers[xs[i]]] = i
		}
	}
}
