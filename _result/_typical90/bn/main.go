...
func main() {
	...
	maxn := 101
	bit := newbit(maxn)
	l := float64(rs[0] - ls[0] + 1)
	for i := ls[0]; i <= rs[0]; i++ {
		bit.add(i, 1/l)
	}
	for i := 0; i < n-1; i++ {
		l := float64(rs[i+1] - ls[i+1] + 1)
		for k := ls[i+1]; k <= rs[i+1]; k++ {
			o += float64(bit.sum(maxn-1)-bit.sum(k)) / l
		}
		for k := ls[i+1]; k <= rs[i+1]; k++ {
			bit.add(k, 1/l)
		}
...
func (b *bit) add(i int, x float64) {
	for i++; i < b.n && i > 0; i += i & -i {
		b.b[i] += x
	}
}

func (b *bit) sum(i int) float64 {
	ret := 0.0
	for i++; i > 0; i -= i & -i {
		ret += b.b[i]
	}
...
func (b *bit) rangesum(l, r int) float64 {
	return b.sum(r-1) - b.sum(l-1)
}

func (b *bit) lowerBound(x float64) int {
	idx, k := 0, 1
	for k < b.n {
		k <<= 1
	}
