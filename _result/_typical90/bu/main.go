...
func main() {
	...
	dfs = func(v, p int) []int {
		...
		if len(edges[v]) == 1 && edges[v][0].to == p {
		} else {
			for _, nv := range edges[v] {
				...
				cr := dfs(nv.to, v)
				var r1, r2, r3 int
				r3 = madd(r3, mmul(r[3], cr[3]))
				r3 = madd(r3, mmul(r[3], cr[2]))
				r3 = madd(r3, mmul(r[3], cr[1]))
				r3 = madd(r3, mmul(r[2], cr[3]))
				r2 = madd(r2, mmul(r[2], cr[2]))
				r3 = madd(r3, mmul(r[2], cr[1]))
				r3 = madd(r3, mmul(r[1], cr[3]))
				r3 = madd(r3, mmul(r[1], cr[2]))
				r1 = madd(r1, mmul(r[1], cr[1]))

				r3 = madd(r3, mmul(r[3], cr[3]))
				r2 = madd(r2, mmul(r[2], cr[3]))
				r1 = madd(r1, mmul(r[1], cr[3]))
				r[1] = r1
				r[2] = r2
				r[3] = r3
			}
