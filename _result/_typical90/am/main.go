...
func newtree(size int, root int, edges [][]edge) *tree {
	...
	depth := make([]int, size)
	order := make([]int, size)
	childcnt := make([]int, size)
	discnt := make([]int, size)
	return &tree{
		size:       size,
		root:       root,
		edges:      edges,
...
func (t *tree) init() {
	_, _ = t.dfs(t.root, -1, 0)
}

func (t *tree) dfs(v, p, d int) (int, int) {
	c := 0
	di := 0
	for _, nv := range t.edges[v] {
		if nv.to != p {
			tc, td := t.dfs(nv.to, v, d+1)
			c += tc
			di += td
...
func (t *tree) solve(v, p, dis int) {
	t.discnt[v] = dis
	for _, nv := range t.edges[v] {
		if nv.to != p {
			t.solve(nv.to, v, dis+t.size-2*(t.childcnt[nv.to]+1))
		}
	}
}
