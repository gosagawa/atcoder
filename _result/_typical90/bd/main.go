...
func main() {
	...
	for i := 0; i < n; i++ {
		as[i], bs[i] = ni2()
		used[i] = make([]bool, 100001)
	}
	used[n] = make([]bool, 100001)
	var dfs func(i, sum int, st string) string
	dfs = func(i, sum int, st string) string {
		if used[i][sum] {
			return "Impossible"
		}
		used[i][sum] = true
		...
		if sum+as[i] <= s {
			ra = dfs(i+1, sum+as[i], st+"A")
			if ra != "Impossible" {
				return ra
			}
		}
		if sum+bs[i] <= s {
			rb = dfs(i+1, sum+bs[i], st+"B")
			if rb != "Impossible" {
				return rb
			}
		}
