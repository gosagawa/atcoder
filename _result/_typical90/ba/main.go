...
func main() {
	...
	for i := 0; i < t; i++ {
		...
		ps = append(ps, culcf(tof-fromf))
		ns = append(ns, get(int(math.Round(ps[0]))))

		ps = append(ps, tof-culcf(tof-fromf))
		ns = append(ns, get(int(math.Round(ps[1]))))
		lidx := 0
		ridx := 1
		nidx := 2
		target := 13
		for {
			...
			if ns[ridx] > ns[lidx] {
				fromf = ps[lidx]
				from = int(math.Round(ps[lidx]))
				np := tof - culcf(tof-fromf)
				ps = append(ps, np)
				lidx = ridx
				ridx = nidx
			} else {
				tof = ps[ridx]
				to = int(math.Round(ps[ridx]))
				np := fromf + culcf(tof-fromf)
				ps = append(ps, np)
				ridx = lidx
				lidx = nidx
			}
			ns = append(ns, get(int(math.Round(ps[nidx]))))
			nidx++
			target--
		}
	}
...
func culcf(f float64) float64 {
	f *= 2
	f /= 3 + math.Sqrt(5)
	return f
}

// https://play.golang.org/p/juBLBJQWq40
func culc(a int) int {
	f := float64(a)
	f *= 2
	f /= 3 + math.Sqrt(5)
