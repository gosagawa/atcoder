...
func main() {
	...
	for i := 0; i < n; i++ {
		s := i + 1
		k := ni()
		setDualEdge(edges, 0, s, as[i])
		setDualEdge(edges, s, n+1, w)
		for j := 0; j < k; j++ {
			t := ni()
			setDualEdge(edges, t, s, inf)
			edges[t] = append(edges[t], edge{to: s, cost: inf, rev: len(edges[s])})
			edges[s] = append(edges[s], edge{to: t, cost: 0, rev: len(edges[t]) - 1})
		}
	}

	graph := newgraph(n+2, edges)

	out(assum - graph.dinic())
}
