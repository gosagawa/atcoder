...
func main() {
	...
	tbs2 := make([][]int, 0)
	for i := 0; i < n; i++ {
		if i == 0 || as[i]%2 != as[i-1]%2 {
			tas2 = append(tas2, []int{as[i]})
		} else {
			tas2[len(tas2)-1] = append(tas2[len(tas2)-1], as[i])
		}
		if i == 0 || bs[i]%2 != bs[i-1]%2 {
			tbs2 = append(tbs2, []int{bs[i]})
		} else {
			tbs2[len(tbs2)-1] = append(tbs2[len(tbs2)-1], bs[i])
		}
	}
	debug(tas2)
	debug(tbs2)
	if len(tas2) != len(tbs2) {
		out("No")
		return
	}
	for i := 0; i < len(tas2); i++ {
		if tas2[i][0]%2 == 0 && len(tas2[i]) >= 3 {
			sorti(tas2[i])
			sorti(tbs2[i])
		}
		if !issame(tas2[i], tbs2[i]) {
			out("No")
			return
		}
	}
