...
func main() {
	...
	_, mp := cocom(sl)
	//	out(mp)
	st1 := newstree(len(mp), stmin)
	st2 := newstree(len(mp), stmax)
	st1.set(mp[A-B], A-B)
	st2.set(mp[A-B], A-B)
	st1.set(mp[A+B], A+B)
	st2.set(mp[A+B], A+B)
	for i := 0; i < q; i++ {
		t, a, b := qa[i][0], qa[i][1], qa[i][2]
		//		st1.debug()
		//		st2.debug()
		if t == 1 {
			st1.set(mp[a-b], a-b)
