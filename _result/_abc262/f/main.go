...
func main() {
	...
	for {
		...
		tm := st.query(t, min(t+tk+1, n))
		//		out(tm, t, tk, t, t+tk+1)
		r1 = append(r1, tm)
		tk -= is[tm] - t
		t = is[tm] + 1
	}
	...
	if fs == -1 {
		...
		for {
			...
			if max(fl+tk+1, t+tk+1) > n {
				break
			}
			tm := st.query(t, min(max(fl+tk+1, t+tk+1), n))
			//out(tm, tk, fl+tk+1, t+tk+1)
			r2 = append(r2, tm)
			if is[tm] > fl {
				tk -= is[tm] - max(fl, t)
			}
