...
func main() {
	...
	nsumc := func(m, i, j int) int {
		return nsum[m][j] - nsum[m][i]
	}

	var solve func(r int) [2]int
	solve = func(r int) [2]int {
		...
		for i := 0; i < m; i++ {
			...
			b := r ^ (1 << i)
			if b == 0 {
				dp[r][0] = nsumc(i, 0, ms[i])
				dp[r][1] = ms[i]
			} else {
				br := solve(b)
				dp[r][0] = min(dp[r][0], br[0]+nsumc(i, br[1], br[1]+ms[i]))
				dp[r][1] = br[1] + ms[i]
			}
		}
		return dp[r]
