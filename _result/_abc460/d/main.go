...
func main() {
	...
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			if mp[i][j] == 0 {
				ts1[i][j] = 0
				q1.PushBack(point{i, j})
			} else {
				ts2[i][j] = 0
				q2.PushBack(point{i, j})
			}
		}
	}
	...
	for e != nil {
		...
		for i := 0; i < 8; i++ {
			...
			if np.isValid(h, w) && ts1[np.x][np.y] == inf {
				ts1[np.x][np.y] = ts1[p.x][p.y] + 1
				q1.PushBack(np)
			}
		}
	...
	for e != nil {
		...
		for i := 0; i < 8; i++ {
			np := point{p.x + dx[i], p.y + dy[i]}
			if np.isValid(h, w) && ts2[np.x][np.y] == inf {
				ts2[np.x][np.y] = ts2[p.x][p.y] + 1
				q2.PushBack(np)
			}
		}
