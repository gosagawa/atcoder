...
func main() {
	...
	n, m := ni2()
	as := ni2d(n, n)
	cusum2d := newcusum2d(as)
	mas := i2s(n-m+1, n-m+1, 0)
	for i := 0; i < n-m+1; i++ {
		for j := 0; j < n-m+1; j++ {
			mas[i][j] = cusum2d.get(i, j, i+m-1, j+m-1)
		}
	}
	rmm := make([][][]int, 4)
	for k := 0; k < 4; k++ {
		rmm[k] = i2s(n-m+1, n-m+1, 0)
		for i := 0; i < n-m+1; i++ {
			for j := 0; j < n-m+1; j++ {
				rmm[k][i][j] = mas[i][j]
				if i != 0 {
					maxs(&rmm[k][i][j], rmm[k][i-1][j])
				}
				if j != 0 {
					maxs(&rmm[k][i][j], rmm[k][i][j-1])
				}
			}
		}
		for i := 0; i < (4-k)%4; i++ {
	...
	for k := 0; k < 4; k++ {
		...
		for i := 0; i < n-m+1; i++ {
			for j := 0; j < n-m+1; j++ {
				...
				maxs(&t, rmm[k][i][j]+rmm[(k+1)%4][i2][j2]+rmm[(k+2)%4][i3][j3])
			}
		}
		maxs(&o, t)
	}
	for k := 0; k < 2; k++ {
		...
		//debug(s.query(0, 4))
		for i := 0; i < n-m+1; i++ {
			for j := i + m*2; j < n-m+1; j++ {
				//debug(i, j, s.query(0, i+1), s.query(i+m, j), s.query(j, n-m+1))
				maxs(&o, s.query(0, i+1)+s.query(i+m, j-m+1)+s.query(j, n-m+1))
			}
		}
