...
func main() {
	...
	rs := [][2]int{}
	if ns[0] >= 0 {
		rs = append(rs, [2]int{ns[0], ns[1]})
		ns[1] = ns[0] - ns[1]
		ns = ns[1:]
	} else if ns[len(ns)-1] < 0 {
		rs = append(rs, [2]int{ns[len(ns)-1], ns[len(ns)-2]})
		ns[len(ns)-2] = ns[len(ns)-1] - ns[len(ns)-2]
		ns = ns[:len(ns)-1]
	}
	debug(ns)
	n = len(ns)
	...
	for i := n - 1; i > 0; i-- {
		...
		rs = append(rs, [2]int{ns[0], ns[i]})
		ns[0] -= ns[i]
	}
	for i := 0; i < ti; i++ {
		rs = append(rs, [2]int{ns[ti], ns[i]})
		ns[ti] -= ns[i]
	}
	o = ns[ti]
