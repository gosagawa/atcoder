...
func main() {
	...
	cs := [2][][2]int{}
	if ga < gb {
		cs[0] = append(cs[0], [2]int{ga, gb})
	} else {
		cs[1] = append(cs[1], [2]int{gb, ga})
	}
	if sa < sb {
		cs[0] = append(cs[0], [2]int{sa, sb})
	} else {
		cs[1] = append(cs[1], [2]int{sb, sa})
	}
	if ba < bb {
		cs[0] = append(cs[0], [2]int{ba, bb})
	} else {
		cs[1] = append(cs[1], [2]int{bb, ba})
	}
	for k := 0; k <= 1; k++ {
		switch len(cs[k]) {
		case 0:
		case 1:
			o = n%cs[k][0][0] + n/cs[k][0][0]*cs[k][0][1]
			n = o
		case 2:
			for i := 0; i <= n/cs[k][0][0]; i++ {
				t := n
				t -= cs[k][0][0] * i
				j := t / cs[k][1][0]
				t %= cs[k][1][0]
				maxs(&o, t+cs[k][0][1]*i+cs[k][1][1]*j)
			}
			n = o
		case 3:
			for i := 0; i <= n/cs[k][0][0]; i++ {
				t := n
				t -= cs[k][0][0] * i
				for j := 0; j <= t/cs[k][1][0]; j++ {
					t -= cs[k][1][0] * j
					l := t / cs[k][2][0]
					t -= cs[k][2][0] * l
					maxs(&o, t+cs[k][0][1]*i+cs[k][1][1]*j+cs[k][2][1]*l)
					t += sa * j
					t += ba * l
				}
			}
