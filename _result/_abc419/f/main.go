...
func main() {
	...
	for i := 0; i < n; i++ {
		for j := 0; j < n; j++ {
			...
			if strings.Contains(tss[i], tss[j]) {
				isSub[j] = true
			}
		}
	}
	...
	ssm := make(map[int]int)
	pfs := []string{""}
	pfsm := make(map[string]int)
	pfsm[""] = 0
	for i := 0; i < n; i++ {
		for j := 0; j < len(ss[i]); j++ {
			pfs = append(pfs, string(ss[i][:j+1]))
			pfsm[string(ss[i][:j+1])] = len(pfs) - 1
			if j == len(ss[i])-1 {
				ssm[len(pfs)-1] = i
			}
		}
