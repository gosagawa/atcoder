...
func solve() {
	o := 0
	c, d := ni2()
	cc := atoi(itoa(c) + itoa(c+1))
	cd := atoi(itoa(c) + itoa(c+d))

	for i := 0; i < 10; i++ {
		st := atoi(itoa(c) + itoa(pow10(i)))
		en := atoi(itoa(c) + itoa(pow10(i+1)-1))
		if en < cc {
			continue
		}
		if st > cd {
...
func count(a, b int) int {
	dbg(a, b)
	fn1 := func(i int) bool {
		return safemul(i, i) < a
	}
	fn2 := func(i int) bool {
		return safemul(i, i) <= b
	}
	t1 := bs(0, sqrt(a)+10, fn1)
	t2 := bs(t1, sqrt(b)+10, fn2)
	dbg(t1, t2)
	return t2 - t1
}
