...
func main() {
	...
	for i := 0; i < 100; i++ {
		...
		for j := 0; j < 100; j++ {
			dp[i][j] = make([]float64, 100)
		}
	}
	dp[a][b][c] = 1.0
	for i := 0; i < 300-2-a-b-c; i++ {
		for j := 0; j <= i; j++ {
			for k := 0; k <= i-j; k++ {
				ac := a + j
				bc := b + k
				...
				if ac == 99 {
					o += dp[ac][bc][cc] * float64((i+1)*ac) / float64(sum)
				} else if ac != 0 {
					dp[ac+1][bc][cc] += dp[ac][bc][cc] * float64(ac) / float64(sum)
				}
				if bc == 99 {
					o += dp[ac][bc][cc] * float64((i+1)*bc) / float64(sum)
				} else if bc != 0 {
					dp[ac][bc+1][cc] += dp[ac][bc][cc] * float64(bc) / float64(sum)
				}
				if cc == 99 {
					o += dp[ac][bc][cc] * float64((i+1)*cc) / float64(sum)
				} else if cc != 0 {
					dp[ac][bc][cc+1] += dp[ac][bc][cc] * float64(cc) / float64(sum)
				}
			}
		}
	}
