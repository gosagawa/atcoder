...
func main() {
	...
	for qi := 0; qi < q; qi++ {
		...
		il := upperBound(l-1, is[1])
		ir := lowerBound(r+1, is[2])
		f := upperBound(l-1, is[0])
		t := lowerBound(r+1, is[0])
		if t < f {
			out(0)
			continue
		}
		o := bs(0, r-l, func(c int) bool {
			if c == 0 {
				return true
			}
			tl := il + c - 1
			...
			f := upperBound(is[1][tl], is[0])
			t := lowerBound(is[2][tr], is[0])
			//		dbg(c, tl, tr, is[1][tl], is[2][tr], f, t, t >= f)
			return t >= f
		})
		out(o*2 + 1)
	}
