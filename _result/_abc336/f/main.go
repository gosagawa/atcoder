...
func main() {
	...
	rotete := func(x, y int, mp [][]int) [][]int {
		rmp := i2s(h, w, 0)
		for i := 0; i < h; i++ {
			for j := 0; j < w; j++ {
				rmp[i][j] = mp[i][j]
		...
		for i := 0; i < h-1; i++ {
			for j := 0; j < w-1; j++ {
				rmp[h-2-i+x][w-2-j+y] = mp[i+x][j+y]
			}
		}
	...
	for e != nil {
		...
		for i := 0; i < 2; i++ {
			for j := 0; j < 2; j++ {
				nmp := rotete(i, j, mp)
				nmpar := convar(nmp)
				if _, ok := dist[nmpar]; ok {
					continue
				}
				dist[nmpar] = dist[ar] + 1
				if dist[nmpar] < 10 {
					q.PushBack(nmpar)
				}
			}
	...
	if v, ok := dist[glar]; ok {
		out(v)
		return
	}

	q = list.New()
	dist2 := make(map[[64]int]int)
	dist2[glar] = 0
	q.PushBack(glar)
	e = q.Front()
	for e != nil {
		...
		for i := 0; i < 2; i++ {
			for j := 0; j < 2; j++ {
				nmp := rotete(i, j, mp)
				nmpar := convar(nmp)
				if _, ok := dist2[nmpar]; ok {
					continue
				}
				dist2[nmpar] = dist2[ar] + 1
				if dist2[nmpar] < 10 {
					q.PushBack(nmpar)
				}
			}
