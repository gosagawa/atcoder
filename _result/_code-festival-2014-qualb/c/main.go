...
func main() {
	...
	dp := make([][2]int, 27)
	for i := 0; i < 26; i++ {
		if s1s[i]+s2s[i] < s3s[i] {
			out("NO")
			return
		}
		s1max := min(min(n/2, s1s[i]), s3s[i])
		s2max := min(min(n/2, s2s[i]), s3s[i])
		dp[i+1][0] = dp[i][0] - 2*s2max + s3s[i]
		dp[i+1][1] = dp[i][1] + 2*s1max - s3s[i]
	}
	if dp[26][0] > 0 || dp[26][1] < 0 {
		out("NO")
		return
	}
	out("YES")
