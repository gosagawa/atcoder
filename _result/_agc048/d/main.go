...
func main() {
	...
	mses := make([][][2]int, n)
	for i := 0; i < n; i++ {
		sems[cm[ss[i]]] = append(sems[cm[ss[i]]], [2]int{ms[i] - 1, cm[es[i]]})
		mses[ms[i]-1] = append(mses[ms[i]-1], [2]int{cm[ss[i]], cm[es[i]]})
	}
	for i := 0; i < n; i++ {
		sort2ar(mses[i], opt2ar(1, asc))
	}

	dp := make([]int, li+1)
	for i := 0; i < li; i++ {
		for _, v := range sems[i] {
			te := v[1]
			th := 0
			maxs(&dp[te], dp[i]+chs.get(th))
			for _, v2 := range mses[v[0]] {
				if te <= v2[0] {
					th++
					te = v2[1]
					maxs(&dp[te], dp[i]+chs.get(th))
