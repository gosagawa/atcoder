...
func main() {
	...
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			...
			for k := 0; k < 4; k++ {
				np := point{x: i + dx[k], y: j + dy[k]}
				if np.isValid(h, w) && mp[np.x][np.y] == 2 {
					uf.unite(i*w+j, np.x*w+np.y)
				}
			}
		}
	}
	mp2 := make(map[int]map[int]bool)
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			if mp[i][j] != 2 {
				continue
			...
			idx := uf.root(i*w + j)
			if _, ok := mp2[idx]; !ok {
				mp2[idx] = make(map[int]bool)
			}
			mp2[idx][i*w+j] = true

			for k := 0; k < 4; k++ {
				np := point{x: i + dx[k], y: j + dy[k]}
				if np.isValid(h, w) && mp[np.x][np.y] == 0 {
					mp2[idx][np.x*w+np.y] = true
				}
			}
		}
	}
