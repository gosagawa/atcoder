...
func main() {
	...
	for i := 0; i < n; i++ {
		...
		ts = append(ts, y+x)
		if (x+y)%2 == 0 {
			sl1 = append(sl1, [2]int{y - x, y + x})
		} else {
			sl2 = append(sl2, [2]int{y - x, y + x})
		}
	}
	_, mp := cocom(ts)
	mpl := len(mp)
	...
	sum := func(sl [][2]int) int {
		...
		for i := len(sl) - 1; i >= 0; i-- {
			...
			sxv := bitx.rangesum(xi, mpl)
			sxvc := bitcx.rangesum(xi, mpl)
			r += sxv - sxvc*sl[i][0]
			sxv = bitx.rangesum(0, xi)
			sxvc = bitcx.rangesum(0, xi)
			r += sxvc*x - sxv

			syv := bity.rangesum(yi, mpl)
			syvc := bitcy.rangesum(yi, mpl)
			r += syv - syvc*sl[i][1]
			syv = bity.rangesum(0, yi)
			syvc = bitcy.rangesum(0, yi)
			r += syvc*y - syv
