...
func main() {
	...
	dbl := func(sl []int) [][2]int {
		r := make([][2]int, n)
		for i := 0; i < n; i++ {
			r[i] = [2]int{0, i}
		}
		next := make([][][2]int, maxlogn+1)
		for k := 0; k < maxlogn+1; k++ {
			next[k] = make([][2]int, n)
		}
		for i := 0; i < n; i++ {
			t := 0
			...
			next[0][i] = [2]int{t, sl[i]}
		}
		for k := 0; k < maxlogn; k++ {
			for i := 0; i < n; i++ {
				nc := next[k][i][0]
				ni := next[k][next[k][i][1]][1]
				if ni <= next[k][i][1] {
					nc++
				}
				next[k+1][i] = [2]int{nc, ni}
			}
		}
