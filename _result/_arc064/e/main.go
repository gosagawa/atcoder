...
func main() {
	...
	edges := make([][]edge2, n+2)

	culc := func(x1, y1, r1, x2, y2, r2 int) float64 {
		t := pointDist(point{x1, y1}, point{x2, y2})
		t -= float64(r1) + float64(r2)
		if t < 0 {
			t = 0
		}
		return t
	...
	s = 0
	t = n + 1
	c = culc(xs, ys, 0, xt, yt, 0)
	edges[s] = append(edges[s], edge2{to: t, cost: c})
	edges[t] = append(edges[t], edge2{to: s, cost: c})
	for i := 0; i < n; i++ {
		s = 0
		t = i + 1
		c = culc(xs, ys, 0, xyr[i][0], xyr[i][1], xyr[i][2])
		edges[s] = append(edges[s], edge2{to: t, cost: c})
		edges[t] = append(edges[t], edge2{to: s, cost: c})

		s = i + 1
		t = n + 1
		c = culc(xt, yt, 0, xyr[i][0], xyr[i][1], xyr[i][2])
		edges[s] = append(edges[s], edge2{to: t, cost: c})
		edges[t] = append(edges[t], edge2{to: s, cost: c})
		...
		for j := i + 1; j < n; j++ {
			...
			s = i + 1
			t = j + 1
			c = culc(xyr[i][0], xyr[i][1], xyr[i][2], xyr[j][0], xyr[j][1], xyr[j][2])
			edges[s] = append(edges[s], edge2{to: t, cost: c})
			edges[t] = append(edges[t], edge2{to: s, cost: c})
		}
	}
...
func newgraph(size int, edges [][]edge2) *graph {
	graph := &graph{
		size:  size,
		edges: edges,
	}
