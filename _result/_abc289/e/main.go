...
func main() {
	...
	for qi := 0; qi < q; qi++ {
		...
		for ee != nil {
			...
			for _, nv1 := range edges[p.x] {
				for _, nv2 := range edges[p.y] {
					if cs[nv1.to] == cs[nv2.to] {
						continue
					}
					if used[nv1.to][nv2.to] == 1 {
						continue
					}
					if rs[nv1.to][nv2.to] <= rs[p.x][p.y]+1 {
						continue
					}
					rs[nv1.to][nv2.to] = rs[p.x][p.y] + 1
					q.PushBack(point{nv1.to, nv2.to})
				}
			}
			ee = ee.Next()
		}
