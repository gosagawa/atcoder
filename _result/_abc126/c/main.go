...
func main() {
	...
	n, k := ni2()
	for i := 1; i <= n; i++ {
		//out(i, k/i, math.Log2(float64(k)/float64(i)), math.Ceil(math.Log2(float64(k/i))))
		if i >= k {
			o += 1 / float64(n)
		} else {
			o += 1 / float64(n) * math.Pow(0.5, math.Ceil(math.Log2(float64(k)/float64(i))))
			//o += 1 / float64(n) * math.Pow(0.5, float64(int((math.Log2(float64(k*2/i))))))
		}
	}
	out(o)
}
