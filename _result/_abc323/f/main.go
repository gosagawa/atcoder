...
func main() {
	...
	b := point{xb, yb}
	c := point{xc, yc}
	bcsub1 := point{b.x, b.y}
	bcsub2 := point{b.x, b.y}
	to := abs(c.x-b.x) + abs(c.y-b.y)
	if b.x != c.x && b.y != c.y {
		to += 2
	}
	...
	fm := func(f, t point) int {
		...
		return tv + abs(f.x-t.x) + abs(f.y-t.y)
	}
	debug(bcsub1, bcsub2, fm(a, bcsub1), fm(a, bcsub2))
	if b.x != c.x {
		mins(&o, to+fm(a, bcsub2))
	}
	if b.y != c.y {
		mins(&o, to+fm(a, bcsub1))
	}
