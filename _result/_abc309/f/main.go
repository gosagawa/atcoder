...
func main() {
	...
	for i := 0; i < n; i++ {
		hwd[i] = nis(3)
		sorti(hwd[i])
		ts = append(ts, hwd[i][0])
		ts = append(ts, hwd[i][1])
		ts = append(ts, hwd[i][2])
	}
	sl, mp := cocom(ts)
	nhwd := make([][][2]int, len(sl))
	for i := 0; i < n; i++ {
		nhwd[mp[hwd[i][0]]] = append(nhwd[mp[hwd[i][0]]], [2]int{mp[hwd[i][1]], mp[hwd[i][2]]})
	}
	s := newstree(len(sl)+1, stmin)
	for _, v := range nhwd {
		for _, v2 := range v {
			r := s.query(-1, v2[0])
			if r < v2[1] {
		...
		for _, v2 := range v {
			s.set(v2[0], min(s.query(v2[0], v2[0]+1), v2[1]))
		}
