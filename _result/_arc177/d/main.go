...
func main() {
	...
	sort2ar(xis, opt2ar(0, asc))
	debug(xis)
	tis := [][]int{}
	ps := make([][2]int, n)
	for i := 0; i < n; i++ {
		if i == 0 || xis[i-1][0]+h < xis[i][0] {
			tis = append(tis, []int{xis[i][1]})
		} else {
			tis[len(tis)-1] = append(tis[len(tis)-1], xis[i][1])
		}
		ps[xis[i][1]] = [2]int{len(tis) - 1, len(tis[len(tis)-1]) - 1}
	}
	debug(tis)
	debug(ps)
	mi := 0
	ttr := make([][]int, len(tis))
	for i, v := range tis {
		t := inf
		for j, v2 := range v {
			if j == 0 || j == len(v)-1 {
	...
	tcs := make([]int, len(tis))
	ttcs := make([]int, len(tis))
	for i := 0; i < n; i++ {
		idx := ps[i][0]
		...
		if ttcs[idx] == 0 {
			...
			t[0] = mdiv(t[0], ttcs[idx])
		}
		ttcs[idx] += ttr[idx][tcs[idx]]
		ttcs[idx] %= mod
		if ttcs[idx] == 0 {
			t[1]++
		} else {
			t[0] = mmul(t[0], ttcs[idx])
		}

		if ttcs[idx] == 0 {
			if t[1] == 1 {
				rs[i] = mmul(t[0], ttr[idx][tcs[idx]])
			} else {
				rs[i] = 0
			}
		} else {
...
func subc(sl []int) []int {
	...
	for i := 0; i < n; i++ {
		...
		if idx == 0 || sl[idx-1] < sl[idx] {
			c := tsl[idx] + tsr[idx+1]
			r[i] += mpow(2, n-c, mod)
			r[i] %= mod
		}
		if idx == n-1 || sl[idx+1] < sl[idx] {
			c := tsl[idx+1] + tsr[idx+2]
			r[i] += mpow(2, n-c, mod)
			r[i] %= mod
		}
	}
...
func getts(sl []int) ([]int, []int) {
	...
	ts1 = append([]int{0}, ts1...)
	ts2 = append([]int{0}, ts2...)
	ts1 = append(ts1, 0)
	ts2 = append(ts2, 0)
