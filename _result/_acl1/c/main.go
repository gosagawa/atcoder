...
func main() {
	...
	o := 0
	n, m := ni2()
	mp := convidxi2s(nsi2s(n), map[string]int{".": 0, "#": 1, "o": 2})

	mg := NewMcfGraph(n*m + 2)
	s := n * m
	t := n*m + 1
	dx := []int{1, 0}
	dy := []int{0, 1}
	...
	for i := 0; i < n; i++ {
		for j := 0; j < m; j++ {
			...
			mg.AddEdge(p1, t, 1, inf-i-j)

			for k := 0; k < 2; k++ {
				npt := pointAdd(pt, point{dx[k], dy[k]})
				if !npt.isValid(n, m) || mp[npt.x][npt.y] == 1 {
					continue
				}
				p2 := npt.x*m + npt.y
				mg.AddEdge(p1, p2, n*m, 0)
			}
		}
	}
