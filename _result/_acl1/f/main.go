...
func main() {
	...
	o := inf
	n := ni()
	xm := make(map[int][][2]int)
	ym := make(map[int][][2]int)
	xy1m := make(map[int][][2]int)
	xy2m := make(map[int][][2]int)
	xy3m := make(map[int][][2]int)
	xy4m := make(map[int][][2]int)
	for i := 0; i < n; i++ {
		x, y := ni2()
		x *= 5
		y *= 5
		...
		case "U":
			...
			xm[x] = append(xm[x], [2]int{y, 1})
			if len(xy2m[x-y]) == 0 {
				xy2m[x-y] = [][2]int{}
			}
			xy2m[x-y] = append(xy2m[x-y], [2]int{x, 1})
			if len(xy3m[x+y]) == 0 {
				xy3m[x+y] = [][2]int{}
			}
			xy3m[x+y] = append(xy3m[x+y], [2]int{y, 1})
		case "D":
			if len(xm[x]) == 0 {
				xm[x] = [][2]int{}
			}
			...
			xy1m[x-y] = append(xy1m[x-y], [2]int{x, -1})
			if len(xy4m[x+y]) == 0 {
				xy4m[x+y] = [][2]int{}
			}
			xy4m[x+y] = append(xy4m[x+y], [2]int{y, -1})
		case "R":
			if len(ym[y]) == 0 {
				ym[y] = [][2]int{}
			}
			ym[y] = append(ym[y], [2]int{x, 1})
			if len(xy1m[x-y]) == 0 {
				xy1m[x-y] = [][2]int{}
			}
			xy1m[x-y] = append(xy1m[x-y], [2]int{x, 1})
