...
func main() {
	...
	var cut func(sl []int) []int
	cut = func(sl []int) []int {
		if len(sl) < 3 {
			return sl
		}
		if sl[len(sl)-3] == 0 && sl[len(sl)-2] == 1 && sl[len(sl)-1] == 2 {
			sl = sl[:len(sl)-3]
			return cut(sl)
		}
		if sl[len(sl)-3] == 1 && sl[len(sl)-2] == 2 && sl[len(sl)-1] == 0 {
			sl = sl[:len(sl)-3]
			return cut(sl)
		}
		if sl[len(sl)-3] == 2 && sl[len(sl)-2] == 0 && sl[len(sl)-1] == 1 {
			sl = sl[:len(sl)-3]
			return cut(sl)
		}
