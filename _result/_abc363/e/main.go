...
func main() {
	...
	h, w, y := ni3()
	mp := ni2d(h, w)
	ptm := make(map[int][][2]int)
	uf := newUnionFind(h*w + 1)
	see := h * w
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			if _, ok := ptm[mp[i][j]]; !ok {
				ptm[mp[i][j]] = [][2]int{}
			}
			ptm[mp[i][j]] = append(ptm[mp[i][j]], [2]int{i, j})
		}
	}
	dx := []int{1, 0, -1, 0, 1, 1, -1, -1}
	dy := []int{0, 1, 0, -1, 1, -1, 1, -1}
	for i := 1; i <= y; i++ {
		for j := 0; j < len(ptm[i]); j++ {
			for k := 0; k < 4; k++ {
				np := point{ptm[i][j][0] + dx[k], ptm[i][j][1] + dy[k]}
				if np.isValid(h, w) {
					if mp[np.x][np.y] <= mp[ptm[i][j][0]][ptm[i][j][1]] {
						uf.unite(ptm[i][j][0]*w+ptm[i][j][1], np.x*w+np.y)
					}
				} else {
					uf.unite(ptm[i][j][0]*w+ptm[i][j][1], see)
				}
			}
		}
		out(h*w - uf.size(see) + 1)
	}
}
