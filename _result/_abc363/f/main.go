...
func main() {
	...
	for i := 1; i < len(dv); i++ {
		if strings.Contains(itoa(dv[i]), "0") {
			continue
		}
		ndv = append(ndv, dv[i])
		mp[dv[i]] = true
	...
	for i := 0; i < len(ndv); i++ {
		...
		if mp[atoi(v2)] {
			ndvp = append(ndvp, [2]int{ndv[i], atoi(v2)})
			delete(mp, ndv[i])
		}
	}
	dbg(pd, ndvp)
	...
	dfs = func(v int) {
		...
		for _, pdv := range pd {
			if v*pdv == n {
				tsl := []string{}
				for i := 0; i < len(sl1); i++ {
					tsl = append(tsl, itoa(sl1[i]))
				}
				tsl = append(tsl, itoa(pdv))
				for i := len(sl2) - 1; i >= 0; i-- {
					tsl = append(tsl, itoa(sl2[i]))
				}
				o = strings.Join(tsl, "*")
			}
			nv := v * pdv * pdv
