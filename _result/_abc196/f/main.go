...
func main() {
	...
	ts2 := make([]int32, lent)
	for i, v := range s {
		ss1[lens-i-1] = int32(v - '0')
		ss2[lens-i-1] = ss1[lens-i-1] ^ 1
	}
	for i, v := range t {
		ts1[i] = int32(v - '0')
		ts2[i] = ts1[i] ^ 1
	}
	sc1 := convolutionMod(ss1, ts1, mod)
	sc2 := convolutionMod(ss2, ts2, mod)
	for i := lent - 1; i < lens; i++ {
		o = max(o, int(sc1[i]+sc2[i]))
	}
