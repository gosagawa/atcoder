...
func main() {
	...
	for i := 72; i >= 0; i-- {
		for j := 0; j <= i; j++ {
			...
			if t1 > 0 {
				dp[t1-1][t2] += dp[t1][t2] * (float64(t1) / float64(t1+t2))
			}

			dp[t1][t2-1] += dp[t1][t2] * (float64(t2) / float64(t1+t2))
			o += 2.0 * dp[t1][t2] * (float64(t1+t2-1) / float64(t1+t2))
