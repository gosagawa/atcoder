...
func main() {
	...
	for i := 0; i < m; i++ {
		as[i]--
		bs[i]--
		abst = append(abst, [3]int{ss[i], 1, i})
		abst = append(abst, [3]int{ts[i], 0, i})
	}
	sort2ar(abst, opt2ar(0, asc), opt2ar(1, asc))
	st := is(n, 0)
	rs := make([]int, m)
	rs[0] = x
	for _, v := range abst {
		...
		case 1:
			if v[2] > 0 {
				rs[v[2]] = max(0, st[as[v[2]]]-v[0])
			}
		case 0:
			maxs(&st[bs[v[2]]], v[0]+rs[v[2]])
		}
