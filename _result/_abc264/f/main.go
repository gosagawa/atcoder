...
func solve() int {
	...
	for r1 := 0; r1 < 2; r1++ {
		c1 := mp[0][0] ^ r1
		dp[0][0][0][c1] = r1*rs[0] + c1*cs[0]
		dp[0][0][1][r1] = r1*rs[0] + c1*cs[0]
	}
		...
		for j := 0; j < w; j++ {
			for d := 0; d < 2; d++ {
				for l := 0; l < 2; l++ {
					...
					//out(i, j, d, l, cost, dp[i][j])
					for nd := 0; nd < 2; nd++ {
						nl := l
						if d != nd {
							nl = flip
						}
						mins(&dp[ni][nj][nd][nl], dp[i][j][d][l]+cost)
					}
				}
			}
		}
