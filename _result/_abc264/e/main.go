...
func main() {
	...
	uf := newUnionFind(n + m + 10)
	for i := 0; i < e; i++ {
		if xis[i] {
			continue
		}
		if uf.issame(us[i]-1, vs[i]-1) {
			continue
		}
		if !hase[uf.root(us[i]-1)] && !hase[uf.root(vs[i]-1)] {
			uf.unite(us[i]-1, vs[i]-1)
			continue
		}
		if !hase[uf.root(us[i]-1)] {
			ec += uf.size(us[i] - 1)
		}
		if !hase[uf.root(vs[i]-1)] {
			ec += uf.size(vs[i] - 1)
		}
		uf.unite(us[i]-1, vs[i]-1)
		hase[uf.root(us[i]-1)] = true
	}
	r := make([]int, q)
	for i := q - 1; i >= 0; i-- {
		r[i] = ec - m
		...
		v := xs[i] - 1
		if uf.issame(us[v]-1, vs[v]-1) {
			continue
		}
		if !hase[uf.root(us[v]-1)] && !hase[uf.root(vs[v]-1)] {
			uf.unite(us[v]-1, vs[v]-1)
			continue
		}
		if !hase[uf.root(us[v]-1)] {
			ec += uf.size(us[v] - 1)
		}
		if !hase[uf.root(vs[v]-1)] {
			ec += uf.size(vs[v] - 1)
		}
		uf.unite(us[v]-1, vs[v]-1)
		hase[uf.root(us[v]-1)] = true
	}
	for _, v := range r {
		out(v)
	}
