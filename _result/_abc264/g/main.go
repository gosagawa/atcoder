...
func main() {
	...
	o := -inf
	n := ni()
	tps := make([]int, 27*27*27)
	sltoi := func(t []int) int {
		tb := 1
		ti := 0
		for i := len(t) - 1; i >= 0; i-- {
			if tb != 1 {
	...
	for i := 0; i < v; i++ {
		for j := 0; j < 26; j++ {
			...
			c += tps[sltoi([]int{j})]
			if i%27 != 26 {
				c += tps[sltoi([]int{i % 27, j})]
			}
			if i/27 != 26 {
				c += tps[sltoi([]int{i / 27, i % 27, j})]
			}

			edges[i] = append(edges[i], edge{to: nj, cost: -c})
		}
	}
	graph := newgraph(v, edges)
	st := 26*27 + 26
	dist, ng := graph.bellmanFord(st)
	if ng {
		out("Infinity")
		return
	}
...
func (g *graph) bellmanFord(start int) ([]int, bool) {
	...
	for i := 0; i < g.size; i++ {
		...
		for j := 0; j < g.size; j++ {
			...
			for _, edge := range g.edges[j] {
				if score[edge.to] == inf || score[edge.to] > score[j]+edge.cost {
					score[edge.to] = score[j] + edge.cost
					update = true
				}
			}
