...
func main() {
	...
	var dfs func(v, p int) (int, int, bool)
	dfs = func(v, p int) (int, int, bool) {
		var r [3]int
		r[0]++
		...
		for _, nv := range edges[v] {
			if nv.to != p {
				tt, ta, tc := dfs(nv.to, v)
				if tc {
					ci = append(ci, [3]int{tt, ta, tt - ta})
					cc++
				} else {
					if tt > ta {
						fi[0] += tt
