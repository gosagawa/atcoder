...
func main() {
	...
	for i := 0; i < n; i++ {
		...
		if l != 0 {
			as[0] = append(as[0], [3]int{l + 1, r - 1, 1})
			as[l] = append(as[l], [3]int{l + 1, r - 1, -1})
		}
		if r != mx {
			as[l+1] = append(as[l+1], [3]int{r + 1, mx, 1})
			as[r] = append(as[r], [3]int{r + 1, mx, -1})
		}
	}
	...
	for i := 0; i <= mx; i++ {
		for _, v := range as[i] {
			lst.applyrange(v[0], v[1]+1, segfstruct(v[2]))
		}
		t := int(lst.allprod())
		if t > mxidx[0] {
			mxidx[0] = t
	...
	for j := 1; j <= mx; j++ {
		if int(lst.get(j)) == mxidx[0] {
			out2d(sl[mxidx[1]], sl[j])
			return
		}
	}
