...
func main() {
	...
	for i := 0; i < n; i++ {
		if ss[i] == 1 {
			t1 := lowerBound(xs[i], ls)
			t2 := lowerBound(xs[i]+t*2+1, ls)
			r := t2 - t1
			dbg(t1, xs[i], t2, xs[i]+t*2+1, ls, r)
			o += r
		} else {
			t1 := upperBound(xs[i], rs)
			t2 := upperBound(xs[i]-t*2-1, rs)
			r := t1 - t2
			dbg(t1, xs[i], t2, xs[i]-t*2-1, rs, r)
			o += t1 - t2
		}
	}
	o /= 2
