...
func main() {
	...
	uf := newUnionFind(n*2 - 1)
	mp := is(n*2-1, 0)
	for i := 0; i < n; i++ {
		mp[i] = i
	}
	edges := make([][]edge, n*2-1)
	for i := 0; i < n-1; i++ {
		v1 := mp[uf.root(ps[i])]
		vc1 := uf.size(ps[i])
		v2 := mp[uf.root(qs[i])]
		...
		edges[v3] = append(edges[v3], edge{to: v1, cost: mdiv(vc1, vc1+vc2)})
		edges[v3] = append(edges[v3], edge{to: v2, cost: mdiv(vc2, vc1+vc2)})
	}
	r := is(n, 0)
