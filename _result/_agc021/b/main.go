...
func main() {
	...
	sts := []st{}
	angles := []float64{0, 360}
	for i := 0; i < n; i++ {
		for j := i + 1; j < n; j++ {
			a := pts[i].getAngle(pts[j])
			a1 := addAngle(a, 90)
			a2 := addAngle(a, 270)
			angles = append(angles, a1)
			angles = append(angles, a2)
			if a1 < a2 {
				sts = append(sts, st{a1, a2, i, j})
			} else {
				sts = append(sts, st{a2, a1, j, i})
			}
		}
	}
	dbg(sts)
...
func addAngle(x, y float64) float64 {
	r := x + y
	if r >= 360 {
		r -= 360
	}
