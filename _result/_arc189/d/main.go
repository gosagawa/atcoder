...
func main() {
	...
	rs := is(n-2, 0)
	mlmp := make(map[[2]int]int)
	mrmp := make(map[[2]int]int)
	ml := func(i, v int) int {
		if _, ok := mlmp[[2]int{i, v}]; ok {
			return mlmp[[2]int{i, v}]
		}
		ni := lst.maxleft(i, func(c segstruct) bool {
			return c < segstruct(v)
		})
		mlmp[[2]int{i, v}] = ni
		return ni
	}
	mr := func(i, v int) int {
		if _, ok := mrmp[[2]int{i, v}]; ok {
			return mrmp[[2]int{i, v}]
		}
		ni := lst.maxright(i, func(c segstruct) bool {
			return c < segstruct(v)
		})
		mrmp[[2]int{i, v}] = ni
		return ni
	...
	for i := 1; i < n-1; i++ {
		...
		for {
			li := ml(tl, rs[i-1])
			ri := mr(tr, rs[i-1])
			dbg(i, tl, tr, rs[i-1], "->", li, ri, as[li], as[ri])
			if tl == li && tr == ri {
				break
			}
			rs[i-1] += cs.get(li, tl)
