...
func main() {
	...
	edges := make([][]edge, pow2(n)+1)
	for i := 0; i <= n; i++ {
		for j := 0; j < pow2(n); j += pow2(i) {
			s := j
			t := j + pow2(i)
	...
	graph := newgraph(pow2(n)+1, edges)
	dist := graph.dijkstra(l)
	debug(dist)
	debug(dist[r+1])
	...
	get := func(x, y int) {
		...
		o %= 100
		debug(pow2(i)*j, pow2(i)*(j+1)-1, o)
	}

	t := r + 1
	for i := 0; i < dist[r+1]; i++ {
		for _, nv := range edges[t] {
			if dist[nv.to] == dist[t]-1 {
				get(nv.to, t)
				t = nv.to
				break
			}
