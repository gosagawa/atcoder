...
func main() {
	...
	for i := 0; i < h; i++ {
		s := 0
		t := i + 1
		setDualEdge(edges, s, t, c, point{})

		for j := 0; j < w; j++ {
			s = i + 1
			t = 1 + h + i*w + j
			setDualEdge(edges, s, t, c, point{i, j})
	...
	for i := 0; i < w; i++ {
		...
		debug(o)
		for j := 0; j < h; j++ {
			for k, nv := range graph.edges[j+1] {
				if nv.to == 0 {
					continue
				}

				if nv.cost == 0 && as[nv.pt.x][nv.pt.y] != 0 {
					debug(nv)
					r[nv.pt.x][i] = as[nv.pt.x][nv.pt.y]
					as[nv.pt.x][nv.pt.y] = 0
					eraseEdgeCost(graph.edges, j+1, k)
				}
			}
		}
		for j := range graph.edges[0] {
			reverseEdgeCost(graph.edges, 0, j)
		}
		for j := range graph.edges[v-1] {
			reverseEdgeCost(graph.edges, v-1, j)
		}
	}
	out("Yes")
	for _, v := range r {
...
func setDualEdge(edges [][]edge, s, t, c int, pt point) {
	edges[s] = append(edges[s], edge{from: s, to: t, cost: c, rev: len(edges[t]), pt: pt})
	edges[t] = append(edges[t], edge{from: t, to: s, cost: 0, rev: len(edges[s]) - 1, pt: pt})
}
