...
func main() {
		...
		for j := 0; j < 8; j++ {
			for k := 0; k < as; k++ {
				for l := 0; l < 8; l++ {
					...
					njs := [3]int{}
					for m := 0; m < 3; m++ {
						ni += sw(nb[i], btoi(hasbit(j, m)), btoi(hasbit(l, m))) * pow2(m)
						if hasbit(l, m) {
							njs[m] = ks[m] + pow2(i)
						} else {
							njs[m] = ks[m]
	...
	*/
	o = dp[maxlogn-1][7][0]
	o = madd(o, -1)
	o = madd(o, -n/gcm(a[0], a[1]))
	o = madd(o, -n/gcm(a[1], a[2]))
	o = madd(o, -n/gcm(a[0], a[2]))
