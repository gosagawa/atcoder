...
func main() {
	...
	as := nsis()
	bs := nsis()
	acs := i2s(len(as)+1, 26, 0)
	bcs := i2s(len(bs)+1, 26, 0)
	for i := 0; i < len(as); i++ {
		acs[i+1][as[i]]++
		for j := 0; j < 26; j++ {
			acs[i+1][j] += acs[i][j]
	...
	ls := []int{len(as), len(bs)}
	lcs := [][]int{}
	lcs = append(lcs, is(26, 0))
	for j := 0; j < 26; j++ {
		lcs[0][j] += acs[len(as)][j]
	}
	lcs = append(lcs, is(26, 0))
	for j := 0; j < 26; j++ {
		lcs[1][j] += bcs[len(bs)][j]
	}
	for {
		t := ls[len(ls)-1] + ls[len(ls)-2]
		ls = append(ls, t)
		lcs = append(lcs, is(26, 0))
		for j := 0; j < 26; j++ {
			lcs[len(lcs)-1][j] = lcs[len(lcs)-2][j] + lcs[len(lcs)-3][j]
		}
		if t > int(1e18) {
			break
		}
	...
	*/
	...
	for i := 0; i < q; i++ {
		...
		ts := nsis()
		c := ts[0]
		var fn func(i, c int) int
		fn = func(i, c int) int {
			ti := lowerBound(i+1, ls)
			if ti < 2 {
				if i > len(bs) {
					return lcs[1][c] + acs[i-len(bs)][c]
				} else {
					return bcs[i][c]
				}
			}
