...
func main() {
	...
	n, m := ni2()
	xys := ni2a(m)
	sort2ar(xys, opt2ar(0, asc))

	// 区間加算・区間和取得
	op := func(a segstruct, b segstruct) segstruct {
		return segstruct((int(a) + int(b)) % mod)
	}
	e := func() segstruct {
		return segstruct(0)
	}
	...
	mapping := func(f segfstruct, x segstruct) segstruct {
		...
		return segstruct(int(x) * int(f) % mod)
	}
	...
	for i := 0; i < m; i++ {
		...
		x--
		y--
		lst.applyrange(0, x, 2)
		lst.applyrange(y+1, n, 2)

		t1 := int(lst.prod(x, y+1))
		t2 := int(lst.get(y))
		//		dbg(x, y, t1, t2)

		lst.set(y, segstruct((t1+t2)%mod))
