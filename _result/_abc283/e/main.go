...
func main() {
	...
	isValid := func(i, j, sw1, sw2, sw3 int) bool {
		if i != 0 && ns[i-1][j]^sw1 == ns[i][j]^sw2 {
			return true
		}
		if j != 0 && ns[i][j-1] == ns[i][j] {
			return true
		}
		if j != w-1 && ns[i][j+1] == ns[i][j] {
			return true
		}
		if i != h-1 && ns[i+1][j]^sw2 == ns[i][j]^sw3 {
			return true
		}
		return false
	}
			...
			for j := 0; j < 2; j++ {
				for k := 0; k < 2; k++ {
					...
					for l := 0; l < 2; l++ {
						isAllValid := true
						for m := 0; m < w; m++ {
							if !isValid(i-1, m, j, k, l) {
								isAllValid = false
								break
							}
							if i == h-1 {
								if !isValid(i, m, k, l, 0) {
									isAllValid = false
									break
								}
							}
						}
						//					out(i, j, k, l, isAllValid)
						if isAllValid {
							mins(&dp[i][k][l], dp[i-1][j][k]+l)
						}
					}
				}
			}
