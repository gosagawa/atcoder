...
func main() {
	...
	sweep := func() {
		...
		ts2 := make([][2]int, n)
		for i, v := range ts {
			ts2[i] = [2]int{v[1] - v[0], i}
		}
		sort2ar(ts2, opt2ar(0, asc))

		st := newstree(n*2+10, stmin)
		for _, v := range ts2 {
			x, y := ts[v[1]][0], ts[v[1]][1]
			t := st.query(y+n, n*2+2)
			if t != inf {
				ni := t % int(1e6)
				mins(&rs[v[1]], abs(x-ts[ni][0])+abs(y-ts[ni][1]))
				mins(&rs[ni], abs(x-ts[ni][0])+abs(y-ts[ni][1]))
				//				out(v[1], ni, abs(x-ts[ni][0])+abs(y-ts[ni][1]))
			}
			st.set(y+n, (x+n+y+n)*int(1e6)+v[1])
		}
	...
	for i := 0; i < 2; i++ {
		for j := 0; j < 2; j++ {
			for k := 0; k < 2; k++ {
				sweep()
				for ti := range ts {
					ts[ti][0], ts[ti][1] = ts[ti][1], ts[ti][0]
				}
			}
			for ti := range ts {
				ts[ti][0] = -ts[ti][0]
