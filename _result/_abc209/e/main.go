...
func main() {
	...
	for i := 0; i < n; i++ {
		s := ns()
		fts[i][0] = gi(string(s[:3]))
		fts[i][1] = gi(string(s[len(s)-3:]))
	}

	edges := make([][]edge, n+len(mp))
	deg := is(n+len(mp), 0)
	for i := 0; i < n; i++ {
		s := i
		t := fts[i][0]
		edges[s] = append(edges[s], edge{to: t, rev: len(edges[s])})
		deg[t]++
