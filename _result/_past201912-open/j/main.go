...
func main() {
	...
	getd := func(i, j int) [][]int {
		...
		pq := newpq([]compFunc{func(p, q interface{}) int {
			if r[p.(point).x][p.(point).y] < r[q.(point).x][q.(point).y] {
				return -1
			} else {
				return 1
			}
		...
		heap.Init(pq)
		heap.Push(pq, point{i, j})
		for !pq.IsEmpty() {
			p := heap.Pop(pq).(point)
			for i := 0; i < 4; i++ {
				np := point{p.x + dx[i], p.y + dy[i]}
				if np.isValid(h, w) && r[np.x][np.y] == inf {
					r[np.x][np.y] = r[p.x][p.y] + mp[np.x][np.y]
					heap.Push(pq, np)
				}
			}
		}
	...
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			o = min(o, mp1[i][j]+mp2[i][j]+mp3[i][j]-2*mp[i][j])
		}
	}
