...
func main() {
	...
	op := func(a segstruct, b segstruct) segstruct {
		return segstruct(maxf(float64(a), float64(b)))
	}
	e := func() segstruct {
		return segstruct(0)
	}
	...
	dp := make([]float64, len(cmp))
	vs := make([]segstruct, n)
	for i := len(cmp) - 1; i >= 0; i-- {
		dp[i] = float64(lst.allprod()) + 1
		for _, j := range is[i] {
			vs[j] += segstruct(dp[i] / 6)
			lst.set(j, vs[j])
		}
	}
	dbg(dp)
