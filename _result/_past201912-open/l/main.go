...
func main() {
	...
	n, m := ni2()
	xs, ys, cs := ni3s(n + m)
	isUse := func(mask int, i int) bool {
		if i < n {
			return true
		}
		i -= n
		return hasbit(mask, i)
	}
	check := func(mask int) {
		edges := []edge{}
		for i := 0; i < n+m; i++ {
			...
			for j := i + 1; j < n+m; j++ {
				...
				edges = append(edges, edge{from: i, to: j, l: pointDist(point{x: xs[i], y: ys[i]}, point{x: xs[j], y: ys[j]}) * t})
			}
		}
