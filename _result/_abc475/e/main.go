...
func main() {
	...
	n, m, k := ni3()
	ts := convidxis(nsis(), map[string]int{"x": 0, "o": 1})
	ss := make([][4]int, n)
	mp := make(map[[4]int]int)
	mp[[4]int{}] = 1
	sw := func(i *[4]int, j int) {
		t1 := j / 60
		t2 := 60 - j%60
		i[t1] ^= pow2(t2)
	}
	...
	for i := 0; i < n; i++ {
		tss := convidxis(nsis(), map[string]int{"x": 0, "o": 1})
		for j := 0; j < k; j++ {
			if tss[j] == ts[j] {
				sw(&ss[i], j)
			}
	...
	for i := 0; i < q; i++ {
		...
		sum := bt.sum(mp[ss[qi]])
		dbg(ss[qi], sum)
		if bt.sum(mp[ss[qi]]) <= m && mp[ss[qi]] != len(mp)-1 {
			out("Yes")
		} else {
			out("No")
		}
