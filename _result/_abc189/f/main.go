...
func main() {
	...
	// 区間加算・区間和取得
	base := make([]segstruct, n+m)
	for _, v := range as {
		base[v] = segstruct{1.0, 0, 1}
	}
	op := func(a segstruct, b segstruct) segstruct {
		return segstruct{a.vk + b.vk, a.v + b.v, a.size + b.size}
	}
	e := func() segstruct {
	...
	for i := n - 1; i >= 0; i-- {
		...
		sum := lst.prod(i+1, i+1+m)
		dbg(sum)

		lst.set(i, segstruct{sum.vk / mf, sum.v/mf + 1.0, 1})
	}
	r := lst.prod(0, 1)
	dbg(r)
	o := r.v / (1 - r.vk)
