...
func main() {
	...
	ns := []int{a, b, c}
	sorti(ns)
	if check3inrow(x, y, a, b, c) {
		out("Yes")
		return
	}
	if check3inrow(y, x, a, b, c) {
		out("Yes")
		return
	}
	for {

		if checkT(x, y, ns[0], ns[1], ns[2]) {
			out("Yes")
			return
		}
		if checkT(y, x, ns[0], ns[1], ns[2]) {
			out("Yes")
			return
		}
...
func check3inrow(x, y, a, b, c int) bool {
	t := 0
	t += (a + x - 1) / x
	t += (b + x - 1) / x
	t += (c + x - 1) / x
...
func check2inrow(x, y, a, b int) bool {
	t := 0
	t += (a + x - 1) / x
	t += (b + x - 1) / x
	return t <= y
...
func checkT(x, y, a, b, c int) bool {
	y -= (a + x - 1) / x
	if y <= 0 {
		return false
	}
	if check2inrow(x, y, b, c) {
		return true
	}
	if check2inrow(y, x, b, c) {
		return true
	}
	return false
}
