...
func main() {
	...
	sumpt := make([][][2]int, h*w+1)
	for i := 1; i <= h; i++ {
		for j := 1; j <= w; j++ {
			sumpt[i*j] = append(sumpt[i*j], [2]int{i, j})
		}
	}
	sumptindexlist := []int{}
	for i, pt := range sumpt {
		if len(pt) > 0 {
			sumptindexlist = append(sumptindexlist, i)
		}
	}
	f := func(c int) bool {
		idx := sumptindexlist[c]
		for _, pt := range sumpt[idx] {
			x := pt[0]
			y := pt[1]
			//out(h, x, w, y)
			for i := 0; i <= h-x; i++ {
				for j := 0; j <= w-y; j++ {
					sum := cusum2d.get(i, j, i+x, j+y)
					//out(i, j, i+x, j+y, sum)
					if sum <= v {
						return true
					}
