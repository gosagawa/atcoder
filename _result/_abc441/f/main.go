...
func main() {
	...
	sort.Slice(sl, func(i, j int) bool {
		if vs[sl[i]] == vs[sl[j]] {
			return ps[sl[i]] < ps[sl[j]]
		}
		return vs[sl[i]] > vs[sl[j]]
	})
	dbg(sl)
	dp := is(m+1, -inf)
	dp2 := is(m+1, -inf)
	...
	for ti := 0; ti < n; ti++ {
		...
		for j := m - 1; j >= 0; j-- {
			...
			if dp[j+ps[i]] < dp[j]+vs[i] {
				logs = append(logs, [3]int{i, j + ps[i], dp[j+ps[i]]})
				dp[j+ps[i]] = dp[j] + vs[i]
			}
		}
	}
	...
	for ti := n - 1; ti >= 0; ti-- {
		i := sl[ti]
		for {
			if len(logs) == 0 || logs[len(logs)-1][0] != i {
				break
			}
			log := logs[len(logs)-1]
		...
		for j := 0; j <= m; j++ {
			maxs(&dpmaxv, dp[j])
			if dpmaxv+dp2[m-j] == maxv {
				case1 = true
			}
			if m-j-ps[i] >= 0 && dpmaxv+dp2[m-j-ps[i]] == maxv-vs[i] {
				case2 = true
			}
		}
		...
		for j := m - 1; j >= 0; j-- {
			...
			if dp2[j+ps[i]] < dp2[j]+vs[i] {
				dp2[j+ps[i]] = dp2[j] + vs[i]
			}
		}
