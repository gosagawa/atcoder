...
func main() {
	...
	for i := 0; i < len(ps); i++ {
		for j := 0; j < len(ps); j++ {
			for k := 0; k < len(ps); k++ {
				if ps[j].x-ps[i].x != ps[i].y-ps[k].y {
					continue
				}
				if ps[j].y-ps[i].y != ps[k].x-ps[i].x {
					continue
				}
				l, ok := pm[point{ps[j].x + ps[k].x - ps[i].x, ps[j].y + ps[k].y - ps[i].y}]
				if !ok {
					continue
				}
				...
				//out(i, j, k, l)
				//out(ps[i], ps[j], ps[k], point{ps[j].x + ps[k].x - ps[i].x, ps[j].y + ps[k].y - ps[i].y})
				o++
