...
func main() {
	...
	al := make([][4]int, 0)
	for i := 0; i < n; i++ {
		al = append(al, [4]int{1, as[i], i, 0})
	}
	for i := 0; i < q; i++ {
		rs[i]--
		al = append(al, [4]int{2, xs[i], rs[i], i})
	}
	sort2ar(al, opt2ar(1, asc), opt2ar(0, asc), opt2ar(2, desc))
	dbg(al)
