...
func main() {
	...
	var dfs func(v, p int) [][2]int
	dfs = func(v, p int) [][2]int {
		dp := make([][2]int, 2)
		dp[0][0] = 1
		dp[1][1] = 1
		for _, nv := range edges[v] {
			...
			ret := dfs(nv.to, v)
			ndp := make([][2]int, len(dp)+len(ret)-1)
			for dpi, dpv := range dp {
				for reti, retv := range ret {
					ndp[dpi+reti][0] += mmul(dpv[0], madd(retv[0], retv[1]))
					ndp[dpi+reti][0] %= mod
					ndp[dpi+reti][1] += mmul(dpv[1], retv[0])
					ndp[dpi+reti][1] %= mod
					if dpi+reti > 0 {
						ndp[dpi+reti-1][1] += mmul(dpv[1], retv[1])
						ndp[dpi+reti-1][1] %= mod
					}
				}
			}
			dp = make([][2]int, len(dp)+len(ret)-1)
			copy(dp, ndp)
		}
		return dp
	}
