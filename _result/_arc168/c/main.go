...
func main() {
	...
	for i := 0; i <= k; i += 2 {
		for l := 0; l <= k-i; l++ {
			for m := 0; m <= k-i-l; m++ {
				for p := 0; p <= k-i-l-m; p++ {
					v := [6]int{i/2 + l, l, i/2 + m, m, i/2 + p, p}
					pt = append(pt, v)
					if i != 0 {
						v = [6]int{l, i/2 + l, m, i/2 + m, p, i/2 + p}
						pt = append(pt, v)
					}
				}
			}
	...
	//	debug(pt, st)
	cf := newcombFactorial(n + 10)
	for _, v := range pt {
		to := 1
		ac := v[0] + v[5]
		bc := v[1] + v[2]
		cc := v[3] + v[4]
		if ac > st[0] || bc > st[1] || cc > st[2] {
			continue
		}
		to = mmul(to, cf.combination(st[0], ac))
		to = mmul(to, cf.combination(st[1], bc))
		to = mmul(to, cf.combination(st[2], cc))
		to = mmul(to, cf.combination(ac, v[1]))
		to = mmul(to, cf.combination(bc, v[3]))
		to = mmul(to, cf.combination(cc, v[5]))
		o = madd(o, to)
	}
