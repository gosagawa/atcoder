...
func main() {
	...
	for i := 0; i < n-1; i++ {
		...
		s--
		t--
		edges[s] = append(edges[s], edge{index: i, to: t})
		edges[t] = append(edges[t], edge{index: i, to: s})
	}
	...
	dp[0][0] = 1
	for i, v := range tree.edgeuses {
		dp[i+1] = make([]int, target+1)
		for j := 0; j <= target; j++ {
			dp[i+1][j] = madd(dp[i+1][j], dp[i][j])
			if j+v <= target {
				dp[i+1][j+v] = madd(dp[i+1][j+v], dp[i][j])
			}
		}
		//out(dp[i+1])
	}
...
func (t *tree) mydfs(v, p, to int) bool {
	if v == to {
		return true
	}
	for _, nv := range t.edges[v] {
		if nv.to != p {
			r := t.mydfs(nv.to, v, to)
			if r == true {
				t.edgeuses[nv.index]++
				return true
			}
