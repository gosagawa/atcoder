...
func main() {
	...
	edges := make([][]edge, h+w+2)
	setDualEdge(edges, 0, s.x+1, inf)
	setDualEdge(edges, 0, s.y+h+1, inf)
	setDualEdge(edges, g.x+1, h+w+1, inf)
	setDualEdge(edges, g.y+h+1, h+w+1, inf)

	for _, v := range ps {
		setDualEdge(edges, v.x+1, v.y+h+1, 1)
		setDualEdge(edges, v.y+h+1, v.x+1, 1)
	}
	graph := newgraph(h+w+2, edges)
	o = graph.dinic()
	out(o)
}
