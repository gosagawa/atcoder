	...
	for j := 0; j < 26; j++ {
		for m := 0; m <= k; m++ {
			...
			for l := 0; l <= cs[j]; l++ {
				...
				dp[j+1][m+l] = madd(dp[j+1][m+l], mmul(dp[j][m], mdiv(1, cf.factorial(l))))
			}
		}
	}
	for i := 1; i <= k; i++ {
		t := mmul(dp[26][i], cf.factorial(i))
		dbg(i, t)
		o = madd(o, t)
	}
	out(o)
...
func mdiv(a, b int) int {
	if v, ok := mpowcache[[3]int{a, b}]; ok {
		return v
	}
	fa := a
	fb := b
	...
	r := a * minvfermat(b, mod) % mod
	mdivcache[[2]int{fa, fb}] = r
	return r
}
