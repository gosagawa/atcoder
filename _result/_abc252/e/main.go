...
func main() {
	...
	for i := 0; i < e; i++ {
		...
		s--
		t--
		edges[s] = append(edges[s], edge{from: i + 1, to: t, cost: d})
		edges[t] = append(edges[t], edge{from: i + 1, to: s, cost: d})
	}
	...
	heap.Init(pq)
	r := []int{}
	heap.Push(pq, edge{from: 0, to: 0, cost: 0})
	for {
		if pq.Len() == 0 {
			break
		}
		...
		for _, nv := range edges[t.to] {
			heap.Push(pq, edge{from: nv.from, to: nv.to, cost: t.cost + nv.cost})
		}
	}
