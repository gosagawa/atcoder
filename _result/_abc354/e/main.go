...
func main() {
	...
	used := make([]bool, pow2(n))
	ans := make([]bool, pow2(n))
	var dfs func(c int, st int) bool
	dfs = func(c int, st int) bool {
		if used[st] {
			return ans[st]
		}
		if c%2 == 0 {
			used[st] = true
			for _, v := range ps {
				if hasbit(st, v[0]) || hasbit(st, v[1]) {
					continue
				}
				if dfs(c+1, st|pow2(v[0])|pow2(v[1])) {
					ans[st] = true
					return true
				}
			}
