...
func solve() {
	...
	to := bsfl(0.0, float64(1e9), 100, func(c float64) bool {
		t := 0
		for i := 0; i < n; i++ {
			j := 0
			t2 := as[i]
	...
	for i := 0; i < n; i++ {
		...
		if tsts[i].c <= t {
			tsts = append(tsts, tst{v: tsts[i].v / 2, c: tsts[i].c * 2})
			t -= tsts[i].c
			tsts[i].c = 0
		} else {
			tsts[i].c -= t
			tsts = append(tsts, tst{v: tsts[i].v / 2, c: t * 2})
			t = 0
		}
	}
	dbg(tsts)
