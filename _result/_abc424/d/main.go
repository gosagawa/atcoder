...
func solve() {
	...
	ok := i2s(pow2(w), pow2(w), 1)
	for i := 0; i < pow2(w); i++ {
		for k := 0; k < pow2(w); k++ {
			for l := 0; l < w-1; l++ {
				if hasbit(i, l) && hasbit(i, l+1) && hasbit(k, l) && hasbit(k, l+1) {
					ok[i][k] = 0
				}
			}
		}
	...
	for j := 1; j < h; j++ {
		...
		for i := 0; i < pow2(w); i++ {
			...
			for k := 0; k < pow2(w); k++ {
				if dp[j-1][k] == inf || ok[i][k] == 0 {
					continue
				}
				mins(&dp[j][i], dp[j-1][k]+pt-popcount(i))
			}
		}
	}
	for i := 0; i < pow2(w); i++ {
