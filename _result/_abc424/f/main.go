...
func main() {
	...
	op := func(a segstruct, b segstruct) segstruct {
		return segstruct{sum: a.sum + b.sum, mi: min(a.sum+b.mi, a.mi)}
	}
	e := func() segstruct {
		return segstruct{sum: 0, mi: 0}
	}
	id := func() segfstruct {
		return segfstruct{}
	}
	...
	/*
		dbg(op(segstruct{1, 1}, segstruct{-1, -1}))
		dbg(op(segstruct{-1, -1}, segstruct{1, 1}))
		dbg(op(segstruct{0, 0}, segstruct{1, 1}))
	*/
	...
	for qi := 0; qi < q; qi++ {
		...
		t := lst.prod(a, b+1)
		if t.mi == 0 && t.sum == 0 {
			lst.set(a, segstruct{1, 1})
			lst.set(b, segstruct{-1, -1})
			out("Yes")
		} else {
			out("No")
		}
