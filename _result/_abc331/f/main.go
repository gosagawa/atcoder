...
func main() {
	...
	n, m := ni2()
	ns := nsis()
	mods := [5]int{998244353, 1000000007, 1000000009, 1000000021, 100000003}
	rns := [5]int{}
	for i := 0; i < 5; i++ {
		rns[i] = rand.Intn(mods[i]-1) + 1
	}

	debug(ns)
	op := func(l segstruct, r segstruct) segstruct {
		res := segstruct{}
		for i := 0; i < 5; i++ {
			res[i][0] = (l[i][0]*r[i][2] + r[i][0]) % mods[i]
			res[i][1] = (l[i][1] + l[i][2]*r[i][1]) % mods[i]
			res[i][2] = l[i][2] * r[i][2] % mods[i]
		}
		return res
	}
	e := func() segstruct {
