...
func solve() {
	...
	for i := 0; i < n; i++ {
		if as[i]+cs[i] <= bs[i] {
			...
			t1 := min(as[i], max(0, bs[i]-cs[i]))
			t2 := min(cs[i], max(0, bs[i]-as[i]))
			ts[0] += min(as[i], max(0, bs[i]-cs[i]))
			ts[1] += min(cs[i], max(0, bs[i]-as[i]))
			ts[2] += bs[i] - t1 - t2
		}
		//		dbg(ts)
	}

	maxs(&o, min(ts[0]+ts[2], ts[1]))
	maxs(&o, min(ts[1]+ts[2], ts[0]))
	if ts[0] <= ts[1] && ts[0]+ts[2] >= ts[1] {
		maxs(&o, (ts[1]+ts[2]+ts[0])/2)
	}
	if ts[0] >= ts[1] && ts[1]+ts[2] >= ts[0] {
		maxs(&o, (ts[1]+ts[2]+ts[0])/2)
	}
	out(o)
}
