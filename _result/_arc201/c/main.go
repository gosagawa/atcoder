...
func main() {
	...
	culc := func(i int) (int, int) {
		cc := 0
		ncc := mpow(2, sl[i].c, mod)

		tc := madd(mpow(2, sl[i].c, mod), -1)
		if sl[i].ai != 0 {
			ncc *= (sl[sl[i].ai].cc + sl[sl[i].ai].ncc) % mod
			ncc %= mod
			tc *= (sl[sl[i].ai].cc + sl[sl[i].ai].ncc) % mod
			tc %= mod
		}
		if sl[i].bi != 0 {
			ncc *= (sl[sl[i].bi].cc + sl[sl[i].bi].ncc) % mod
			ncc %= mod
			tc *= (sl[sl[i].bi].cc + sl[sl[i].bi].ncc) % mod
			tc %= mod
		}
		cc += tc
		cc %= mod
