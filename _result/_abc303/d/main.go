...
func main() {
	...
	for i := 0; i < n; i++ {
		if string(s[i]) == "a" {
			if dp[i][0] != inf {
				mins(&dp[i+1][0], dp[i][0]+x)
				mins(&dp[i+1][1], dp[i][0]+x+z)
				mins(&dp[i+1][1], dp[i][0]+y+z)
			}
			if dp[i][1] != inf {
				mins(&dp[i+1][1], dp[i][1]+y)
				mins(&dp[i+1][0], dp[i][1]+z+x)
				mins(&dp[i+1][0], dp[i][1]+y+z)
			}
		} else {
			if dp[i][0] != inf {
				mins(&dp[i+1][0], dp[i][0]+y)
				mins(&dp[i+1][1], dp[i][0]+z+x)
				mins(&dp[i+1][1], dp[i][0]+y+z)
			}
			if dp[i][1] != inf {
				mins(&dp[i+1][1], dp[i][1]+x)
				mins(&dp[i+1][0], dp[i][1]+x+z)
				mins(&dp[i+1][0], dp[i][1]+y+z)
			}
		}
	}
