...
func main() {
	...
	for i := 0; i < m; i++ {
		x, y, c := ni3()
		ps[i+n] = [4]int{1, x, y, c}
	}
	for i := 0; i < pow2(m); i++ {
		minsf(&o, solve(n, m, ps, i))
	}
	outfl(o)
}
func solve(n, m int, ps [][4]int, mask int) float64 {
	o := 0.0
	uf := newUnionFind(n + m)
	type tst struct {
		d float64
	...
	getDist := func(i, j int) tst {
		fp := point{ps[i][1], ps[i][2]}
		tp := point{ps[j][1], ps[j][2]}
		d := 0.0
		if ps[i][0] == 0 && ps[j][0] == 0 {
			d = pointDist(fp, tp)
		}
		if ps[i][0] == 1 && ps[j][0] == 0 {
			d = float64(inf)
			minsf(&d, absf(pointDist(fp, tp)-float64(ps[i][3])))
		}
		if ps[i][0] == 0 && ps[j][0] == 1 {
			d = float64(inf)
			minsf(&d, absf(pointDist(fp, tp)-float64(ps[j][3])))
		}
		if ps[i][0] == 1 && ps[j][0] == 1 {
			d = float64(inf)
			if istri(pointDist(fp, tp), float64(ps[i][3]), float64(ps[j][3])) {
				d = 0.0
			} else {
				minsf(&d, absf(pointDist(fp, tp)-float64(ps[i][3])-float64(ps[j][3])))
				minsf(&d, absf(pointDist(fp, tp)-float64(ps[i][3])+float64(ps[j][3])))
				minsf(&d, absf(pointDist(fp, tp)+float64(ps[i][3])-float64(ps[j][3])))
				minsf(&d, absf(pointDist(fp, tp)+float64(ps[i][3])+float64(ps[j][3])))
			}
	...
	for i := 0; i < n+m; i++ {
		for j := i + 1; j < n+m; j++ {
			tsts = append(tsts, getDist(i, j))
		}
	}
	sort.Slice(tsts, func(i, j int) bool {
		return tsts[i].d < tsts[j].d
	})
	for _, v := range tsts {
		if v.i >= n && hasbit(mask, v.i-n) {
			continue
		}
		if v.j >= n && hasbit(mask, v.j-n) {
			continue
		}
		if uf.issame(v.i, v.j) {
			continue
		}
		uf.unite(v.i, v.j)
		o += v.d
