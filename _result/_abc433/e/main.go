...
func solve() {
	...
	pqn := newpq[pqst](func(p, q pqst) bool {
		return p.x > q.x
	})
	pqm := newpq[pqst](func(p, q pqst) bool {
		return p.x > q.x
	})
	for i := 0; i < n; i++ {
		pqn.Push(pqst{x: as[i], i: i})
	}
	for i := 0; i < m; i++ {
		pqm.Push(pqst{x: bs[i], i: i})
	}
	...
	sort.Slice(ts, func(i, j int) bool {
		t1 := min(as[ts[i][0]], bs[ts[i][1]])
		t2 := min(as[ts[j][0]], bs[ts[j][1]])
		if t1 == t2 {
			return rs[ts[i][0]][ts[i][1]] < rs[ts[j][0]][ts[j][1]]
		}
		return t1 < t2
	})
	i := 1
	used := make([]bool, n*m+10)
	for _, v := range ts {
		if rs[v[0]][v[1]] == 0 {
			rs[v[0]][v[1]] = i
			i++
