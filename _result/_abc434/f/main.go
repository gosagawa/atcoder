...
func solve() {
	...
	compare := func(x, y int) int {
		if len(ss[x]) < len(ss[y]) {
			return -compareSub(y, x)
		}
		return compareSub(x, y)
	}
	...
	if n == 2 {
		...
		if !isSame {
			...
			copy(ts1, ts)
			copy(ts2, ts)
			ts1[n-2], ts1[n-1] = ts1[n-1], ts1[n-2]
			ts2[n-3], ts2[n-2], ts2[n-1] = ts2[n-2], ts2[n-3], ts2[n-1]
			rs1 := []int{}
			rs2 := []int{}
			for i := 0; i < n; i++ {
				rs1 = append(rs1, ss[ts1[i]]...)
			}
			for i := 0; i < n; i++ {
				rs2 = append(rs2, ss[ts2[i]]...)
			}
			s1 := istos(rs1, 'a')
			s2 := istos(rs2, 'a')
			dbg(s1, s2)
