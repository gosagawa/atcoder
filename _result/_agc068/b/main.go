...
func main() {
	...
	for i := 0; i < int(1e5); i++ {
		...
		a := as[i]
		dbg(i, a, ihs[a].Len(), "ces", ces[a], "cs", cs[a], "ti", ti, uf.issame(i, ti))
		dbg(ss[:i])
		dbg(ts[:tni])
		ihs[a].Pop()
		if a == as[ti] && (ces[a] == cs[a]-1 || !uf.issame(i, ti)) {
			uf.unite(i, ti)
			ti++
			for {
				if ts[ti] == 0 {
			...
			if ihs[a].Len() > 0 {
				ni := ihs[a].Pop()
				dbg(ni, tni, ces[a], cs[a], uf.issame(i, ni))
				if ni > tni && (ces[a] == cs[a]-1 || !uf.issame(i, ni)) {
					ss[i] = 1
					ts[ni] = 1
					uf.unite(i, ni)
					tni = ni + 1
