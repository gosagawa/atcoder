...
func main() {
	...
	op := func(a segstruct, b segstruct) segstruct {
		return segstruct{(a.v2 + b.v2) % mod, (a.v + b.v) % mod, a.size + b.size}
	}
	e := func() segstruct {
		return segstruct{0, 0, 0}
	}
	id := func() segfstruct {
	...
	mapping := func(f segfstruct, x segstruct) segstruct {
		...
		t := int(f)
		return segstruct{(x.v2 + mmul(mmul(2, x.v), t) + mmul(mmul(x.size, t), t)) % mod, (x.v + t*x.size) % mod, x.size}
	}
	compostion := func(f segfstruct, g segfstruct) segfstruct {
		...
		return segfstruct((int(f) + int(g)) % mod)
	}
	base := make([]segstruct, n)
	for i := 0; i < n; i++ {
		base[i] = segstruct{0, 0, 1}
	}
	...
	for i := 0; i < q; i++ {
		l, r, a := ni3()
		l--
		lst.applyrange(l, r, segfstruct(a))

		v := lst.prod(l, r)

		o := mmul(madd(mmul(v.v, v.v), -v.v2), inv2)
		out(o)
	}
}
