...
func main() {
	...
	for i := 0; i < e; i++ {
		...
		s--
		t--
		edges[s] = append(edges[s], edge{to: t, fee: a, cost: b})
		edges[t] = append(edges[t], edge{to: s, fee: a, cost: b})
	}
	cs, ds := ni2s(v)

	g := newgraph(v, edges)

	g.starts = []state{{silver: s, node: 0}}

	score := make([][2501]int, v)
	for i := 0; i < v; i++ {
		for j := 0; j < 2501; j++ {
			score[i][j] = inf
		}
	...
	que := newpq(g.comps)
	score[0][s] = 0
	heap.Push(que, state{silver: s, node: 0})

	for !que.IsEmpty() {
		st := heap.Pop(que).(state)
		if st.score > score[st.node][st.silver] {
			continue
		}
		for _, edge := range g.edges[st.node] {
			if st.silver < edge.fee {
				continue
			}
			newScore := st.score + edge.cost
			newSilver := st.silver - edge.fee
			if score[edge.to][newSilver] > newScore {
				score[edge.to][newSilver] = newScore
				heap.Push(que, state{score: newScore, silver: newSilver, node: edge.to})
			}
		}

		newScore := st.score + ds[st.node]
		newSilver := min(2500, st.silver+cs[st.node])
		if score[st.node][newSilver] > newScore {
			score[st.node][newSilver] = newScore
			heap.Push(que, state{score: newScore, silver: newSilver, node: st.node})
		}
	}
	for i := 1; i < v; i++ {
		t := inf
