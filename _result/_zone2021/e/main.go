...
func main() {
	...
	for i := 0; i < r; i++ {
		for j := 0; j < c-1; j++ {
			...
			s := i*c + j
			t := i*c + j + 1
			edges[s] = append(edges[s], edge{to: t, cost: a})
			edges[t] = append(edges[t], edge{to: s, cost: a})
		}
		if i != 0 {
			for k := 0; k < c; k++ {
				s := i*c + k
				t := i*c - c + k
				edges[s] = append(edges[s], edge{to: t, cost: 1, opt: 1})
			}
		}
	}
	for i := 0; i < r-1; i++ {
...
func (g *graph) dijkstra(start int) [][]int {
	...
	for i := 0; i < g.size; i++ {
		score[i] = make([]int, 2)
		score[i][0] = g.defaultScore
		score[i][1] = g.defaultScore
	}
	que := newpq(g.comps)
	for _, start := range g.starts {
		score[start.node][0] = start.score
		heap.Push(que, start)
	}

	for !que.IsEmpty() {
		st := heap.Pop(que).(state)
		if st.score > score[st.node][st.opt] {
			continue
		}
		for _, edge := range g.edges[st.node] {

			newScore := st.score + edge.cost
			if edge.opt == 1 && st.opt != 1 {
				newScore++
			}
			if score[edge.to][edge.opt] > newScore {
				score[edge.to][edge.opt] = newScore
				heap.Push(que, state{score: newScore, node: edge.to, opt: edge.opt})
			}
		}
	}
	return score
