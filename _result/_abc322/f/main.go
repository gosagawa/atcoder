...
func main() {
	...
	mapping := func(a segfstruct, b segstruct) segstruct {
		if a.v == 1 {
			return b.flip()
		} else {
			return b
	...
	composition := func(a segfstruct, b segfstruct) segfstruct {
		return segfstruct{v: a.v ^ b.v}
	}
	id := func() segfstruct {
		return segfstruct{}
	}
	seg := newlazysegtree(n, v, op, e, mapping, composition, id)

	for qi := 0; qi < q; qi++ {
		t, l, r := ni3()
		l--
		if t == 1 {
			seg.applyrange(l, r, segfstruct{v: 1})
		} else {
			r := seg.prod(l, r)
			out(r.mx[1])
		}
...
func (l segstruct) merge(r segstruct) segstruct {
	rseg := l
	for i := 0; i < 2; i++ {
		rseg.l[i] = l.l[i]
		if l.l[i] == l.size {
		...
		maxs(&rseg.mx[i], l.mx[i])
		maxs(&rseg.mx[i], r.mx[i])
		maxs(&rseg.mx[i], l.r[i]+r.l[i])
	}
	rseg.size = l.size + r.size
	return rseg
}

func (seg segstruct) flip() segstruct {
	seg.mx[0], seg.mx[1] = seg.mx[1], seg.mx[0]
	seg.l[0], seg.l[1] = seg.l[1], seg.l[0]
	seg.r[0], seg.r[1] = seg.r[1], seg.r[0]
	return seg
}
