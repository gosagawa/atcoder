...
func main() {
	...
	for i := 0; i < e; i++ {
		...
		edges[s] = append(edges[s], edge{to: t, cost: cs})
		edges[t] = append(edges[t], edge{to: s, cost: ct})
	}

	graph := newgraph(v, edges)
	dist := graph.dijkstrae(0)
	o := 0
	for _, v := range dist {
		o = max(o, v)
	}
...
func (g *graph) dijkstrae(start int) []int {
