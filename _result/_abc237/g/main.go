...
func main() {
	...
	check := func(sl []int) []int {
		...
		op := func(i segstruct, j segstruct) segstruct {
			return segstruct{i.v + j.v, i.size + j.size}
		}
		e := func() segstruct {
		...
		mapping := func(i segfstruct, j segstruct) segstruct {
			if i == id() {
				return j
			}
			return segstruct{int(i) * j.size, j.size}
		}
		compostion := func(i segfstruct, j segfstruct) segfstruct {
			if i == id() {
				return j
			}
			return segfstruct(int(i))
		}
		lst := newlazysegtree(
			n,
			base,
		...
		for i := 0; i < q; i++ {
			st := lst.prod(ls[i], rs[i]+1)
			t := st.v
			if cs[i] == 0 {
				lst.applyrange(ls[i], rs[i]-t+1, segfstruct(0))
				lst.applyrange(rs[i]-t+1, rs[i]+1, segfstruct(1))
			} else {
				lst.applyrange(ls[i], ls[i]+t, segfstruct(1))
				lst.applyrange(ls[i]+t, rs[i]+1, segfstruct(0))
			}
		}
		for i := 0; i < n; i++ {
			t := lst.get(i)
