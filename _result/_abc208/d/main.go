...
func (g *graph) floydWarshall() ([][]int, bool, int) {
	...
	for k := 0; k < g.size; k++ {
		for i := 0; i < g.size; i++ {
			for j := 0; j < g.size; j++ {
				...
				if score[i][j] == g.defaultScore {
					sum += (score[i][k] + score[k][j]) * (g.size - k)

				} else if score[i][j] > score[i][k]+score[k][j] {
					sum -= (score[i][j] - score[i][k] - score[k][j]) * (g.size - k)
				}
