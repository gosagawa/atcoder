...
func main() {
	...
	n := ni()
	mp := convidxi2s(nsi2s(3), map[string]int{".": 0, "#": 1})
	...
	op := func(a segstruct, b segstruct) segstruct {
		...
		for i := 0; i < 3; i++ {
			for j := 0; j < 3; j++ {
				for k := 0; k < 3; k++ {
					if a.v[i][j] == inf || b.v[j][k] == inf {
						continue
					}
					mins(&r.v[i][k], a.v[i][j]+b.v[j][k]+1)
				}
			}
		}
		return r
	...
	for i := 0; i < n; i++ {
		base[i] = culc([]int{mp[0][i], mp[1][i], mp[2][i]})
	}
	...
	for qi := 0; qi < q; qi++ {
		...
		c--
		mp[r][c] ^= 1
		lst.set(c, culc([]int{mp[0][c], mp[1][c], mp[2][c]}))
		t := lst.allprod()
		o := -1
		if t.v[0][2] != inf {
			o = t.v[0][2]
