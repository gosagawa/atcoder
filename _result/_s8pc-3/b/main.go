...
func main() {
	...
	for i := 0; i < h; i++ {
		...
		s := ns()
		for j, v := range s {
			hs[i][j] = atoi(string(v))
		}
	}
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			score := getScore(i, j, h, w, k, hs)
			o = max(score, o)
		}
	}
	out(o)
...
func getScore(x, y, h, w, k int, nsorg [][]int) int {
		...
		for i := 0; i < h; i++ {
			...
			for j := 0; j < w; j++ {
				if ns[i][j] == check {
					columnCounter++
					if j == w-1 {
						if ns[i][j] != 0 && columnCounter >= k {
							for l := j + 1 - columnCounter; l < w; l++ {
								tmpo += ns[i][l]
								ns[i][l] = 0
							}
							erase = true
