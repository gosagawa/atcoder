...
func main() {
	...
	for i := 0; i < q; i++ {
		...
		case 3:
			...
			pp := []int{}
			np := []int{}
			var getPreNode func(n *node)
			getPreNode = func(n *node) {
				if n.pre != nil {
					pp = append(pp, n.pre.val)
					getPreNode(n.pre)
				}
			}
			getPreNode(ns[x])
			var getNextNode func(n *node)
			getNextNode = func(n *node) {
				if n.next != nil {
					np = append(np, n.next.val)
					getNextNode(n.next)
				}
			}
			getNextNode(ns[x])
			r := make([]string, len(pp)+len(np)+2)
			r[0] = itoa(len(pp) + len(np) + 1)
			idx := 1
