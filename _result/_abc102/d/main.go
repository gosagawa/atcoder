...
func main() {
	...
	as := nis(n)
	dsr := make([][2]int, n)
	conv := func(v1, v2 [2]int) bool {
		return abs(v1[0]-v1[1]) > abs(v2[0]-v2[1])
	}
	ti := 1
	dsr[1] = [2]int{as[0], as[1]}
	for i := 2; i < n; i++ {
		tv := dsr[i-1]
		tv[1] += as[i]
		for {
			...
			nv := [2]int{tv[0] + as[ti], tv[1] - as[ti]}
			if conv(tv, nv) {
				tv = nv
				ti++
			} else {
	...
	for i := 1; i < n-2; i++ {
		tm := min(min(dsr[i][0], dsr[i][1]), min(dsl[i+1][0], dsl[i+1][1]))
		tx := max(max(dsr[i][0], dsr[i][1]), max(dsl[i+1][0], dsl[i+1][1]))
		mins(&o, tx-tm)
	}
