...
func main() {
	...
	for i := 0; i < n; i++ {
		...
		for j, v := range ss {
			if v == 1 {
				...
				edgesr[t] = append(edgesr[t], edge{to: s, cost: c})
			}
		}
	}
	...
	graph2 := newgraph(v, edgesr)
	dist2 := graph2.dijkstra(n - 1)
	//	out(dist, dist2)
	r := []int{}
	for i := 1; i < n-1; i++ {
		t := inf
		for j := max(0, i-m); j <= i-1; j++ {
			if j == i {
				continue
			}
			if dist[j] == inf {
			...
			for _, nv := range edges[j] {
				if nv.to > i && dist2[nv.to] != inf {
					mins(&t, dist[j]+dist2[nv.to]+1)
				}
			}
		}
		if t == inf {
