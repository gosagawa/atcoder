...
func main() {
	...
	for i := 0; i < n-1; i++ {
		...
		s--
		t--
		edges[s] = append(edges[s], edge{to: t, cost: 1, rev: len(edges[t])})
		edges[t] = append(edges[t], edge{to: s, cost: 1, rev: len(edges[s]) - 1})
		ec[s]++
		ec[t]++
	}
	rs := is(n, 0)
	...
	getp := func(i, size int) int {
		...
		dfs = func(v, p int) int {
			...
			for _, nv := range edges[v] {
				if nv.to == p || nv.cost == 0 {
					continue
				}
				t += dfs(nv.to, v)
				if dist[nv.to] <= size/2 {
				} else {
					isP = false
				}
			}
			if isP && size-t <= size/2 {
				ans = v
			}
			dist[v] = t
			return t
	...
	delEdge := func(e edge) {
		re := edges[e.to][e.rev]
		edges[e.to][e.rev].cost = 0
		edges[re.to][re.rev].cost = 0
		ec[e.to]--
		ec[re.to]--
	}

	var dfs2 func(v, p int)
	dfs2 = func(v, p int) {
		rs[v] = p + 1
		for _, nv := range edges[v] {
			if nv.to == p || nv.cost == 0 {
			...
			if dist[nv.to] <= 2 {
				...
				dbg(v+1, nv.to+1, dist[nv.to])
				nr := getp(nv.to, dist[nv.to])
				dbg(nr + 1)
				getp(nr, dist[nv.to])
				dfs2(nr, v)
			}
