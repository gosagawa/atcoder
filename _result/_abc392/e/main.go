...
func main() {
	...
	rmp := []int{}
	for i := 0; i < n; i++ {
		if _, ok := mp[uf.root(i)]; !ok {
			rmp = append(rmp, uf.root(i))
			mp[uf.root(i)] = len(mp)
		}
	}
	dbg(rmp)
	...
	for i := 0; i < m; i++ {
		if uf2.issame(as[i], bs[i]) {
			dbg(i)
			rti := mp[uf.root(as[i])]
			lps[rti] = append(lps[rti], [3]int{i, as[i], bs[i]})
			lpcs[rti][1]++
		} else {
			uf2.unite(as[i], bs[i])
		}
	...
	for i := 0; i < mpc-1; i++ {
		...
		tg := rmp[lpcs[i+1][0]]
		if uf2.issame(fi, use[1]) {
			rs = append(rs, [3]int{use[0] + 1, use[1] + 1, tg + 1})
		} else {
			rs = append(rs, [3]int{use[0] + 1, use[2] + 1, tg + 1})
		}
		uf2.unite(fi, tg)
	}
	out(len(rs))
	for _, v := range rs {
		outf("%v %v %v", v[0], v[1], v[2])
	}
