...
func main() {
	...
	dp[0][st.x][st.y] = 0
	q := list.New()
	q.PushBack(pointv{x: st.x, y: st.y, v: 0})
	e := q.Front()
	dx := []int{1, 0, -1, 0, 1, 1, -1, -1}
	dy := []int{0, 1, 0, -1, 1, -1, 1, -1}
	for e != nil {
		p := e.Value.(pointv)
		dbg(p, dp[p.v][p.x][p.y])
		if p.x == en.x && p.y == en.y && p.v == 10 {
			out(dp[10][en.x][en.y])
			return
		}
		for k := 0; k < 4; k++ {
			np := point{x: p.x + dx[k], y: p.y + dy[k]}
			if np.isValid(h, w) {
				if mp[np.x][np.y] == p.v+1 && dp[p.v+1][np.x][np.y] == inf {
					dp[p.v+1][np.x][np.y] = dp[p.v][p.x][p.y] + 1
					q.PushBack(pointv{x: np.x, y: np.y, v: p.v + 1})
				} else if dp[p.v][np.x][np.y] == inf {
					dp[p.v][np.x][np.y] = dp[p.v][p.x][p.y] + 1
					q.PushBack(pointv{x: np.x, y: np.y, v: p.v})
				}
			}
		}
		e = e.Next()
