...
func main() {
	...
	cks3 := make([][]int, csmax)
	for i, v := range cs {
		cks[v] = append(cks[v], i)
	}
	for i, ck := range cks {
		for _, v := range ck {
			cks[i] = append(cks[i], v+d)
		}
	}
	for i, ck := range cks {
		for j, v := range ck {
			if j == 0 {
				cks2[i] = append(cks2[i], 0)
				cks3[i] = append(cks3[i], 0)
			} else {
				ds := v - ck[j-1]
				cks2[i] = append(cks2[i], cks2[i][j-1]+(ds-1)/l)
				cks3[i] = append(cks3[i], cks3[i][j-1]+(ds-1)/l+1)
			}
	...
	for i := 0; i < n; i++ {
		...
		t--
		o++
		dbg("loopst", loopst, "t_rest", t)

		looplen := len(cks2[k]) / 2
		looptotal := looplen + cks2[k][looplen]
		loopc := t / looptotal
		o += loopc * looplen
		t %= looptotal

		ci := upperBound(loopst-1, cks[k])
		ni := lowerBound(cks3[k][ci]+t+1, cks3[k])
		o += ni - ci
		dbg("ci", ci, "ni", ni)
