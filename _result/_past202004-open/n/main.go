...
func main() {
	...
	ts = append(ts, bs...)
	for i := 0; i < n; i++ {
		ts = append(ts, xs[i]+ds[i]+1)
		ts = append(ts, ys[i]+ds[i]+1)
	}
	_, mp := cocom(ts)
	dbg(mp)
	qs := [][5]int{}
	for i := 0; i < n; i++ {
		qs = append(qs, [5]int{1, mp[xs[i]], mp[ys[i]], mp[ys[i]+ds[i]+1], cs[i]})
		qs = append(qs, [5]int{1, mp[xs[i]+ds[i]+1], mp[ys[i]], mp[ys[i]+ds[i]+1], -cs[i]})
	}
	for i := 0; i < q; i++ {
		qs = append(qs, [5]int{2, mp[as[i]], mp[bs[i]], i, 0})
	}
	sort2ar(qs, opt2ar(1, asc), opt2ar(0, asc))
	dbg(qs)
	...
	for i := 0; i < len(qs); i++ {
		switch qs[i][0] {
		case 1:
			lst.applyrange(qs[i][2], qs[i][3], segfstruct(qs[i][4]))
		case 2:
			rs[qs[i][3]] = lst.get(qs[i][2]).v
		}
	}
...
func sort2ar(sl [][5]int, setters ...Sort2ArOption) {
	args := &Sort2ArOptions{}
