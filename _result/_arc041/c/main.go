...
func main() {
	...
	for i := 0; i < n; i++ {
		...
		if d == td {
			...
			if td == 0 {
				...
				for j := 0; j < len(tds)-1; j++ {
					o += tds[len(tds)-1] - tds[j] - j - 1
				}
			}
			if td == 0 {
				xs = append(xs, [3]int{tds[0], td, len(tds)})
			} else {
				xs = append(xs, [3]int{tds[len(tds)-1], td, len(tds)})
			}
			td = d
			tds = []int{x}
		}
	...
	for i := st; i < len(xs); i += 2 {
		if i == li {
			break
		}
		o += (xs[i+1][0] - xs[i][0] - 1) * max(xs[i+1][2], xs[i][2])
	}
