	...
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			if i != 0 {
				...
				if t == 1 {
					fc := cf.combination(i+j-1, i-1)
					tc := cf.combination(h+w-i-j-2, h-i-1)
					o += mmul(madd(fc, dp[i-1][j]*2), tc)
					o %= mod
					//out(fc, tc, i, j, o)
					dp[i][j] += dp[i-1][j] + fc
				} else {
			...
			if j != 0 {
				...
				if t == 1 {
					fc := cf.combination(i+j-1, i)
					tc := cf.combination(h+w-i-j-2, h-i-1)
					o += mmul(madd(fc, dp[i][j-1]*2), tc)
					o %= mod
					//out(fc, tc, i, j, o)
					dp[i][j] += dp[i][j-1] + fc
				} else {
