...
func main() {
	...
	xc := n - tc
	if k <= xc {
		sort2ar(xl, opt2ar(4, asc), opt2ar(3, asc))
		for i := 0; i < len(xl); i++ {
			if k == 0 {
				break
			}
			if xl[i][0] == 0 {
				for j := xl[i][1]; j >= xl[i][0]; j-- {
					s[j] = 1
					k--
					if k == 0 {
						break
		...
		sort2ar(yl, opt2ar(4, desc), opt2ar(3, desc))
		for i := 0; i < len(yl); i++ {
			if k == 0 {
				break
			}
			if yl[i][1] == n-1 {
				for j := yl[i][1]; j >= yl[i][0]; j-- {
					s[j] = 0
					k--
					if k == 0 {
						break
