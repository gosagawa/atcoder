...
func main() {
	...
	mergeSort(sl, 0, n-1, func(i, j int) bool {
		return !check(i+1, t1, j+1)
	})
	r := make([]int, n)
	for i := 0; i < n; i++ {
		r[sl[i]] = i + 1
...
func merge(sl []int, l int, m int, r int, f func(i, j int) bool) {
...
func mergeSort(sl []int, l int, r int, f func(i, j int) bool) {
	if l >= r {
		return
	}
	m := l + (r-l)/2
