...
func main() {
	...
	ts := []int{y}
	for i := 0; i < d-1; i++ {
		for _, v := range edges[ts[len(ts)-1]] {
			if dist[v.to] == dist[ts[len(ts)-1]]-1 {
				ts = append(ts, v.to)
			}
		}
	}
	...
	dist = is(d+1, 0)
	dist2 := make([]map[int]int, len(edges[root]))
	for i := range edges[root] {
		dist2[i] = make(map[int]int)
	}
	var dfs2 func(i, p, d, base int)
	dfs2 = func(i, p, d, base int) {
		...
		for _, nv := range edges[i] {
			...
			dfs2(nv.to, i, d+1, base)
		}
	}
	for i, v := range edges[root] {
		dfs2(v.to, root, 1, i)
	}
	debug(root, d)
	debug(dist)
	...
	for _, v := range dist2 {
		for i, v2 := range v {
			if i > d/2 {
				o = madd(o, mmul(v2, dist[d-i]-v[d-i]))
			}
		}
	}
	if d%2 == 0 {
