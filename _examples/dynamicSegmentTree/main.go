...
func main() {
	...
	q := ni()
	ys := nis(q)
	op := func(f, d dsegstruct) dsegstruct {
		if f.ct%2 == 0 {
			f.even += d.even
			f.odd += d.odd
		} else {
...
type dsegtree struct {
	n    int
	root *int
	op   func(dsegstruct, dsegstruct) dsegstruct
	e    func() dsegstruct
	d    []dsegnode
}

type dsegnode struct {
	index   int
	value   dsegstruct
	product dsegstruct
	left    *int
...
func (d *dsegtree) set(p int, x dsegstruct) {
	if p >= d.n {
		panic("p must less than n")
	}
	d._set(d.root, 0, d.n, p, x)
}

func (d *dsegtree) get(p int) dsegstruct {
	if p >= d.n {
		panic("p must less than n")
	}
	return d._get(d.root, 0, d.n, p)
}

func (d *dsegtree) prod(l, r int) dsegstruct {
	if l > r || r > d.n {
		panic("invalid l,r")
	}
	return d._prod(d.root, 0, d.n, l, r)
}

func (d *dsegtree) allProd() dsegstruct {
	if d.root != nil {
		return d.d[*d.root].product
	}
	return d.e()
}

/*
func (d *dsegtree) reset(l, r int) dsegstruct {
     assert(l <= r && r <= n);
	 if l > r || r > n{
		panic("invalid l,r")
	 }
	return d._reset(d.root, 0, d.n, l, r)
}

func (d *dsegtree) maxRight(l int,fn func(dsegstruct) bool ) int {
	 if l > n {
		panic("invalid l")
	 }
	 if !fn(d.e()){
		panic("fn(e()) must be true")
	 }
	return _maxRight(d.root,0,n,l,fn,d.e())
}

func (d *dsegtree) maxLeft(l int,fn func(dsegstruct) bool ) int {
	 if r > n {
		panic("invalid r")
	 }
	 if !fn(d.e()){
	...
	return _maxLeft(d.root,0,n,l,fn,d.e())
}
*/

func (d *dsegtree) update(i int) {
	nd := d.d[i]
	left := d.e()
	if *nd.left != -1 {
		left = d.d[*nd.left].product
	}
	right := d.e()
	if *nd.right != -1 {
		right = d.d[*nd.right].product
	}
	d.d[i].product = d.op(d.op(left, d.d[i].value), right)
}

func (d *dsegtree) _set(t *int, a, b, p int, x dsegstruct) {
	if *t == -1 {
		d.d = append(d.d, dsegnode{
			index:   p,
			value:   x,
			product: x,
			left:    &[]int{-1}[0],
	...
	if p < c {
		if d.d[*t].index < p {
			d.d[*t].index, p = p, d.d[*t].index
			d.d[*t].value, x = x, d.d[*t].value
		}
		d._set(d.d[*t].left, a, c, p, x)
	} else {
		if p < d.d[*t].index {
			d.d[*t].index, p = p, d.d[*t].index
			d.d[*t].value, x = x, d.d[*t].value
		}
		d._set(d.d[*t].right, c, b, p, x)
	}
	d.update(*t)
}

func (d *dsegtree) _get(t *int, a, b, p int) dsegstruct {
	if *t == -1 {
		return d.e()
	}
	if d.d[*t].index == p {
	...
	c := (a + b) >> 1
	if p < c {
		return d._get(d.d[*t].left, a, c, p)
	} else {
		return d._get(d.d[*t].right, c, b, p)
	}
}

func (d *dsegtree) _prod(t *int, a, b, l, r int) dsegstruct {
	if t == nil || b <= l || r <= a {
		return d.e()
	}
	if l <= a && b <= r {
		return d.d[*t].product
	}
	c := (a + b) >> 1
	result := d._prod(d.d[*t].left, a, c, l, r)
	if r <= d.d[*t].index && d.d[*t].index < r {
		result = d.op(result, d.d[*t].value)
	}
	return d.op(result, d._prod(d.d[*t].right, c, b, l, r))
}
