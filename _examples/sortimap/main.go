...
type sortimap struct {
	l   int
	idx []int
	val []int
}

func newsortimap() *sortimap {
	return &sortimap{}
}

func (s *sortimap) size() int {
	return s.l
}

func (s *sortimap) clear() {
	s.l = 0
	s.idx = []int{}
	s.val = []int{}
}

func (s *sortimap) begin() int {
	return 0
}

func (s *sortimap) end() int {
	return s.l
}

func (s *sortimap) erase(idx int) {
	s.l--
	s.idx = delIdx(idx, s.idx)
	s.val = delIdx(idx, s.val)
}
func (s *sortimap) upperBound(k int) int {
	idx := bs(0, s.l-1, func(c int) bool {
		return s.idx[c] <= k
	})
	return idx + 1
}

func (s *sortimap) find(k int) int {
	idx := bs(0, s.l-1, func(c int) bool {
		return s.idx[c] <= k
	})
	if idx == -1 {
...
func (s *sortimap) insert(k, v int) {
	if s.l == 0 {
		s.l++
	...
	if idx == -1 {
		s.idx = append([]int{k}, s.idx...)
		s.val = append([]int{v}, s.val...)
		s.l++
		return
	}
	if s.idx[idx] == k {
	...
	s.l++
	s.idx = addIdx(idx+1, k, s.idx)
	s.val = addIdx(idx+1, v, s.val)
}

func (s *sortimap) String() string {
	r := make([]string, len(s.idx))
	for i, v := range s.idx {
		r[i] = itoa(v) + ":" + itoa(s.val[i])
	}

	return "map[" + strings.Join(r, " ") + "]"
}
