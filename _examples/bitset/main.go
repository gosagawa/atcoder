...
func newbitset(n int) *bitset {
	return &bitset{data: make([]int, (n+maxlogn-1)/maxlogn)}
}

func (b *bitset) set(i int) {
	b.data[i/maxlogn] |= pow2(i % maxlogn)
}

func (b *bitset) reset(i int) {
	b.data[i/maxlogn] &= pow2(maxlogn) - 1 ^ pow2(i%maxlogn)
}

func (b *bitset) merge(bs *bitset, fn func(x, y int) int) {
	for i := 0; i < len(b.data); i++ {
		b.data[i] = fn(b.data[i], bs.data[i])
	}
}
