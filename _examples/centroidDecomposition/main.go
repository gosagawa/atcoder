...
func main() {
	...
	// メイン処理
	allSize := make(map[int]int)
	centroids := []int{}
	var FindCentroidRecursive func(int, int, int)
	FindCentroidRecursive = func(v, size, p int) {
		sizeSubtree[v] = 1
		whoIsParent[v] = p
		isCentroid := true
		allSize[as[v]]++
		for _, nv := range edges[v] {
			...
			FindCentroidRecursive(nv.to, size, v)
			if sizeSubtree[nv.to] > size/2 {
				isCentroid = false
			}
			sizeSubtree[v] += sizeSubtree[nv.to]
		}
		if size-sizeSubtree[v] > size/2 {
			isCentroid = false
		}
		if isCentroid {
	...
	funcCentroid := func(root, size int) (int, [][2]int) {
		subTrees := [][2]int{}
		centroids = []int{}
		FindCentroidRecursive(root, size, -1)
		dbg(centroids)
		center := centroids[0]
		for _, nv := range edges[center] {
			if isRemoved[nv.to] {
				continue
			}
			if nv.to == whoIsParent[center] {
				subTrees = append(subTrees, [2]int{nv.to, size - sizeSubtree[center]})
			} else {
				subTrees = append(subTrees, [2]int{nv.to, sizeSubtree[nv.to]})
			}
		}
		return center, subTrees
	}

	var dfs func(root, size int)
	dfs = func(root, size int) {
		center, subTrees := funcCentroid(root, size)
		dbg(center, subTrees)
		...
		for _, st := range subTrees {
			...
			mp := make(map[int]int)
			da := make(map[int]int)
			var dfs3 func(v, p, d, t int)
			dfs3 = func(v, p, d, t int) {
				mp[as[v]]++
				o += d * allSize[as[v]]
				da[as[v]] += d
				for _, nv := range edges[v] {
