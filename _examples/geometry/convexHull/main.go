...
func main() {
	...
	for i := 0; i < len(ch)-1; i++ {
		...
		dy := ch[i+1].y - ch[i].y
		if dx > 0 && dy < 0 {
			o -= culc(dx, -dy, ch[i].y-ymin)
		}
		if dx > 0 && dy > 0 {
			o -= culc(dx, dy, ch[i+1].y-ymin)
		}
		if dx < 0 && dy > 0 {
			o -= culc(-dx, dy, ymax-ch[i].y)
		}
		if dx < 0 && dy < 0 {
			o -= culc(-dx, -dy, ymax-ch[i+1].y)
		}

	}
	out(o)
}
func culc(x, y, ymax int) int {
	o := 0
	if ymax > y {
		o += (ymax - y) * x
	}
...
func convexHull(ps []point) []point {
	...
	for i := 0; i < n; i++ {
		for {
			...
			tp := point{qs[k-1].x - qs[k-2].x, qs[k-1].y - qs[k-2].y}
			tq := point{ps[i].x - qs[k-1].x, ps[i].y - qs[k-1].y}
			if tp.x*tq.y-tp.y*tq.x > 0 {
				break
			}
			k--
		}
