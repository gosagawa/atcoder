...
func main() {
	...
	for i := 0; i < n; i++ {
		...
		for j := 0; j < n; j++ {
			...
			pts[tj] = point{x: xs[j] - xs[i], y: ys[j] - ys[i]}
			tj++
		}

		sort.Slice(pts, func(i, j int) bool {
			if pts[i].quad() != pts[j].quad() {
				return pts[i].quad() < pts[j].quad()
			}
			return pointInnerProduct(pts[i], pts[j]) > 0
		})
		pts = append(pts, pts...)
		dbg(pts)
		for j := 0; j < n-1; j++ {
			op := point{x: -pts[j].x, y: -pts[j].y}
			ra := point{x: -pts[j].y, y: pts[j].x}
			r1 := bs(j, j+n-2, func(c int) bool {
				if pts[c].quad() == pts[j].quad() {
					return pointInnerProduct(pts[j], pts[c]) >= 0
				}
				if pts[c].quad() == op.quad() {
					return pointInnerProduct(pts[c], op) >= 0
				}
				return pts[c].quad() == ra.quad()
			})
			r2 := bs(j, r1, func(c int) bool {
				if pts[c].quad() == pts[j].quad() {
					return true
				}
				if pts[c].quad() == ra.quad() {
					return pointInnerProduct(pts[c], ra) >= 0
				}
				return false
			})
			if pointInnerProduct(ra, pts[r2]) == 0 {
				t2++
			}
			dbg(j, op, ra, r1, r2)
			t1 += r1 - r2
	...
	outf("%d %d %d", n*(n-1)*(n-2)/6-t1-t2, t2, t1)
